...
#include "analysis/PAOSelection/DOIBasedSelecter.h"
...
std::shared_ptr<Eigen::SparseMatrix<int>> DOIBasedSelecter::selectPAOs() {
  ...
  for (unsigned int iOrb = 0; iOrb < occC.cols(); ++iOrb) {
    for (const auto& index : idx) {
      ...
      unsigned int nImportant = (dois.block(index.first, iOrb, nAtomPAOs, 1).array().abs() >= _doiThreshold).count();
      if (nImportant != 0) {
