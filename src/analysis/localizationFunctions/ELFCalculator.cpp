...
ELFCalculator<SCFMode>::ELFCalculator(
  std::shared_ptr<SystemController> systemController):
  _systemController(systemController),
  _basisController(systemController->getAtomCenteredBasisController()){
    assert(_systemController);
    assert(_basisController);
}

template<Options::SCF_MODES SCFMode>
GridData<SCFMode> ELFCalculator<SCFMode>::calcKineticEnergyDensity(
    std::shared_ptr<GridController> gridController){
  assert(gridController);

  // Definitions
  CoefficientMatrix<SCFMode> coefficientMatrix(_systemController->getElectronicStructure<SCFMode>()->
                                            getMolecularOrbitals()->getCoefficients());
  auto basisFunctionOnGridController = BasisFunctionOnGridControllerFactory::produce(
          _systemController->getSettings(), _basisController, gridController);
    ...
    for (unsigned int i=0; i<nOccOrbitals_spin; ++i){
      ...
      for (unsigned int blockIndex = 0 ; blockIndex < nBlocks; ++blockIndex) {
        ...
        for (unsigned int blockData = 0; blockData < blockSize; ++blockData) {
          ...
          const unsigned int pointIndex = basisFunctionOnGridController->
...
GridData<SCFMode> ELFCalculator<SCFMode>::calculateELFOnGrid(std::shared_ptr<GridController> gridController){
    ...
    auto densityOnGrid = densOnGridCalculator->calcDensityOnGrid(electronicStructure->
                         getDensityMatrix());
    ...
    densOnGridCalculator->calcDensityAndGradientOnGrid (densityMatrix,
                                                        densityOnGrid,
                                                        densityGradientOnGrid);
    ...
    for_spin(densityOnGrid,nablaDensityOnGrid2,kineticEnergyDensity,elf){
      ...
      for (unsigned int i=0; i<nGridPoints; ++i){
        ...
        // Calculation of d
        const double d = (densityOnGrid_spin[i] > 1.0e-10) ? kineticEnergyDensity_spin[i]
                       - (nablaDensityOnGrid2_spin[i]/(8.0*densityOnGrid_spin[i])) : 0.0;
        // Calculation of elf
...
template<>
GridData<RESTRICTED> ELFCalculator<RESTRICTED>::calculateTotalELFOnGrid(
    std::shared_ptr<GridController> gridController) {
    return this->calculateELFOnGrid(gridController);
}

template<>
GridData<RESTRICTED> ELFCalculator<UNRESTRICTED>::calculateTotalELFOnGrid(
    std::shared_ptr<GridController> gridController){
    assert(gridController);

    // Definitions
    GridData<RESTRICTED> totalELF(gridController);
    auto electronicStructure = _systemController->
    ...
    auto densityOnGrid = densOnGridCalculator->calcDensityOnGrid(electronicStructure->
        getDensityMatrix());
    auto densityGradientOnGrid = makeGradient<DensityOnGrid<Options::SCF_MODES::UNRESTRICTED>>
        (gridController);
    ...
    for (unsigned int i=0; i<nGridPoints; ++i){
      // Calculation of d_0(r)
      const double d_0  = prefactor *(  pow(densityOnGrid.alpha[i],(5.0/3.0))
                                      + pow(densityOnGrid.beta[i] ,(5.0/3.0)));
      // Calculation of d
      double d = 0.0;
...
template<Options::SCF_MODES SCFMode>
GridData<SCFMode> ELFCalculator<SCFMode>::calculateELFTSOnGrid(
    std::shared_ptr<GridController> gridController) {
    assert(gridController);
    ...
    const unsigned int nGridPoints=gridController->getNGridPoints();
    auto densityMatrix(electronicStructure->getDensityMatrix());
    auto densityOnGrid = densOnGridCalculator->
                         calcDensityOnGrid(electronicStructure->getDensityMatrix());
    auto densityGradientOnGrid = makeGradient<DensityOnGrid<SCFMode>>(gridController);
    ...
    densOnGridCalculator->calcDensityAndDerivativesOnGrid(densityMatrix,
        densityOnGrid,
        densityGradientOnGrid,
    ...
    for_spin(laplaceDensityOnGrid,densityHessianOnGridxx,densityHessianOnGridyy,densityHessianOnGridzz){
      laplaceDensityOnGrid_spin.array() = densityHessianOnGridxx_spin.array() +
          densityHessianOnGridyy_spin.array() +
          densityHessianOnGridzz_spin.array();
    };
    ...
    for_spin(laplaceDensityOnGrid,densityOnGrid,nablaDensityOnGrid2,elf_TS){
      for (unsigned int i=0; i<nGridPoints; ++i){
        ...
        // Calculation of d_0(r)
        const double d_0 = 0.3*pow(((SCFMode==RESTRICTED?3.0:6.0)*M_PI*M_PI),(2.0/3.0))*pow(densityOnGrid_spin[i],(5.0/3.0));
        ...
        if (d_0 > 1.0e-10) elf_TS_spin[i]= 1.0/(1.0+(d_TS*d_TS)/(d_0*d_0));
      }
    };
    return elf_TS;
...
template<>
GridData<RESTRICTED> ELFCalculator<RESTRICTED>::calculateTotalELFTSOnGrid(
    std::shared_ptr<GridController> gridController) {
    return this->calculateELFTSOnGrid(gridController);
}

template<>
GridData<RESTRICTED> ELFCalculator<UNRESTRICTED>::calculateTotalELFTSOnGrid(
    std::shared_ptr<GridController> gridController){
    assert(gridController);
    ...
    auto densityGradientOnGrid = makeGradient<DensityOnGrid<Options::SCF_MODES::UNRESTRICTED>>
                                 (gridController);
    auto densityHessianOnGrid = makeHessian<DensityOnGrid<Options::SCF_MODES::UNRESTRICTED>>
                                (gridController);
    ...
    for (unsigned int i=0; i<nGridPoints; ++i){
      ...
      for_spin(d_0,kineticEnergyDensity_TS,densityOnGrid,nablaDensityOnGrid2,laplaceDensityOnGrid){
        if (densityOnGrid_spin[i] > 1.0e-10){
          kineticEnergyDensity_TS_spin = (d_0_spin + (1.0/72.0) *
                                         (nablaDensityOnGrid2_spin[i]/densityOnGrid_spin[i]) +
                                         (1.0/6.0) * laplaceDensityOnGrid_spin[i]);
        }
      };
      // Calculation of elf
      if (densityOnGrid.alpha[i] > 1.0e-10 && densityOnGrid.beta[i] > 1.0e-10 ){
        totalELF_TS[i]= 1.0/(1.0 + pow((kineticEnergyDensity_TS.alpha
                        + kineticEnergyDensity_TS.beta
                        ...
                        (pow(densityOnGrid.alpha[i],(5.0/3.0))
                        + pow(densityOnGrid.beta[i],(5.0/3.0)))),2.0));
      }
    }
    return totalELF_TS;
