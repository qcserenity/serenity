...
SEDD<SPIN>::getSEDDLambda(const std::shared_ptr<SystemController> systemController){
  ...
  auto calculateSEDD = [systemController](std::shared_ptr<GridController> gridController){
    ...
    densOnGridCalculator->calcDensityAndDerivativesOnGrid(densMatrix,
                                                          dens,
                                                          densGradOnGrid,
                                                          densHessOnGrid);
    ...
    tmp.array() = ( -1.0*densGradOnGrid.x.total().array() * scalarGradient.array() +dens.total().array()*
                 (  densGradOnGrid.x.total().array()*densHessOnGrid.xx.total().array()
    ...
    tmp.array() = ( -1.0*densGradOnGrid.y.total().array() * scalarGradient.array() +dens.total().array()*
                     (  densGradOnGrid.x.total().array()*densHessOnGrid.xy.total().array()
                      + densGradOnGrid.y.total().array()*densHessOnGrid.yy.total().array()
                      + densGradOnGrid.z.total().array()*densHessOnGrid.yz.total().array() ));
    result.array() += tmp.array()*tmp.array();
    tmp.array()  = ( -1.0*densGradOnGrid.z.total().array() * scalarGradient.array() +dens.total().array()*
                     (  densGradOnGrid.x.total().array()*densHessOnGrid.xz.total().array()
