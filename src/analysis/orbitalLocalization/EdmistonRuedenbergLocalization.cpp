...
template<Options::SCF_MODES SCFMode>EdmistonRuedenbergLocalization<SCFMode>::
EdmistonRuedenbergLocalization(std::shared_ptr<SystemController> systemController) :
_systemController(systemController) {

}

template<Options::SCF_MODES SCFMode>void EdmistonRuedenbergLocalization<SCFMode>::localizeOrbitals(
  ...
  for_spin(coefficients,nOccOrbs){
    ...
    while(true){
      ...
      for (unsigned int i = 0; i < nOccOrbs_spin; i++){
        for (unsigned int j = i + 1; j < nOccOrbs_spin; j++){
          double factorA = 0.0;
          double factorB = 0.0;
