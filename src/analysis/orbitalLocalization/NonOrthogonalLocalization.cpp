...
template<Options::SCF_MODES T>NonOrthogonalLocalization<T>::NonOrthogonalLocalization(std::shared_ptr<SystemController> systemController) :
         _system(systemController) {

}


template<Options::SCF_MODES T>void NonOrthogonalLocalization<T>::localizeOrbitals(
    OrbitalController<T>& orbitals,unsigned int maxSweeps){
  ...
  auto basisFuncOnGridController(BasisFunctionOnGridControllerFactory::produce(
      _system->getSettings(), basisController, gridController));
  ...
  Matrix<double> IntMatrix(basisController->getNBasisFunctions(),
  ...
  for (unsigned int i = 0; i < nOccOrbs_spin; i++) {
    for (unsigned int j = 0; j < nOccOrbs_spin; j++) {
      double factorA = 0.0;
      double factorB = 0.0;
      for (unsigned int m = 0; m < 3; m++) {
        factorA += moDipoleInt[m](i, j) * moDipoleInt[m](i, j)
                    - ((moDipoleInt[m](i, i) - moDipoleInt[m](j, j)) * (moDipoleInt[m](i, i)
                        - moDipoleInt[m](j, j)))
                        / 4;
  ...
  for (unsigned i = 0; i < nOccOrbs_spin; i++) {
    for (unsigned j = 0; j < nOccOrbs_spin; j++) {
  ...
  for (unsigned int i=0; i < nOccOrbs_spin; i++){
    ...
    auto const updateFunction2 = [&]
                                  (const Eigen::VectorXd& parameters2,
                                      double& value2,
                                      Eigen::VectorXd& gradients2,
                                      std::shared_ptr<Eigen::MatrixXd> hessian2,
                                      bool print2){
      (void)print2;
      (void)hessian2;
      cycle=0;
      ...
      auto const updateFunction = [&]
        ...
        for (unsigned int j=0; j < 3; j++){
          ...
          wValue += 20.0*(dipoleScalar-moDipoleInt[j](i,i))
                        *(dipoleScalar-moDipoleInt[j](i,i));
