...
template<Options::SCF_MODES SCFMode>PipekMezeyLocalization<SCFMode>::PipekMezeyLocalization(
...
template<Options::SCF_MODES SCFMode>void PipekMezeyLocalization<SCFMode>::localizeOrbitals(
    OrbitalController<SCFMode>& orbitals,unsigned int maxSweeps) {
...
#pragma omp for schedule(dynamic)
        for (unsigned int s=0; s<nOccOrbs_spin; ++s) {
          ...
          for (unsigned int t=0; t<s; ++t) {
            ...
            for (unsigned int a=0; a<nAtoms; ++a) {
              ...
              for (unsigned int mu = basisIndices[a].first; mu < basisIndices[a].second; ++mu) {
                ...
                for (unsigned int nu=0; nu<nBasisFunctions; ++nu) {
                  Q_a_st[a] += (coefficientsMuS * coefficients_spin(nu,t) +
                      coefficients_spin(nu,s) * coefficientsMuT )
                      * overlaps(nu,mu);
                }
              }
              Q_a_st[a] *= 0.5;
      ...
      if (fabs(P-oldLocalizationMeasure) < _convThreshold or counter==maxSweeps) break;
