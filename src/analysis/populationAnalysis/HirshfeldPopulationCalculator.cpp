...
HirshfeldPopulationCalculator<SCFMode>::HirshfeldPopulationCalculator(std::shared_ptr<SystemController> systemController,
    ...
    _densOnGrid(densOnGrid),
    _atomPopulations(nullptr){
...
template<Options::SCF_MODES SCFMode>
void
HirshfeldPopulationCalculator<SCFMode>::calculateHirshfeldAtomPopulations(){
  ...
  ScalarOperatorToMatrixAdder<RESTRICTED> gridToMat(targetBasisFunctionOnGridController,_system->getSettings().grid.blockAveThreshold);
  ...
  for (unsigned int iAtom=0; iAtom < nAtoms; ++iAtom){
    ...
    atomDensityMatrix=proMolecularDensityMat.block(indices[iAtom].first,indices[iAtom].first,indices[iAtom].second-indices[iAtom].first,indices[iAtom].second-indices[iAtom].first);
...
#pragma omp for schedule(dynamic)
    for (unsigned int block=0; block<nBlocks; block++){
      ...
      for (unsigned int i=blockStart; i< blockEnd; ++i){
        if (proMolDens[i]<1.0e-12) continue;
