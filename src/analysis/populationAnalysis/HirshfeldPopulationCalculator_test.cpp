...
TEST_F(HirshfeldPopulationCalculatorTest, populationsRestricted) {
  ...
  auto densOnGridController=std::make_shared<DensityMatrixDensityOnGridController<RESTRICTED>>(densOnGridCalculator,systemController->template getElectronicStructure<RESTRICTED>()->getDensityMatrixController());
  ...
  std::remove((systemController->getSettings().path+"C_FREE/C_FREE.orbs.res.h5").c_str());
  std::remove((systemController->getSettings().path+"C_FREE/C_FREE.energies.res.h5").c_str());
  std::remove((systemController->getSettings().path+"C_FREE/C_FREE.dmat.res.h5").c_str());
  std::remove((systemController->getSettings().path+"C_FREE").c_str());

  std::remove((systemController->getSettings().path+"O_FREE/O_FREE.settings").c_str());
  std::remove((systemController->getSettings().path+"O_FREE/O_FREE.xyz").c_str());
  std::remove((systemController->getSettings().path+"O_FREE/O_FREE.orbs.res.h5").c_str());
  std::remove((systemController->getSettings().path+"O_FREE/O_FREE.energies.res.h5").c_str());
  std::remove((systemController->getSettings().path+"O_FREE/O_FREE.dmat.res.h5").c_str());
  std::remove((systemController->getSettings().path+"O_FREE").c_str());
...
TEST_F(HirshfeldPopulationCalculatorTest, populationsRestricted2) {
   ...
   */
  auto systemController =
    SystemController__TEST_SUPPLY::getSystemController(TEST_SYSTEM_CONTROLLERS::WaterMonOne_6_31Gs_DFT);
   ...
   * convergence criteria need to be tightened (take a look at the AtomDensityGuessCalculator for this).
   */
  EXPECT_NEAR(populations[0], 0.83529197517882869, 1e-2);
  EXPECT_NEAR(populations[1], 0.83395609847200558, 1e-2);
  EXPECT_NEAR(populations[2], 8.3307968792775249, 1e-2);

  std::remove((systemController->getSettings().path+"H_FREE/H_FREE.settings").c_str());
  std::remove((systemController->getSettings().path+"H_FREE/H_FREE.xyz").c_str());
  std::remove((systemController->getSettings().path+"H_FREE/H_FREE.orbs.res.h5").c_str());
  std::remove((systemController->getSettings().path+"H_FREE/H_FREE.energies.res.h5").c_str());
  std::remove((systemController->getSettings().path+"H_FREE/H_FREE.dmat.res.h5").c_str());
  std::remove((systemController->getSettings().path+"H_FREE").c_str());
...
TEST_F(HirshfeldPopulationCalculatorTest, populationsUnrestricted) {
  ...
  auto densOnGridCalculator = std::make_shared<DensityOnGridCalculator<UNRESTRICTED> >(
       basisFunctionOnGridController, systemController->getSettings().grid.blockAveThreshold);
  auto densOnGridController=std::make_shared<DensityMatrixDensityOnGridController<UNRESTRICTED>>(densOnGridCalculator,systemController->template getElectronicStructure<UNRESTRICTED>()->getDensityMatrixController());
  ...
  std::remove((systemController->getSettings().path+"H_FREE/H_FREE.settings").c_str());
  std::remove((systemController->getSettings().path+"H_FREE/H_FREE.xyz").c_str());
  std::remove((systemController->getSettings().path+"H_FREE/H_FREE.orbs.unres.h5").c_str());
  std::remove((systemController->getSettings().path+"H_FREE/H_FREE.energies.unres.h5").c_str());
  std::remove((systemController->getSettings().path+"H_FREE/H_FREE.dmat.unres.h5").c_str());
  std::remove((systemController->getSettings().path+"H_FREE").c_str());
