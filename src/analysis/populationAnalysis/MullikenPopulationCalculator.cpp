...
MullikenPopulationCalculator<SCFMode>::calculateMullikenPopulations(
    std::shared_ptr<SystemController> systemController) {
  return calculateAtomPopulations(
          systemController->getElectronicStructure<SCFMode>()->getDensityMatrix(),
...
template<Options::SCF_MODES SCFMode>
SpinPolarizedData<SCFMode, Eigen::VectorXd >
MullikenPopulationCalculator<SCFMode>::calculateAtomPopulations(
  ...
  const auto basisFunctionPopulations =
      calculateBasisFunctionPopulations(densityMatrix, overlapMatrix);
  /* Prepare output */
...
template<Options::SCF_MODES SCFMode>SpinPolarizedData<SCFMode, Eigen::VectorXd >
MullikenPopulationCalculator<SCFMode>::calculateBasisFunctionPopulations(
...
template<Options::SCF_MODES SCFMode>SPMatrix<SCFMode>
MullikenPopulationCalculator<SCFMode>::calculateAtomwiseOrbitalPopulations(
  ...
  for (unsigned int i=0; i<nOrbitals; ++i) {
    for_spin(atomwiseOrbitalPopulations, coefficients) {
      const auto basisFunctionOrbitalPopulations =
          calculateOrbitalPopulations(coefficients_spin.col(i), overlapMatrix);
      /*
       * Loop over atoms
       */
