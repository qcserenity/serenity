...
TEST_F(MullikenPopulationCalculatorTest, atomPopulationsUnrestricted) {
  ...
  auto basisController =
      BasisController__TEST_SUPPLY::getAtomCenteredBasisController(TEST_BASIS_CONTROLLERS::MINIMAL);
  /*
   * Set up input data
   */
  ...
  auto populations =
    Calculator.calculateAtomPopulations(
      (const DensityMatrix<Options::SCF_MODES::UNRESTRICTED>)densityMatrix,
      (const MatrixInBasis<RESTRICTED>)overlaps,
      indices);
  /*
   * Check results
   */
...
TEST_F(MullikenPopulationCalculatorTest, atomOrbPopulationsRestricted) {
  ...
  MullikenPopulationCalculator<Options::SCF_MODES::RESTRICTED> Calculator;
  auto populations =
    Calculator.calculateAtomwiseOrbitalPopulations(
      (const CoefficientMatrix<Options::SCF_MODES::RESTRICTED>)coeffMatrix,
      (const MatrixInBasis<RESTRICTED>)overlaps,
      indices);
  /*
   * Check results
...
TEST_F(MullikenPopulationCalculatorTest, atomOrbPopulationsUnrestricted) {
  ...
  auto populations =
    Calculator.calculateAtomwiseOrbitalPopulations(
      (const CoefficientMatrix<Options::SCF_MODES::UNRESTRICTED>)coeffMatrix,
      (const MatrixInBasis<RESTRICTED>)overlaps,
      indices);
  /*
   * Check results
