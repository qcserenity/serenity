...
AtomCenteredBasisController::AtomCenteredBasisController(
...
std::unique_ptr<Basis> AtomCenteredBasisController::produceBasisFunctionVector() {
  ...
  for (const auto& atom : _geometry->getAtoms()) {
    if (!atom->basisFunctionsExist(_basisLabel)){
      BasisFunctionProvider::provideAtomWithBasisFunction(
          *atom,
          _basisLibrary,
          _basisLoadingData[atomCount].label,
          _makeSphericalBasis,
          _makePrimary,
          _firstECP);
    }
    ...
    if(_basisLoadingData[atomCount].shells.size()!=0){
      for(unsigned int shell=0; shell<thisAtomBasis.size(); shell++){
        if(_basisLoadingData[atomCount].shells[shell]) basis->push_back(thisAtomBasis[shell]);
      }
    }else{
...
void AtomCenteredBasisController::postConstruction() {
  ...
  for (unsigned int atomIndex=0; atomIndex<_geometry->getNAtoms(); atomIndex++) {
    ...
    endIndex = this->extendedIndex(basisIndex-1) +
        (*this->_basis)[basisIndex-1]->getNContracted();
    }
    ...
    _basisIndicesRedOfAtom.push_back(
        std::pair<unsigned int, unsigned int>(firstIndexRed, endIndexRed));
  }
}
