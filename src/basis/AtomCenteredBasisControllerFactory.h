 ...
 */
class AtomCenteredBasisControllerFactory : public RememberingFactory<
      AtomCenteredBasisController,
      const std::shared_ptr<Geometry>, const std::string, bool, bool, const std::string, int> {
private:
...
public:
   ...
   */
  static std::shared_ptr<AtomCenteredBasisController> produce(
...
private:
  std::unique_ptr<AtomCenteredBasisController> produceNew(
      const std::shared_ptr<Geometry> geometry,
      const std::string basisLibrary,
      bool makeSphericalBasis,
      bool makePrimary,
      const std::string basisLabel,
      int firstECP) override final;
  /*
