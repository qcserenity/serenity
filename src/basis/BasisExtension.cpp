...
void BasisExtension::extendAllSystems(
...
inline void BasisExtension::addGhostAtoms(
    std::vector<Eigen::VectorXi > newImportantShells,
    ...
    if (impShellsOnAtom.sum()>0) {
      ...
      for(unsigned int iShell = 0; iShell < nShells; ++iShell) {
        if(impShellsOnAtom(iShell)) {
          ...
          std::cout <<"l: "<< shell->getAngularMomentum()
                << "      Spherical? " << spherical
...
inline std::vector<Eigen::VectorXi> BasisExtension::getListOfImportantShells(
    std::shared_ptr<SystemController> environmentSystem,
  ...
  Eigen::MatrixXd s_AB = libint.compute1eInts(
      libint2::Operator::overlap,envBasisController,activeSystem->getBasisController());
  // vector of important environment shells
  std::vector<Eigen::VectorXi> importantShells; // = Eigen::VectorXi::Zero(envBasisController->getReducedNBasisFunctions())
...
inline std::vector<std::shared_ptr<Atom> > BasisExtension::buildActiveSystemAtoms(
    std::shared_ptr<SystemController> activeSystem) {
...
inline void BasisExtension::buildBasisFromGeometry(
    std::shared_ptr<SystemController> activeSystem,
    std::vector<std::shared_ptr<Atom> > activeAtoms,
  ...
  // set new basis
  auto newBas = std::make_shared<AtomCenteredBasisController>(geom,
      activeSystem->getSettings().basis.basisLibPath,
      activeSystem->getSettings().basis.makeSphericalBasis,true,
      activeSystem->getAtomCenteredBasisController()->getBasisLabel(),
      37,
      importantActiveShells);
  auto newAuxBas = std::make_shared<AtomCenteredBasisController>(geom,
      activeSystem->getSettings().basis.basisLibPath,
      activeSystem->getSettings().basis.makeSphericalBasis,false,
