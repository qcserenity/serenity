...
BasisFunctionMapper::BasisFunctionMapper(
    std::shared_ptr<BasisController> basisControllerA):
  _basisControllerA(basisControllerA){}


std::shared_ptr<BasisController> BasisFunctionMapper::getDifferentialBasis(
  ...
  for(const auto shellB : basisControllerB->getBasis()) {
    ...
    if(shellIndexInA >= nShellsA) differentialBasis.push_back(shellB);
  }
  if(differentialBasis.size()==0) {
    if(iOOptions.printDebugInfos)
      std::cout << "  Remark: Basis Function Mapper -- Basis sets are identical!"<<std::endl;
    return nullptr;
  }
  auto customBasisController = std::make_shared<CustomBasisController>(
      differentialBasis,
      "DifferentialBasis");
  return customBasisController;
}

std::shared_ptr<Eigen::SparseMatrix<double> > BasisFunctionMapper::getSparseProjection(
      std::shared_ptr<BasisController> basisControllerB) {
  ...
  assert(row+1==basisControllerB->getNBasisFunctions());
  Eigen::SparseMatrix<double> sparseProjection(
  ...
  for(unsigned int iCol = 0; iCol < sparseProjection.cols();++iCol) assert(sparseProjection.col(iCol).sum()<=1);
...
unsigned int BasisFunctionMapper::getShellIndex(const Shell& other) {
  ...
  for(iShellA = 0; iShellA < basisA.size(); ++iShellA) {
    if(*basisA[iShellA]==other) return iShellA;
  }
  return iShellA + 1;
}
