...
double CartesianToSphericalTransformer::coef(unsigned int l, unsigned int abs_m, unsigned int t, unsigned int u, unsigned int vtimes2) {
  ...
  double c = pre * pow(0.25,t)
           *boost::math::binomial_coefficient<double>(l,t)
           *boost::math::binomial_coefficient<double>(l-t,abs_m+t)
           *boost::math::binomial_coefficient<double>(t,u)
           *boost::math::binomial_coefficient<double>(abs_m,vtimes2);
  return c;
}

unsigned int CartesianToSphericalTransformer::mapToCartHarmonics(
    unsigned int l, unsigned int abs_m, unsigned int ttimes2, unsigned int utimes2, unsigned int vtimes2) {
  ...
  for (int x = l; x >= 0; --x) {
    for (int y = l-x; y >= 0; --y) {
      int z = l - x - y;
      if (x == lx && y == ly && z == lz) return index;
      index++;
    }
  }
