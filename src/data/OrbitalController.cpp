...
template<Options::SCF_MODES T>OrbitalController<T>::OrbitalController(
...
template<Options::SCF_MODES T>
OrbitalController<T>::OrbitalController(std::string filePath,
    std::shared_ptr<BasisController> basisController,
    std::string id):
        _coefficients(
        new CoefficientMatrix<T>(basisController)),
        _basisController(basisController),
        _eigenvalues(
            new SpinPolarizedData<T, Eigen::VectorXd >(basisController->getNBasisFunctions())),
        _canOrthTh(1.0e-7),
        _linearDependent(false),
        _nZero(0),
        _fBaseName(filePath),
...
void OrbitalController<T>::calculateTransformationX(std::shared_ptr<OneElectronIntegralController> oneIntController) {
  ...
  if (_linearDependent) {
    WarningTracker::printWarning((std::string)"Warning: Basis-Set (near) linear dependent. Will try to use canonical orthogonalization. Removed " + _nZero + " columns from transformation matrix.",true);
  }
}

template<Options::SCF_MODES T>void OrbitalController<T>::updateOrbitals(
    const FockMatrix<T>& fockMatrix,
    std::shared_ptr<OneElectronIntegralController> oneIntController) {
  auto shift = std::make_pair<Eigen::VectorXd,SpinPolarizedData<T, Eigen::VectorXd > >(Eigen::VectorXd(Eigen::VectorXd::Zero(2)),SpinPolarizedData<T, Eigen::VectorXd >(Eigen::VectorXd(0)));
  this->updateOrbitals(shift,fockMatrix,oneIntController);
}

template<Options::SCF_MODES T>void OrbitalController<T>::updateOrbitals(
    const std::pair<Eigen::VectorXd,SpinPolarizedData<T, Eigen::VectorXd > > levelshift,
    const FockMatrix<T>& fockMatrix,
    std::shared_ptr<OneElectronIntegralController> oneIntController) {
  ...
  if (!_fIsInOthoBasis and !_customS) calculateTransformationX(oneIntController);
    ...
    for_spin(fockMatrix, eps, c, occupation) {
      ...
      if (levelshift.first[0] > 0.0 and !_firstIteration){
        ...
        for(unsigned int i=0; i<occupation_spin.size(); i++){
          if(occupation_spin[i]==0){
            tmpFockMatrix(i,i)+=levelshift.first[0];
            for(unsigned int j=0; j<occupation_spin.size(); j++){
              if(occupation_spin[j]!=0){
...
template<Options::SCF_MODES T>
void OrbitalController<T>::fromHDF5(
    std::string fBaseName, std::string id){
...
template<>void OrbitalController<Options::SCF_MODES::RESTRICTED>::coefficientsfromHDF5(
    std::string fBaseName, std::string id){
...
template<>void OrbitalController<Options::SCF_MODES::UNRESTRICTED>::coefficientsfromHDF5(
    std::string fBaseName, std::string id){
...
template<>void OrbitalController<Options::SCF_MODES::RESTRICTED>::toHDF5(
    std::string fBaseName, std::string id) {
...
template<>void OrbitalController<Options::SCF_MODES::UNRESTRICTED>::toHDF5(
    std::string fBaseName, std::string id) {
...
template<>void OrbitalController<Options::SCF_MODES::RESTRICTED>::eigenvaluesfromHDF5(
    std::string fBaseName, std::string id){
  HDF5::Filepath name(fBaseName+".orbs.res.h5");
  HDF5::H5File file(name.c_str(),H5F_ACC_RDONLY,H5P_DEFAULT);
  ...
  HDF5::attribute_exists(file,"ID");
  HDF5::check_attribute(file,"ID",id);
  _eigenvalues.reset(new SpinPolarizedData<Options::SCF_MODES::RESTRICTED, Eigen::VectorXd >(_basisController->getNBasisFunctions()));
  HDF5::load(file,"eigenvalues",*_eigenvalues);
  file.close();
}
template<>void OrbitalController<Options::SCF_MODES::UNRESTRICTED>::eigenvaluesfromHDF5(
    std::string fBaseName, std::string id){
  HDF5::Filepath name(fBaseName+".orbs.unres.h5");
  HDF5::H5File file(name.c_str(),H5F_ACC_RDONLY,H5P_DEFAULT);
  ...
  HDF5::attribute_exists(file,"ID");
  HDF5::check_attribute(file,"ID",id);
  _eigenvalues.reset(new SpinPolarizedData<Options::SCF_MODES::UNRESTRICTED, Eigen::VectorXd >(_basisController->getNBasisFunctions()));
