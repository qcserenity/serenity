...
class SparseMapsController {
 public:
   ...
   * @param mullikenThreshold The Mulliken population threshold which is used to truncate
   *                          the auxiliary basis for each orbital pair.
   * @param orbitalToShellThreshold The Mulliken population threshold which is used to truncate
   *                                the AO basis for each occupied orbital based on its coefficients.
   */
  SparseMapsController(std::shared_ptr<SystemController> system, std::shared_ptr<PAOController> paoController,
                       ...
                       std::vector<std::shared_ptr<OrbitalPair>> distantOrbitalPairs, double mullikenThreshold,
                       double orbitalToShellThreshold, double strongTripletMullikenThreshold = 1e-3,
                       double weakTripletMullikenThreshold = 1e-3);
  /**
   ...
   * @param Defines the threshold used for the map construction.
  ...
  /**
   * @brief Getter for the extended map occ->aux. shell.
   *        Map extension is done with close pairs only.
   * @return The extended map occ->aux. shell.
   */
  const SparseMap& getCloseExtendedOccToAuxShellMap();
  /**
 ...
 private:
  ...
  std::shared_ptr<SparseMap> constructShellToOrbitalMap(const Eigen::MatrixXd& coefficients);
  /*
  ...
  SparseMap buildOccToAtomMap(double threshold);
  ...
  // Prescreening thresholds for pair aux. domain construction.
  double _mullikenThreshold;
  // Prescreening threshold for orbital coefficients.
  double _orbitalToShellThreshold;
  // Prescreening threshold for strong triplet aux. domain construction.
  double _strongTripletMullikenThreshold;
  // Prescreening threshold for weak triplet aux. domain construction.
  double _weakTripletMullikenThreshold;
