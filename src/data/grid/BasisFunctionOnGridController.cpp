...
#endif
    _workspace[i].reset(new BasisFunctionBlockOnGridData(
            _basisController->getNBasisFunctions(), _maxBlockSize, _highestDerivative));
  }
...
void BasisFunctionOnGridController::setHighestDerivative(unsigned int newHighestDerivative) {
  ...
  if (newHighestDerivative > _highestDerivative) {
    ...
    for (int i=0; i<omp_get_max_threads(); ++i) {
      _workspace[i].reset(new BasisFunctionBlockOnGridData(
              _basisController->getNBasisFunctions(), _maxBlockSize, newHighestDerivative));
    }
  }
...
const std::shared_ptr<BasisFunctionOnGridController::BasisFunctionBlockOnGridData>
    BasisFunctionOnGridController::getBlockOnGridData(
        const unsigned int blockIndex) {
  assert(blockIndex < _nBlocks);
  return calculateBasisFunctionData(blockIndex);
}

unsigned int BasisFunctionOnGridController::getFirstIndexOfBlock(
    const unsigned int blockIndex) {
  assert(blockIndex < _nBlocks);
...
std::shared_ptr<BasisFunctionOnGridController::BasisFunctionBlockOnGridData>
    BasisFunctionOnGridController::calculateBasisFunctionData(
      const unsigned int blockIndex) {
  assert(blockIndex < _nBlocks);

  if (omp_get_thread_num()==0) Timings::takeTime("Tech. -    Basis On Grid Eval.");
  ...
  if (_highestDerivative >= 1)
    zip(*thisBlockData->derivativeValues, derivatives, [](Matrix<double>& matrix, double*& ptr){
      ptr = matrix.data();});
  Hessian<double*> secondDerivatives;
  if (_highestDerivative >= 2)
    zip(*thisBlockData->secondDerivativeValues, secondDerivatives, [](Matrix<double>& matrix, double*& ptr){
      ptr = matrix.data();});
  ...
  double spread = (_gridController->getGridPoints().block(0,firstIndex,3,blockSize).colwise() - center).colwise().norm().maxCoeff();
  ...
  for (unsigned int iBlock=0; iBlock < blockSize; iBlock++) {
     ...
     */
    for (unsigned int muRed = 0; muRed < nReducedBasisFunctions; muRed++) {
      if (thisBlockData->negligible[_basisController->extendedIndex(muRed)]) continue;
      ...
      if (fabs(radial) < _radialThreshold) {
        ...
        for (unsigned int iMuP = iMuPStart; iMuP < iMuPEnd; iMuP+=blockSize) {
          values[iMuP] = 0.0;
          if (_highestDerivative >= 1) {
            for (auto& component : derivatives) component[iMuP] = 0.0;
            if (_highestDerivative >= 2) {
              for (auto& component : secondDerivatives) component[iMuP] = 0.0;
            }
          }
        }
        continue;
      ...
      if (_highestDerivative >= 1) {
        zip(
            ...
            [&iMuPStart](double*& store, double*& orig) {store = orig + iMuPStart;});
        if (_highestDerivative >= 2) {
          zip(
              hessianStore, secondDerivatives,
              [&iMuPStart](double*& store, double*& orig) {store = orig + iMuPStart;});
        }
      }
      offset = blockSize;
      ...
      if (!basMu->isSpherical()) {
        for (int muX = angularMomentumOfMu, iMuP = 0, iMu = iMuStart, iMuShell = 0; muX >= 0;
            muX--) {
          for (int muY = angularMomentumOfMu - muX; muY >= 0;
              muY--, iMuP += offset, iMu++, iMuShell++) {
            ...
            if (_highestDerivative >= 1) {
              ...
              derivativeStore.x[iMuP] = dradial * x[muX + 1] * y[muY]
                                        * z[muZ] * normFactorOfMu[iMuShell];
              if (muX > 0) {
                derivativeStore.x[iMuP] += muX * x[muX - 1] * y[muY] * z[muZ] * radial
                                           * normFactorOfMu[iMuShell];
              }
              derivativeStore.y[iMuP] = dradial * x[muX] * y[muY + 1]
                                        * z[muZ] * normFactorOfMu[iMuShell];
              if (muY > 0) {
                derivativeStore.y[iMuP] += muY * x[muX] * y[muY - 1] * z[muZ] * radial
                                           * normFactorOfMu[iMuShell];
              }
              derivativeStore.z[iMuP] = dradial * x[muX] * y[muY]
                                        * z[muZ + 1] * normFactorOfMu[iMuShell];
              if (muZ > 0) {
                derivativeStore.z[iMuP] += muZ * x[muX] * y[muY] * z[muZ - 1] * radial
                                           * normFactorOfMu[iMuShell];
              }

              if (_highestDerivative >= 2) {
                // XX
                hessianStore.xx[iMuP] = (ddradial * x[muX + 2] + dradial * x[muX] * (2 * muX + 1))
                                        * y[muY] * z[muZ] * normFactorOfMu[iMuShell];
                if (muX > 1) {
                  hessianStore.xx[iMuP] += muX * (muX - 1) * x[muX - 2] * y[muY] * z[muZ]
                                           * radial * normFactorOfMu[iMuShell];
                }
                // XY
                hessianStore.xy[iMuP] = ddradial * x[muX + 1]
                                        * y[muY + 1] * z[muZ] * normFactorOfMu[iMuShell];
                if (muX > 0) {
                  hessianStore.xy[iMuP] += dradial * muX * x[muX - 1]
                                           * y[muY + 1] * z[muZ] * normFactorOfMu[iMuShell];
                }
                if (muY > 0) {
                  hessianStore.xy[iMuP] += dradial * muY * x[muX + 1]
                                           * y[muY - 1] * z[muZ] * normFactorOfMu[iMuShell];
                  if (muX > 0) {
                    hessianStore.xy[iMuP] += muX * muY * x[muX - 1] * y[muY - 1] * z[muZ]
                                             * normFactorOfMu[iMuShell] * radial;
                  }
                }
                // XZ
                hessianStore.xz[iMuP] = x[muX+1]*y[muY]*z[muZ+1] * ddradial * normFactorOfMu[iMuShell];
                if (muX > 0) {
                  hessianStore.xz[iMuP] += dradial * muX * x[muX - 1] * y[muY]
                                           * z[muZ + 1] * normFactorOfMu[iMuShell];
                }
                if (muZ > 0) {
                  hessianStore.xz[iMuP] += dradial * muZ * x[muX + 1] * y[muY]
                                           * z[muZ - 1] * normFactorOfMu[iMuShell];
                  if (muX > 0) {
                    hessianStore.xz[iMuP] += muX * muZ * x[muX - 1] * y[muY] * z[muZ - 1]
                                             * normFactorOfMu[iMuShell] * radial;
                  }
                }
                // YY
                hessianStore.yy[iMuP] = (ddradial * y[muY + 2]
                    + dradial * y[muY] * (2 * muY + 1))
                                        * x[muX] * z[muZ] * normFactorOfMu[iMuShell];
                if (muY > 1) {
                  hessianStore.yy[iMuP] +=

                  muY * (muY - 1) * y[muY - 2] * x[muX] * z[muZ] * radial
                  * normFactorOfMu[iMuShell];
                }
                // YZ
                hessianStore.yz[iMuP] = ddradial * x[muX] * y[muY + 1]
                                        * z[muZ + 1] * normFactorOfMu[iMuShell];
                if (muY > 0) {
                  hessianStore.yz[iMuP] += dradial * muY * x[muX] * y[muY - 1]
                                           * z[muZ + 1] * normFactorOfMu[iMuShell];
                }
                if (muZ > 0) {
                  hessianStore.yz[iMuP] += dradial * muZ * x[muX] * y[muY + 1]
                                           * z[muZ - 1] * normFactorOfMu[iMuShell];
                  if (muY > 0) {
                    hessianStore.yz[iMuP] += muY * muZ * x[muX] * y[muY - 1] * z[muZ - 1]
                                             * normFactorOfMu[iMuShell] * radial;
                  }
                }
                // ZZ
                hessianStore.zz[iMuP] = (ddradial * z[muZ + 2]
                    + dradial * z[muZ] * (2 * muZ + 1))
                                        * y[muY] * x[muX] * normFactorOfMu[iMuShell];
                if (muZ > 1) {
                  hessianStore.zz[iMuP] += muZ * (muZ - 1) * z[muZ - 2] * y[muY] * x[muX]
                                           * radial * normFactorOfMu[iMuShell];
                }
              }
        ...
        switch (angularMomentumOfMu) {
          ...
          case 4:
            ...
            Y[ 4] = sqrt(1.0/64.0) * (8.0*z[4]-24.0*x[2]*z[2]-24.0*y[2]*z[2]+3.0*x[4]+3.0*y[4]+6.0*x[2]*y[2]);
          ...
          case 5:
            ...
            Y[ 2] = sqrt(35.0/128.0) * (24.0*x[2]*y[1]*z[2]-3.0*x[4]*y[1]-2.0*x[2]*y[3]-8.0*y[3]*z[2]+1.0*y[5]);
            Y[ 3] = sqrt(105.0/16.0) * (4.0*x[1]*y[1]*z[3]-2.0*x[3]*y[1]*z[1]-2.0*x[1]*y[3]*z[1]);
            Y[ 4] = sqrt(15.0/64.0) * (8.0*y[1]*z[4]-12.0*x[2]*y[1]*z[2]-12.0*y[3]*z[2]+1.0*x[4]*y[1]+1.0*y[5]+2.0*x[2]*y[3]);
            Y[ 5] = sqrt(1.0/64.0) * (8.0*z[5]-40.0*x[2]*z[3]-40.0*y[2]*z[3]+15.0*x[4]*z[1]+15.0*y[4]*z[1]+30.0*x[2]*y[2]*z[1]);
            Y[ 6] = sqrt(15.0/64.0) * (8.0*x[1]*z[4]-12.0*x[3]*z[2]-12.0*x[1]*y[2]*z[2]+1.0*x[5]+1.0*x[1]*y[4]+2.0*x[3]*y[2]);
            Y[ 7] = sqrt(105.0/16.0) * (2.0*x[2]*z[3]-1.0*x[4]*z[1]-2.0*y[2]*z[3]+1.0*y[4]*z[1]);
            Y[ 8] = sqrt(35.0/128.0) * (8.0*x[3]*z[2]-1.0*x[5]+2.0*x[3]*y[2]-24.0*x[1]*y[2]*z[2]+3.0*x[1]*y[4]);
            ...
            if (_highestDerivative >= 1) {
              ...
              dYdy[ 2] = sqrt(35.0/128.0) * (24.0*x[2]*z[2]-3.0*x[4]-6.0*x[2]*y[2]-24.0*y[2]*z[2]+5.0*y[4]);
              ...
              dYdy[ 4] = sqrt(15.0/64.0) * (8.0*z[4]-12.0*x[2]*z[2]-36.0*y[2]*z[2]+1.0*x[4]+5.0*y[4]+6.0*x[2]*y[2]);
              ...
              dYdz[ 5] = sqrt(1.0/64.0) * (40.0*z[4]-120.0*x[2]*z[2]-120.0*y[2]*z[2]+15.0*x[4]+15.0*y[4]+30.0*x[2]*y[2]);
              dYdx[ 6] = sqrt(15.0/64.0) * (8.0*z[4]-36.0*x[2]*z[2]-12.0*y[2]*z[2]+5.0*x[4]+1.0*y[4]+6.0*x[2]*y[2]);
              ...
              dYdx[ 8] = sqrt(35.0/128.0) * (24.0*x[2]*z[2]-5.0*x[4]+6.0*x[2]*y[2]-24.0*y[2]*z[2]+3.0*y[4]);
          ...
          case 6:
            ...
            Y[ 2] = sqrt(63.0/256.0) * (40.0*x[3]*y[1]*z[2]-4.0*x[5]*y[1]-40.0*x[1]*y[3]*z[2]+4.0*x[1]*y[5]);
            Y[ 3] = sqrt(105.0/128.0) * (24.0*x[2]*y[1]*z[3]-9.0*x[4]*y[1]*z[1]-6.0*x[2]*y[3]*z[1]-8.0*y[3]*z[3]+3.0*y[5]*z[1]);
            Y[ 4] = sqrt(105.0/512.0) * (32.0*x[1]*y[1]*z[4]-32.0*x[3]*y[1]*z[2]-32.0*x[1]*y[3]*z[2]+2.0*x[5]*y[1]+2.0*x[1]*y[5]+4.0*x[3]*y[3]);
            Y[ 5] = sqrt(21.0/64.0) * (8.0*y[1]*z[5]-20.0*x[2]*y[1]*z[3]-20.0*y[3]*z[3]+5.0*x[4]*y[1]*z[1]+5.0*y[5]*z[1]+10.0*x[2]*y[3]*z[1]);
            Y[ 6] = sqrt(1.0/256.0) * (331.0*z[6]-120.0*x[2]*z[4]-120.0*y[2]*z[4]-315.0*z[6]+90.0*x[4]*z[2]+90.0*y[4]*z[2]+180.0*x[2]*y[2]*z[2]-5.0*x[6]-5.0*y[6]-15.0*x[4]*y[2]-15.0*x[2]*y[4]);
            Y[ 7] = sqrt(21.0/64.0) * (8.0*x[1]*z[5]-20.0*x[3]*z[3]-20.0*x[1]*y[2]*z[3]+5.0*x[5]*z[1]+5.0*x[1]*y[4]*z[1]+10.0*x[3]*y[2]*z[1]);
            Y[ 8] = sqrt(105.0/512.0) * (16.0*x[2]*z[4]-16.0*x[4]*z[2]+1.0*x[6]-1.0*x[2]*y[4]+1.0*x[4]*y[2]-34.0*y[2]*z[4]+16.0*y[4]*z[2]+18.0*y[2]*z[4]-1.0*y[6]);
            Y[ 9] = sqrt(105.0/128.0) * (8.0*x[3]*z[3]-3.0*x[5]*z[1]+6.0*x[3]*y[2]*z[1]-24.0*x[1]*y[2]*z[3]+9.0*x[1]*y[4]*z[1]);
            Y[10] = sqrt(63.0/256.0) * (10.0*x[4]*z[2]-1.0*x[6]+5.0*x[4]*y[2]-60.0*x[2]*y[2]*z[2]+5.0*x[2]*y[4]+10.0*y[4]*z[2]-1.0*y[6]);
            ...
            if (_highestDerivative >= 1) {
              ...
              dYdx[ 2] = sqrt(63.0/256.0) * (120.0*x[2]*y[1]*z[2]-20.0*x[4]*y[1]-40.0*y[3]*z[2]+4.0*y[5]);
              dYdy[ 2] = sqrt(63.0/256.0) * (40.0*x[3]*z[2]-4.0*x[5]-120.0*x[1]*y[2]*z[2]+20.0*x[1]*y[4]);
              dYdz[ 2] = sqrt(63.0/256.0) * (80.0*x[3]*y[1]*z[1]-80.0*x[1]*y[3]*z[1]);
              dYdx[ 3] = sqrt(105.0/128.0) * (48.0*x[1]*y[1]*z[3]-36.0*x[3]*y[1]*z[1]-12.0*x[1]*y[3]*z[1]);
              dYdy[ 3] = sqrt(105.0/128.0) * (24.0*x[2]*z[3]-9.0*x[4]*z[1]-18.0*x[2]*y[2]*z[1]-24.0*y[2]*z[3]+15.0*y[4]*z[1]);
              dYdz[ 3] = sqrt(105.0/128.0) * (72.0*x[2]*y[1]*z[2]-9.0*x[4]*y[1]-6.0*x[2]*y[3]-24.0*y[3]*z[2]+3.0*y[5]);
              dYdx[ 4] = sqrt(105.0/512.0) * (32.0*y[1]*z[4]-96.0*x[2]*y[1]*z[2]-32.0*y[3]*z[2]+10.0*x[4]*y[1]+2.0*y[5]+12.0*x[2]*y[3]);
              dYdy[ 4] = sqrt(105.0/512.0) * (32.0*x[1]*z[4]-32.0*x[3]*z[2]-96.0*x[1]*y[2]*z[2]+2.0*x[5]+10.0*x[1]*y[4]+12.0*x[3]*y[2]);
              dYdz[ 4] = sqrt(105.0/512.0) * (128.0*x[1]*y[1]*z[3]-64.0*x[3]*y[1]*z[1]-64.0*x[1]*y[3]*z[1]);
              dYdx[ 5] = sqrt(21.0/64.0) * (-40.0*x[1]*y[1]*z[3]+20.0*x[3]*y[1]*z[1]+20.0*x[1]*y[3]*z[1]);
              dYdy[ 5] = sqrt(21.0/64.0) * (8.0*z[5]-20.0*x[2]*z[3]-60.0*y[2]*z[3]+5.0*x[4]*z[1]+25.0*y[4]*z[1]+30.0*x[2]*y[2]*z[1]);
              dYdz[ 5] = sqrt(21.0/64.0) * (40.0*y[1]*z[4]-60.0*x[2]*y[1]*z[2]-60.0*y[3]*z[2]+5.0*x[4]*y[1]+5.0*y[5]+10.0*x[2]*y[3]);
              dYdx[ 6] = sqrt(1.0/256.0) * (-240.0*x[1]*z[4]+360.0*x[3]*z[2]+360.0*x[1]*y[2]*z[2]-30.0*x[5]-60.0*x[3]*y[2]-30.0*x[1]*y[4]);
              dYdy[ 6] = sqrt(1.0/256.0) * (-240.0*y[1]*z[4]+360.0*y[3]*z[2]+360.0*x[2]*y[1]*z[2]-30.0*y[5]-30.0*x[4]*y[1]-60.0*x[2]*y[3]);
              dYdz[ 6] = sqrt(1.0/256.0) * (1986.0*z[5]-480.0*x[2]*z[3]-480.0*y[2]*z[3]-1890.0*z[5]+180.0*x[4]*z[1]+180.0*y[4]*z[1]+360.0*x[2]*y[2]*z[1]);
              dYdx[ 7] = sqrt(21.0/64.0) * (8.0*z[5]-60.0*x[2]*z[3]-20.0*y[2]*z[3]+25.0*x[4]*z[1]+5.0*y[4]*z[1]+30.0*x[2]*y[2]*z[1]);
              dYdy[ 7] = sqrt(21.0/64.0) * (-40.0*x[1]*y[1]*z[3]+20.0*x[1]*y[3]*z[1]+20.0*x[3]*y[1]*z[1]);
              dYdz[ 7] = sqrt(21.0/64.0) * (40.0*x[1]*z[4]-60.0*x[3]*z[2]-60.0*x[1]*y[2]*z[2]+5.0*x[5]+5.0*x[1]*y[4]+10.0*x[3]*y[2]);
              dYdx[ 8] = sqrt(105.0/512.0) * (32.0*x[1]*z[4]-64.0*x[3]*z[2]+6.0*x[5]-2.0*x[1]*y[4]+4.0*x[3]*y[2]);
              dYdy[ 8] = sqrt(105.0/512.0) * (-4.0*x[2]*y[3]+2.0*x[4]*y[1]-68.0*y[1]*z[4]+64.0*y[3]*z[2]+36.0*y[1]*z[4]-6.0*y[5]);
              dYdz[ 8] = sqrt(105.0/512.0) * (64.0*x[2]*z[3]-32.0*x[4]*z[1]-136.0*y[2]*z[3]+32.0*y[4]*z[1]+72.0*y[2]*z[3]);
              dYdx[ 9] = sqrt(105.0/128.0) * (24.0*x[2]*z[3]-15.0*x[4]*z[1]+18.0*x[2]*y[2]*z[1]-24.0*y[2]*z[3]+9.0*y[4]*z[1]);
              dYdy[ 9] = sqrt(105.0/128.0) * (12.0*x[3]*y[1]*z[1]-48.0*x[1]*y[1]*z[3]+36.0*x[1]*y[3]*z[1]);
              dYdz[ 9] = sqrt(105.0/128.0) * (24.0*x[3]*z[2]-3.0*x[5]+6.0*x[3]*y[2]-72.0*x[1]*y[2]*z[2]+9.0*x[1]*y[4]);
              dYdx[10] = sqrt(63.0/256.0) * (40.0*x[3]*z[2]-6.0*x[5]+20.0*x[3]*y[2]-120.0*x[1]*y[2]*z[2]+10.0*x[1]*y[4]);
              dYdy[10] = sqrt(63.0/256.0) * (10.0*x[4]*y[1]-120.0*x[2]*y[1]*z[2]+20.0*x[2]*y[3]+40.0*y[3]*z[2]-6.0*y[5]);
              ...
              if (_highestDerivative >= 2) {
                ...
                d2Ydydz[ 3] = sqrt(105.0/128.0) * (72.0*x[2]*z[2]-9.0*x[4]-18.0*x[2]*y[2]-72.0*y[2]*z[2]+15.0*y[4]);
                ...
                d2Ydxdy[ 4] = sqrt(105.0/512.0) * (32.0*z[4]-96.0*x[2]*z[2]-96.0*y[2]*z[2]+10.0*x[4]+10.0*y[4]+36.0*x[2]*y[2]);
                ...
                d2Ydydz[ 5] = sqrt(21.0/64.0) * (40.0*z[4]-60.0*x[2]*z[2]-180.0*y[2]*z[2]+5.0*x[4]+25.0*y[4]+30.0*x[2]*y[2]);
                d2Ydzdz[ 5] = sqrt(21.0/64.0) * (160.0*y[1]*z[3]-120.0*x[2]*y[1]*z[1]-120.0*y[3]*z[1]);
                d2Ydxdx[ 6] = sqrt(1.0/256.0) * (-240.0*z[4]+1080.0*x[2]*z[2]+360.0*y[2]*z[2]-150.0*x[4]-180.0*x[2]*y[2]-30.0*y[4]);
                ...
                d2Ydydy[ 6] = sqrt(1.0/256.0) * (-240.0*z[4]+1080.0*y[2]*z[2]+360.0*x[2]*z[2]-150.0*y[4]-30.0*x[4]-180.0*x[2]*y[2]);
                d2Ydydz[ 6] = sqrt(1.0/256.0) * (-960.0*y[1]*z[3]+720.0*y[3]*z[1]+720.0*x[2]*y[1]*z[1]);
                d2Ydzdz[ 6] = sqrt(1.0/256.0) * (9930.0*z[4]-1440.0*x[2]*z[2]-1440.0*y[2]*z[2]-9450.0*z[4]+180.0*x[4]+180.0*y[4]+360.0*x[2]*y[2]);
                ...
                d2Ydxdz[ 7] = sqrt(21.0/64.0) * (40.0*z[4]-180.0*x[2]*z[2]-60.0*y[2]*z[2]+25.0*x[4]+5.0*y[4]+30.0*x[2]*y[2]);
                ...
                d2Ydxdx[ 8] = sqrt(105.0/512.0) * (32.0*z[4]-192.0*x[2]*z[2]+30.0*x[4]-2.0*y[4]+12.0*x[2]*y[2]);
                d2Ydxdy[ 8] = sqrt(105.0/512.0) * (-8.0*x[1]*y[3]+8.0*x[3]*y[1]);
                d2Ydxdz[ 8] = sqrt(105.0/512.0) * (128.0*x[1]*z[3]-128.0*x[3]*z[1]);
                d2Ydydy[ 8] = sqrt(105.0/512.0) * (-12.0*x[2]*y[2]+2.0*x[4]-68.0*z[4]+192.0*y[2]*z[2]+36.0*z[4]-30.0*y[4]);
                d2Ydydz[ 8] = sqrt(105.0/512.0) * (-272.0*y[1]*z[3]+128.0*y[3]*z[1]+144.0*y[1]*z[3]);
                d2Ydzdz[ 8] = sqrt(105.0/512.0) * (192.0*x[2]*z[2]-32.0*x[4]-408.0*y[2]*z[2]+32.0*y[4]+216.0*y[2]*z[2]);
                ...
                d2Ydxdz[ 9] = sqrt(105.0/128.0) * (72.0*x[2]*z[2]-15.0*x[4]+18.0*x[2]*y[2]-72.0*y[2]*z[2]+9.0*y[4]);
                ...
                d2Ydxdx[10] = sqrt(63.0/256.0) * (120.0*x[2]*z[2]-30.0*x[4]+60.0*x[2]*y[2]-120.0*y[2]*z[2]+10.0*y[4]);
                ...
                d2Ydydy[10] = sqrt(63.0/256.0) * (10.0*x[4]-120.0*x[2]*z[2]+60.0*x[2]*y[2]+120.0*y[2]*z[2]-30.0*y[4]);
        ...
        if (_highestDerivative >= 2) {
          for (unsigned int m=0;m<2*angularMomentumOfMu+1;++m){
            hessianStore.xx[m*offset] = radial*d2Ydxdx[m] + 2.0*dradial*x[1]*dYdx[m] + ddradial*x[2]*Y[m] + dradial*Y[m];
            hessianStore.xy[m*offset] = radial*d2Ydxdy[m] + dradial*x[1]*dYdy[m] + dradial*y[1]*dYdx[m] + ddradial*x[1]*y[1]*Y[m];
            hessianStore.xz[m*offset] = radial*d2Ydxdz[m] + dradial*x[1]*dYdz[m] + dradial*z[1]*dYdx[m] + ddradial*x[1]*z[1]*Y[m];
            hessianStore.yy[m*offset] = radial*d2Ydydy[m] + 2.0*dradial*y[1]*dYdy[m] + ddradial*y[2]*Y[m] + dradial*Y[m];
            hessianStore.yz[m*offset] = radial*d2Ydydz[m] + dradial*y[1]*dYdz[m] + dradial*z[1]*dYdy[m] + ddradial*y[1]*z[1]*Y[m];
            hessianStore.zz[m*offset] = radial*d2Ydzdz[m] + 2.0*dradial*z[1]*dYdz[m] + ddradial*z[2]*Y[m] + dradial*Y[m];
          }
        }
      } /* if spherical*/
    }
  }
  if (omp_get_thread_num()==0) Timings::timeTaken("Tech. -    Basis On Grid Eval.");
