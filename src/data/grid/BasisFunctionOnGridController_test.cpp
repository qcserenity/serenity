...
TEST_F(BasisFunctionOnGridControllerTest, TestBasisFunctionValuesAndDerivatives) {
   ...
   */
  BasisFunctionOnGridController testObject(
    basisController,
    gridController,
    10,
    expectedPrecision*0.001, 2);
...
protected:
  static void SetUpTestCase(){
    ...
    _geom = std::make_shared<Geometry>(std::vector<std::shared_ptr<Atom> >(
                     1,std::make_shared<Atom>("SC",0.0,0.0,0.0)));
    auto basis = AtomCenteredBasisControllerFactory::produce(_geom,baslibpath,true,true,37,"CC-PV5Z");

    auto grid = GridControllerFactory::produce(_geom, Options::GRID_TYPES::BECKE,3,
...
TEST_F(SphBFOnGridTest, S_Functions) {
  for (unsigned int n=0; n<9;n++){ // for n>7 the grid is not large enough
    double integral_f_r = (_data->functionValues.col(n).array()*_data->functionValues.col(n).array()*_weights.array()).sum();
