...
template<Options::SCF_MODES T> void DensityAdder<T>::add(
    DensityOnGrid<T>& densityToBeAddedTo,
  ...
  auto basisFunctionOnGridController = std::make_shared<BasisFunctionOnGridController>(
      basisController,
      gridController,
      128,
      1e-9,
