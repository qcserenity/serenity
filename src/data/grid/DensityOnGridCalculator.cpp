...
template<Options::SCF_MODES T>DensityOnGridCalculator<T>::DensityOnGridCalculator(
    shared_ptr<BasisFunctionOnGridController> basisFunctionOnGridController,
    const double blockAverageThreshold) :
        _basisFunctionOnGridController(basisFunctionOnGridController),
        _blockAverageThreshold(blockAverageThreshold){
  assert(_basisFunctionOnGridController);
}

template<Options::SCF_MODES T>void DensityOnGridCalculator<T>::calcDensityOnGrid(
    const DensityMatrix<T>& densityMatrix, DensityOnGrid<T>& densityOnGrid) {
  ...
  MatrixOperatorToGridTransformer::transform(
    densityMatrix, densityOnGrid, *_basisFunctionOnGridController);
  Timings::timeTaken("Tech. -  Density On Grid Eval.");
}

template<Options::SCF_MODES T>
DensityOnGrid<T> DensityOnGridCalculator<T>::calcDensityOnGrid(
    const DensityMatrix<T>& densityMatrix) {
...
template<Options::SCF_MODES T>
void DensityOnGridCalculator<T>::calcDensityAndGradientOnGrid(
    const DensityMatrix<T>& densityMatrix,
...
template<Options::SCF_MODES T>
DensityOnGrid<T> DensityOnGridCalculator<T>::calcDensityAndGradientOnGrid(
    const DensityMatrix<T>& densityMatrix,
    Gradient<DensityOnGrid<T> >& densityGradientOnGrid) {
  DensityOnGrid<T> densOnGrid(_basisFunctionOnGridController->getGridController());
...
template<Options::SCF_MODES T>
void DensityOnGridCalculator<T>::calcDensityAndDerivativesOnGrid(
    const DensityMatrix<T>& densityMatrix,
    DensityOnGrid<T>& densityOnGrid,
    Gradient<DensityOnGrid<T> >& densityGradientOnGrid,
