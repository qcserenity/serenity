...
template<Options::SCF_MODES SCFMode>
std::shared_ptr<DensityOnGridController<SCFMode> > DensityOnGridFactory<SCFMode>::produce(
  ...
  if (!_instance) _instance.reset(new DensityOnGridFactory<SCFMode>());
  return _instance->getOrProduce(density, gridController, highestDerivative,\
                                 settings.grid.blocksize,settings.grid.basFuncRadialThreshold,
                                 settings.grid.blockAveThreshold);
}

template<Options::SCF_MODES SCFMode>
std::unique_ptr<DensityOnGridController<SCFMode> > DensityOnGridFactory<SCFMode>::produceNew(
    const std::shared_ptr<DensityMatrixController<SCFMode> > density,
    const std::shared_ptr<GridController> gridController,
    const unsigned int highestDerivative,
  ...
  auto basisFunctionOnGridController( BasisFunctionOnGridControllerFactory::produce(
      ...
      highestDerivative,
      density->getDensityMatrix().getBasisController(),
      gridController) );
  auto densityOnGridCalculator = std::make_shared<DensityOnGridCalculator<SCFMode> >(
      basisFunctionOnGridController, blockAveThreshold);
  auto densOnGrid = std::make_shared<DensityMatrixDensityOnGridController<SCFMode> >(
      densityOnGridCalculator,density);

  return std::move(std::unique_ptr<DensityOnGridController<SCFMode> >(
          new DensityMatrixDensityOnGridController<SCFMode> (
