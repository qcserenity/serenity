...
Eigen::MatrixXd MOCalculator::calcMOValuesOnGrid(
 ...
 #pragma omp parallel for schedule(dynamic)
    for (unsigned int block=0; block<nBlocks; block++){
      ...
      for(unsigned int iMO=0; iMO < nMOs; ++iMO) {
        ...
        const Eigen::SparseMatrix<double> cProjection = constructSignificantMnPProjectionMatrix(
            coefficients_i,mnpTruncationThreshold);
...
SpinPolarizedData<SCFMode, Matrix<double>>MOCalculator::calcAllMOValuesOnGrid(
  ...
  SpinPolarizedData<SCFMode, Matrix<double> > mosOnGrid=calcOccMOValuesOnGrid<SCFMode>(coefficientMatrix,orbs,mnpTruncationThreshold);
