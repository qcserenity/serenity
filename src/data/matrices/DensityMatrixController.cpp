...
template<>
DensityMatrixController<Options::SCF_MODES::UNRESTRICTED>::
DensityMatrixController(const std::shared_ptr<OrbitalController<Options::SCF_MODES::UNRESTRICTED> > molecularOrbitals,
...
template<>
DensityMatrixController<RESTRICTED>::
DensityMatrixController(const DensityMatrix<RESTRICTED>& densityMatrix):
...
template<>
DensityMatrixController<UNRESTRICTED>::
DensityMatrixController(const DensityMatrix<UNRESTRICTED>& densityMatrix):
...
template<> void
DensityMatrixController<Options::SCF_MODES::RESTRICTED>::attachOrbitals(
  ...
  if (update) _outOfDate = true;
}

template<> void
DensityMatrixController<Options::SCF_MODES::UNRESTRICTED>::attachOrbitals(
...
template<Options::SCF_MODES SCFMode>
void DensityMatrixController<SCFMode>::attachOrbitals(
...
SpinPolarizedData<SCFMode, Eigen::VectorXd > DensityMatrixController<SCFMode>::getOccupations(bool aufbau){
  if (aufbau) return _aufbauOccupations;
  ...
  if(_outOfDate){
    ...
    if (_occupations==nullptr) return _aufbauOccupations;
    SpinPolarizedData<SCFMode, Eigen::VectorXd > occ(*_occupations);
    return *_occupations;
  }
}
...
DensityMatrixController<SCFMode>::updateDensityMatrix(){
  ...
  if (_fractionalDegeneracy){
    ...
    for(unsigned int k=0; k<occupations_spin.size(); ++k){
      ...
      for(t=k+1; t<occupations_spin.size(); ++t){
        if (fabs(energies_spin[k]-energies_spin[t]) < 1e-1){
      ...
      sum /= (double)(t-k);
      for (;k<t;k++) occupations_spin[k] = sum;
      k-=1;
    }
    for(unsigned int k=0; k<occupations_spin.size(); ++k){
    }
...
template<>
void DensityMatrixController<Options::SCF_MODES::RESTRICTED>::toHDF5(
...
template<>
void DensityMatrixController<Options::SCF_MODES::UNRESTRICTED>::toHDF5(
    std::string fBaseName, std::string id){
  if(!_occupations or !_densityMatrix){
    bool tmp = _diskmode;
    this->setDiskMode(false,_fBaseName,_id);
...
template<>
void DensityMatrixController<Options::SCF_MODES::RESTRICTED>::fromHDF5(
    std::string fBaseName, std::string id){
...
template<>
void DensityMatrixController<Options::SCF_MODES::UNRESTRICTED>::fromHDF5(
    std::string fBaseName, std::string id){
