...
Eigen::VectorXd totalOnGrid(std::shared_ptr<DensityMatrixController<SCFMode> > dmatC,
            ...
            double avgthrhld=1e-11,
            double radialthrhld=1e-9) {

    // generate data on grid
    auto bfOnGrid = std::make_shared<BasisFunctionOnGridController>(
        dmatC->getDensityMatrix().getBasisController(),
        grid,128,radialthrhld,0);
    auto densOnGridCalc = std::make_shared<DensityOnGridCalculator<SCFMode> >(
        bfOnGrid,avgthrhld);
    DensityMatrixDensityOnGridController<SCFMode>dmdogCont(
        densOnGridCalc,
        dmatC,0);
...
void setR (std::shared_ptr<DensityMatrixController<RESTRICTED> > dmatC,
...
void export_DensityMatrixController(py::module &spy) {
    py::class_<DensityMatrixController<Options::SCF_MODES::RESTRICTED>,
               std::shared_ptr<DensityMatrixController<Options::SCF_MODES::RESTRICTED> > >(spy,"DensityMatrix_R",
        "@brief A restricted density matrix.")
        .def("totalOnGrid",totalOnGrid<Options::SCF_MODES::RESTRICTED>,
             py::arg("grid"),
             py::arg("avgthrhld") = 1e-11,
    ...
    py::class_<DensityMatrixController<Options::SCF_MODES::UNRESTRICTED>,
               std::shared_ptr<DensityMatrixController<Options::SCF_MODES::UNRESTRICTED> > >(spy,"DensityMatrix_U",
        "@brief An unrestricted density matrix.")
        .def("totalOnGrid",totalOnGrid<Options::SCF_MODES::UNRESTRICTED>,
             py::arg("grid"),
             py::arg("avgthrhld") = 1e-11,
             py::arg("radialthrhld") = 1e-9)
        .def("total",&totalU)
