...
public:
   ...
   */
  inline SPMatrix(unsigned int nRows,unsigned int nCols) :
      Eigen::MatrixXd(nRows, nCols) {
    this->Base::setZero();
  }
   ...
   */
  inline SPMatrix<RESTRICTED>(const SPMatrix<RESTRICTED>& orig) :
      Eigen::MatrixXd( Eigen::MatrixXd(orig)) {
  }
  /**
   * @brief Constructor for the use in vector initializations etc.
   */
  inline SPMatrix<RESTRICTED>() :
      Eigen::MatrixXd (Eigen::MatrixXd()) {
  }
  /**
   * @brief Move constructor
   */
  inline SPMatrix<RESTRICTED>(SPMatrix<RESTRICTED>&& orig) :
      Eigen::MatrixXd(Eigen::MatrixXd(orig)){
  }
  /**
   ...
   */
  void toHDF5(std::string fBaseName,
...
public:
  ...
  inline SPMatrix<RESTRICTED>& operator=(const Eigen::MatrixBase <OtherDerived>& other){
    this->Base::operator=(other);
    return *this;
  }
  /// @brief Move assignement operator for Eigen3 objects.
  template<typename OtherDerived> __attribute__((always_inline)) 
  inline  SPMatrix<RESTRICTED>& operator=(Eigen::MatrixBase <OtherDerived>&& other){
    this->Base::operator=(other);
    return *this;
  }
  ...
  friend std::ostream& operator<< (std::ostream& stream,
                                   const SPMatrix<RESTRICTED>& matrix){
...
public:
   ...
   * @brief Constructor for the use in vector initializations etc.
   */
  inline SPMatrix<UNRESTRICTED>() :
      alpha(Eigen::MatrixXd()),
      beta(Eigen::MatrixXd()) {
  }
   ...
   */
  inline SPMatrix<UNRESTRICTED>(const SPMatrix<UNRESTRICTED>& orig) :
      alpha( orig.alpha),
      beta( orig.beta) {
  }
  inline SPMatrix<UNRESTRICTED>(const SPMatrix<RESTRICTED>& orig) :
        alpha( orig),
        beta( orig) {
  }
  /**
  ...
  inline SPMatrix<UNRESTRICTED>(SPMatrix<UNRESTRICTED>&& orig) :
      alpha( (Eigen::MatrixXd)orig.alpha),
      beta( (Eigen::MatrixXd)orig.beta){
  }
  /// @brief assignement operator
...
public:
  ...
  friend std::ostream& operator<< (std::ostream& stream,
                                   const SPMatrix<UNRESTRICTED>& matrix){
    stream<<matrix.alpha<<"\n \n"<<matrix.beta;
    return stream;
  }
  /// @brief The SCF_MOODE of this matrix
