...
class Functional {
public:
  Functional(const std::vector<BASIC_FUNCTIONALS>& basicFunctionals);
  /**
   *
   * @param basicFunctionals
   * @param mixingFactors
   * @param hfExchangeRatio  cam_alpha
   * @param lrExchangeRatio  cam_beta
   * @param mu
   */
  Functional(
      const std::vector<BASIC_FUNCTIONALS>& basicFunctionals,
      const std::vector<double>& mixingFactors,
      double hfExchangeRatio = 0.0,
      double hfCorrelRatio = 0.0,
      double lrExchangeRatio = 0.0,
      double mu = 0.0,
      double ssScaling = 1.0,
   ...
   */
  inline const std::vector<BASIC_FUNCTIONALS>& getBasicFunctionals() const {
   ...
   */
  inline FUNCTIONAL_CLASSES getFunctionalClass() const {
  ...
  /**
   * @returns the scaling for same spin contributions of the Hartree--Fock correlation (MP2). Only works for double-hybrid functionals.
   */
  inline double getssScaling() const {
    return _ssScaling;
  }
  /**
   * @returns the scaling for opposite spin contributions of the Hartree--Fock correlation (MP2). Only works for double-hybrid functionals.
   */
...
private:
  std::vector<BASIC_FUNCTIONALS> _basicFunctionals;
  /**
   ...
   */
  FUNCTIONAL_CLASSES _functionalClass;
  /**
...
inline bool operator==(const Functional& lhs, const Functional& rhs){
  bool fclass = (lhs.getFunctionalClass() == rhs.getFunctionalClass());
  bool fmix   = (lhs.getMixingFactors() == rhs.getMixingFactors());
  bool ffunc  = (lhs.getBasicFunctionals() == rhs.getBasicFunctionals());
  return (fclass and fmix and ffunc);
}
