...
template<> double
DispersionCorrectionCalculator::calcDispersionEnergyCorrection<Options::DFT_DISPERSION_CORRECTIONS::NONE>(
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
  return 0.0;
}
#pragma GCC diagnostic pop

template<> double
DispersionCorrectionCalculator::calcDispersionEnergyCorrection<Options::DFT_DISPERSION_CORRECTIONS::D3>(
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
	  ...
	  if (nuclearChargeI != 0) {
		  for (unsigned int j=0;j<i;++j){
			  ...
			  if (nuclearChargeJ != 0) {
				  ...
				  if (r > DispersionData::RANGE_THRESHOLD_N2) continue;
				  double rr=DispersionData::r0ab(nuclearChargeI,
						  nuclearChargeJ)/r;
...
template<> double
DispersionCorrectionCalculator::calcDispersionEnergyCorrection<Options::DFT_DISPERSION_CORRECTIONS::D3ABC>(
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
	  ...
	  if (nuclearChargeI != 0) {
		  for (unsigned int j=0;j<i;++j){
			  ...
			  if (nuclearChargeJ != 0) {
				  double r = distance(*atomI,*atomJ);
				  if (r > DispersionData::RANGE_THRESHOLD_N2) continue;
				  double rr=DispersionData::r0ab(atomI->getAtomType()->getNuclearCharge(),
						  atomJ->getAtomType()->getNuclearCharge())/r;
				  double damp6=1.0/(1.0+6.0*pow(rs6*rr,alp));
				  double c6 = getC6(atomI,atomJ,coordNums[i],coordNums[j]);
				  double r6 = pow(r,6);
...
template<> double
DispersionCorrectionCalculator::calcDispersionEnergyCorrection<Options::DFT_DISPERSION_CORRECTIONS::D3BJ>(
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
	  ...
	  if (nuclearChargeI != 0) {
		  for (unsigned int j=0;j<i;++j){
			  ...
			  if (nuclearChargeJ != 0) {
				  ...
				  // r2r4 stored as sqrt
				  double c8 = 3.0*c6*DispersionData::r2r4(nuclearChargeI)
				  	  	  	  	    *DispersionData::r2r4(nuclearChargeJ);
				  double r8 = r6*r*r;
...
template<> double
DispersionCorrectionCalculator::calcDispersionEnergyCorrection<Options::DFT_DISPERSION_CORRECTIONS::D3BJABC>(
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
...
DispersionCorrectionCalculator::calcDispersionGradientCorrection<Options::DFT_DISPERSION_CORRECTIONS::NONE>(
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
...
template<> Eigen::MatrixXd
DispersionCorrectionCalculator::calcDispersionGradientCorrection<Options::DFT_DISPERSION_CORRECTIONS::D3>(
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
	  ...
	  if (nuclearChargeI != 0) {
		  for (unsigned int j=0;j<i;++j){
			  ...
			  if (nuclearChargeJ != 0) {
				  ...
				  double r0ab=DispersionData::r0ab(nuclearChargeI,
						  nuclearChargeJ);

				  double r2r4Prod = DispersionData::r2r4(nuclearChargeI)
				  *DispersionData::r2r4(nuclearChargeJ);
				  ...
				  // now the last term of the 2-body gradient, i.e., the derivative dEdisp/dCNi = (dEdisp/dC6ij) * (dCij/dCNi) //+ the dCNj term
	  ...
	  if (nuclearChargeI != 0) {
		  for (unsigned int j=0;j<i;++j){
			  ...
			  if (nuclearChargeJ != 0) {
				  ...
				  double deltaCN = 0.0;
				  if (r < DispersionData::RANGE_THRESHOLD_N3){
					  double rcovij =   DispersionData::rcov(nuclearChargeI)
					  + DispersionData::rcov(nuclearChargeJ);
...
template<> Eigen::MatrixXd
DispersionCorrectionCalculator::calcDispersionGradientCorrection<Options::DFT_DISPERSION_CORRECTIONS::D3ABC>(
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
				  ...
				  if(important[ij]){
					  for (unsigned int k=0;k<j;++k){
						  ...
						  if(important[ik] && important[jk]){
							  ...
							  double ang = 1.0 + 0.375*(r2ij + r2jk - r2ik)*(r2ij + r2ik - r2jk)*(r2ik + r2jk - r2ij)/(r2ij*r2jk*r2ik) ;
							  ang /= rijk3Pow ;
...
template<> Eigen::MatrixXd
DispersionCorrectionCalculator::calcDispersionGradientCorrection<Options::DFT_DISPERSION_CORRECTIONS::D3BJ>(
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
...
template<> Eigen::MatrixXd
DispersionCorrectionCalculator::calcDispersionGradientCorrection<Options::DFT_DISPERSION_CORRECTIONS::D3BJABC>(
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
...
Eigen::MatrixXd DispersionCorrectionCalculator::calcDispersionGradientCorrection(Options::DFT_DISPERSION_CORRECTIONS dispType,
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
  Eigen::MatrixXd grad;
  switch (dispType){
...
template<Options::DFT_DISPERSION_CORRECTIONS SCFMode> std::vector<Eigen::MatrixXd>
DispersionCorrectionCalculator::calcDispersionHessianCorrection(
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
  ...
  for (auto atom : geometry->getAtoms()){
    atomCopies.push_back(std::make_shared<Atom>(atom->getAtomType()->getElementSymbol(),
                                                atom->getX(),
                                                atom->getY(),
                                                atom->getZ()));
  }
  ...
  for (unsigned int i=0; i!=nAtoms;++i){
    ...
    auto grad = DispersionCorrectionCalculator::calcDispersionGradientCorrection<SCFMode>(
                      geometryCopy,
                      functional);
    ...
    grad = DispersionCorrectionCalculator::calcDispersionGradientCorrection<SCFMode>(
                      geometryCopy,
                      functional);
    for (unsigned int j=0; j!=nAtoms;++j){
      xx(i,j) -= grad(j,0);
...
std::vector<Eigen::MatrixXd> DispersionCorrectionCalculator::calcDispersionHessianCorrection(
    Options::DFT_DISPERSION_CORRECTIONS dispType,
    std::shared_ptr<const Geometry> geometry,
    const Options::XCFUNCTIONALS functional){
  std::vector<Eigen::MatrixXd> hessian;
  switch (dispType){
...
double DispersionCorrectionCalculator::getC6(std::shared_ptr<Atom> atomI,
    std::shared_ptr<Atom> atomJ,
    double& nCoordI,
    double& nCoordJ){
  double c6abMem=0.0;
  double rsum=0.0;
...
std::pair<double,double> DispersionCorrectionCalculator::getDeltaC6(std::shared_ptr<Atom> atomI,
    std::shared_ptr<Atom> atomJ,
    double& nCoordI,
    double& nCoordJ){
...
std::vector<double> DispersionCorrectionCalculator::calcCoordNumbers(
    std::shared_ptr<const Geometry> geometry){
