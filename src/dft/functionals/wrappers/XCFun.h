 ...
 */
template<Options::SCF_MODES T> class XCFun {
...
public:
 ...
 /**
  * @brief Evaluates functional energy expression and derivatives according to specified type up to chosen order and stores data
  *        in FunctionalData object.
  * @param type XCFun can either evaluate derivatives w.r.t. gradients,  gradient invariants or directly evaluate the LDA/GGA potential.
...
private:
  ...
  void prepareInput(
      ...
      const DensityOnGrid<T>& density,
      const std::shared_ptr<Gradient<DensityOnGrid<T> > > gradient,
      const std::shared_ptr<Hessian<DensityOnGrid<T> > > hessian,
