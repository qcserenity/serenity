...
public:
   ...
   */
 static const std::map<ENERGY_CONTRIBUTIONS, EnergyContributionParents>
  ...
  static bool checkEnergyContributionMap() {
    for (const auto& energyType : ENERGY_CONTRIBUTIONS_VECTOR) {
      if (_energyContributionMap.find(energyType) == _energyContributionMap.end()) return false;
    }
    return true;
  };
  /**
  ...
  static bool checkEnergyContributionChildrenMap() {
    for (const auto& energyType : ENERGY_CONTRIBUTIONS_VECTOR) {
      if (ENERGY_CONTRIBUTIONS_CHILDREN_MAP.find(energyType) ==
              ENERGY_CONTRIBUTIONS_CHILDREN_MAP.end()) return false;
    }
    return true;
  };
#endif
  ...
  static bool isAncestorOf(
      const ENERGY_CONTRIBUTIONS& descendant, const ENERGY_CONTRIBUTIONS& ancestor);
private:
