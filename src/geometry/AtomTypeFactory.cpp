...
void AtomTypeFactory::generateAtomType(string name) {
  ...
  if(name.substr(name.length()-1)==":"){
    ...
    _atomTypes.insert(pair<string, shared_ptr<const AtomType>>(name, std::make_shared<AtomType>(name,
            atom->getNuclearCharge(),
            0.0,
            atom->getBraggSlaterRadius(),
    ...
    _atomTypes.insert(pair<string, shared_ptr<const AtomType> >(
      "H", shared_ptr<const AtomType>(new AtomType(
        "H", 1, 1.00782503223, 0.25*ANGSTROM_TO_BOHR, 1.20*ANGSTROM_TO_BOHR, {{{ANGULAR_QUANTUM_NUMBER::s, 1}}},false)))); // 0.999 885
