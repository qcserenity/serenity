...
Geometry::Geometry(vector<shared_ptr<Atom> > atoms) :
  ...
  for (unsigned int i = 0; i < _atoms.size(); ++i) {
    ...
    if (_atoms[i]->getX() < _minX || i==0) _minX = _atoms[i]->getX();
    if (_atoms[i]->getY() < _minY || i==0) _minY = _atoms[i]->getY();
    if (_atoms[i]->getZ() < _minZ || i==0) _minZ = _atoms[i]->getZ();
    if (_atoms[i]->getX() > _maxX || i==0) _maxX = _atoms[i]->getX();
    if (_atoms[i]->getY() > _maxY || i==0) _maxY = _atoms[i]->getY();
    if (_atoms[i]->getZ() > _maxZ || i==0) _maxZ = _atoms[i]->getZ();
  }
}
...
Geometry::Geometry(std::vector<std::string> atomSymbols,
  ...
  for (unsigned int i=0; i<atomSymbols.size();i++){
    _atoms.push_back(
        std::make_shared<Atom>(atomSymbols[i],
            atomPositions(i,0),
            atomPositions(i,1),
            atomPositions(i,2)));
  }
...
void Geometry::calcCoreCoreRepulsion() const {
  _coreCoreRepulsion = 0.0;
  for (unsigned int i = 0; i < _atoms.size(); ++i) {
    if (_atoms[i]->isDummy()) continue;
    for (unsigned int j = 0; j < i; ++j){
      if (!_atoms[j]->isDummy())_coreCoreRepulsion +=
          _atoms[i]->getEffectiveCharge()*_atoms[j]->getEffectiveCharge()/
...
void Geometry::print() const{
  ...
  for (auto atom : _atoms){
    ...
    printf("%4s %4d %2s %+15.10f %+15.10f %+15.10f\n","",counter,
        atom->getAtomType()->getName().c_str(),
...
Matrix<double> Geometry::getAlignedCoordinates() const {
  ...
  Eigen::Vector3d at1;Eigen::Vector3d at2;Eigen::Vector3d at3;
...
void  Geometry::setCoordinates(Eigen::MatrixXd& newCoords) const {
...
//void  Geometry::setCoordinates(const Eigen::VectorXd& newCoords) const {
//  assert(newCoords.size()==this->getNAtoms()*3);
//  for (unsigned int i=0;i<this->getNAtoms();++i){
//    this->_atoms[i]->setX(newCoords[i]);
//    this->_atoms[i]->setY(newCoords[i+this->getNAtoms()]);
//    this->_atoms[i]->setZ(newCoords[i+2*this->getNAtoms()]);
//  }
//}
...
void  Geometry::setGradients(Matrix<double>& newGradients) const {
...
//void  Geometry::setGradients(Eigen::VectorXd& newGradients) const {
//  assert(newGradients.size()==this->getNAtoms()*3);
//  for (unsigned int i=0;i<this->getNAtoms();++i){
//    this->_atoms[i]->_gradient[0] = newGradients[3*i];
//    this->_atoms[i]->_gradient[1] = newGradients[3*i+1];
//    this->_atoms[i]->_gradient[2] = newGradients[3*i+2];
//    _atoms[i]->_gradientsUpToDate = true;
//  }
//}
...
void Geometry::printGradients() const{
  ...
  for (auto atom : _atoms){
    ++counter;
    printf("%4s %4d %2s %+15.10f %+15.10f %+15.10f\n","",counter,
        atom->getAtomType()->getElementSymbol().c_str(),
...
void Geometry::calcMomentOfInertiaX(){
   for(unsigned int i = 0; i < _atoms.size(); i++){

     _momentOfInertiaX += _atoms[i]->getAtomType()->getMass() * (

             (getCenterOfMass().getY() - _atoms[i]->getY()) *
             (getCenterOfMass().getY() - _atoms[i]->getY()) +
             (getCenterOfMass().getZ() - _atoms[i]->getZ()) *
             (getCenterOfMass().getZ() - _atoms[i]->getZ())
...
void Geometry::calcMomentOfInertiaY(){
   for(unsigned int i = 0; i < _atoms.size(); i++){
     _momentOfInertiaY += _atoms[i]->getAtomType()->getMass() * (

             (getCenterOfMass().getX() - _atoms[i]->getX()) *
             (getCenterOfMass().getX() - _atoms[i]->getX()) +
             (getCenterOfMass().getZ() - _atoms[i]->getZ()) *
             (getCenterOfMass().getZ() - _atoms[i]->getZ())
...
void Geometry::calcMomentOfInertiaZ(){
   for(unsigned int i = 0; i < _atoms.size(); i++){
     _momentOfInertiaZ += _atoms[i]->getAtomType()->getMass() * (
...
Eigen::MatrixXd Geometry::getRotModes(){
  ...
  for (unsigned int i=0; i!=3;++i){
    for (unsigned int j=0; j!=3;++j){
      for (unsigned int k=0; k!=nAtoms;++k){
        inertiaMat(i,j) += atoms[k]->getAtomType()->getMass()*(IDMat(i,j)
                          * (atoms[k]->getX()*atoms[k]->getX()
                            +atoms[k]->getY()*atoms[k]->getY()
  ...
  for (unsigned int i=0; i!=3;++i){
    if (inertiaEigenvalues[i]>0.75) {
      for (unsigned int k=0; k!=nAtoms;++k){
        x_a[0] = atoms[k]->getX() - ((atoms[k]->getX()*eVecs(0,i)+atoms[k]->getY()*eVecs(1,i)+atoms[k]->getZ()*eVecs(2,i))
            /(eVecs(0,i)*eVecs(0,i)
                +eVecs(1,i)*eVecs(1,i)
                +eVecs(2,i)*eVecs(2,i))*eVecs(0,i));

        x_a[1] = atoms[k]->getY() - ((atoms[k]->getX()*eVecs(0,i)+atoms[k]->getY()*eVecs(1,i)+atoms[k]->getZ()*eVecs(2,i))
            /(eVecs(0,i)*eVecs(0,i)
                +eVecs(1,i)*eVecs(1,i)
                +eVecs(2,i)*eVecs(2,i))*eVecs(1,i));

        x_a[2] = atoms[k]->getZ() - ((atoms[k]->getX()*eVecs(0,i)+atoms[k]->getY()*eVecs(1,i)+atoms[k]->getZ()*eVecs(2,i))
            /(eVecs(0,i)*eVecs(0,i)
                +eVecs(1,i)*eVecs(1,i)
                +eVecs(2,i)*eVecs(2,i))*eVecs(2,i));
  ...
  for (unsigned int j=0; j!=3;++j){
    ...
    if (normFactor>0.00001) normFactor = 1.0/normFactor;
...
void Geometry::deleteIdenticalAtoms() {
  ...
  for (unsigned int i = 0; i < this->_atoms.size(); i++) {
	  if (!deleteAtom[i]) newAtomSet.push_back(this->_atoms[i]);
  }
...
void Geometry::addAsDummy(const Geometry& add, bool toFront) {
  ...
  for(auto atom : addAtoms){
    ...
    auto atomType=std::make_shared<AtomType>(oldAtomType->getName()+":",
        oldAtomType->getNuclearCharge(),
        oldAtomType->getMass(),
        oldAtomType->getBraggSlaterRadius(),
        oldAtomType->getVanDerWaalsRadius(),
        oldAtomType->getOccupations(),
        true);
    ...
    auto dummyAtom=std::make_shared<Atom>(atomType,
        atom->getX(),
        atom->getY(),
        atom->getZ(),
        atomBasisFunctions);
