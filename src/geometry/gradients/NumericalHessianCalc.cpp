...
NumericalHessianCalc<T>::NumericalHessianCalc(
    double stepsizeGrad, double stepsizeHess, bool printToFile):
  _deltaGrad(stepsizeGrad),
  _deltaHess(stepsizeHess),
  _printToFile(printToFile){
}

template<Options::SCF_MODES T>
void NumericalHessianCalc<T>::calcHessian(std::shared_ptr<SystemController> systemController) {
...
auto gradtype = Options::GRADIENT_TYPES::ANALYTICAL;
  ...
  for (unsigned int i=0; i!=nAtoms;++i){
    ...
    for (unsigned int hessBlock=0; hessBlock < nAtoms; hessBlock++){
      std::cout << "            " << 3*hessBlock+1 << "                   " << 3*hessBlock+2 << "                   " << 3*hessBlock+3 << std::endl;
    for (unsigned int i=0; i!=3*nAtoms;++i){
        std::cout << i+1 << ": " << hessian(i,3*hessBlock+0) << " "
                         << hessian(i,3*hessBlock+1) << " "
...
template<Options::SCF_MODES T>
void NumericalHessianCalc<T>::calcFaTHessian( std::vector<std::shared_ptr<SystemController> > activeSystems,
  ...
  for (unsigned int i=0; i!=nAtoms;++i){
    ...
    systemGeometry->getAtoms()[i]->addToX(delta);
      ...
      // Active system cycles
        {GradientTask<T> Gradtask(activeSystems,passiveSystems);
        Gradtask.settings.naddKinFunc = FaTnaddKinFunc;
        Gradtask.settings.naddXCFunc = FaTnaddXCFunc;
        Gradtask.settings.FaTmaxCycles = FatmaxCycles;
        Gradtask.settings.FaTenergyConvThresh = FaTenergyConvThresh;
        Gradtask.settings.FDEgridCutOff = FaTgridCutOff;
        Gradtask.settings.dispersion= dispersion;
        Gradtask.settings.print = false;
        Gradtask.run();}
...
template<Options::SCF_MODES T>
void NumericalHessianCalc<T>::frequencyCalculation(Matrix<double> hessian, std::shared_ptr<Geometry> geometry, const std::vector<Settings> settings) {
  ...
  for (unsigned int i=0; i!=nAtoms;++i){
    for (unsigned int j=0; j!=nAtoms;++j){
      hessian(3*i+0,3*j+0) *= 1.0/(sqrt(atoms[i]->getAtomType()->getMass())*sqrt(atoms[j]->getAtomType()->getMass()));
      hessian(3*i+0,3*j+1) *= 1.0/(sqrt(atoms[i]->getAtomType()->getMass())*sqrt(atoms[j]->getAtomType()->getMass()));
      hessian(3*i+0,3*j+2) *= 1.0/(sqrt(atoms[i]->getAtomType()->getMass())*sqrt(atoms[j]->getAtomType()->getMass()));
      hessian(3*i+1,3*j+0) *= 1.0/(sqrt(atoms[i]->getAtomType()->getMass())*sqrt(atoms[j]->getAtomType()->getMass()));
      hessian(3*i+1,3*j+1) *= 1.0/(sqrt(atoms[i]->getAtomType()->getMass())*sqrt(atoms[j]->getAtomType()->getMass()));
      hessian(3*i+1,3*j+2) *= 1.0/(sqrt(atoms[i]->getAtomType()->getMass())*sqrt(atoms[j]->getAtomType()->getMass()));
      hessian(3*i+2,3*j+0) *= 1.0/(sqrt(atoms[i]->getAtomType()->getMass())*sqrt(atoms[j]->getAtomType()->getMass()));
      hessian(3*i+2,3*j+1) *= 1.0/(sqrt(atoms[i]->getAtomType()->getMass())*sqrt(atoms[j]->getAtomType()->getMass()));
      hessian(3*i+2,3*j+2) *= 1.0/(sqrt(atoms[i]->getAtomType()->getMass())*sqrt(atoms[j]->getAtomType()->getMass()));
    }
  }
  ...
  // Convert frequencies units to wavenumbers
    ...
    for (unsigned int freq = 0; freq < frequencies.size(); freq++){
      ...
      for (unsigned int normCoord = 0; normCoord < nAtoms; normCoord++){
        std::cout << "x  " << geometry->getAtoms()[normCoord]->getAtomType()->getElementSymbol()
              << "   " << normCoord+1
            << "   " << normalModes(3*normCoord+0,freq) << std::endl;
        std::cout << "y  " << geometry->getAtoms()[normCoord]->getAtomType()->getElementSymbol()
              << "   " << normCoord+1
            << "   " << normalModes(3*normCoord+1,freq) << std::endl;
        std::cout << "z  " << geometry->getAtoms()[normCoord]->getAtomType()->getElementSymbol()
              << "   " << normCoord+1
    ...
    for (auto atom : geometry->getAtoms()){
      atomIndex++;
    output << atom->getAtomType()->getElementSymbol()
       << "   " << atomIndex << "   "
       << atom->getAtomType()->getNuclearCharge() << " "
       << std::to_string(atom->getX()) << " " << std::to_string(atom->getY()) << " " << std::to_string(atom->getZ()) << "\n";
    }
    output << "[FREQ]\n";
    for (unsigned int freq = 0; freq < frequencies.size(); freq++){
    ...
    atomIndex = 0;
      for (auto atom : geometry->getAtoms()){
      output << normalModes(3*atomIndex+0,i) << " "
         << normalModes(3*atomIndex+1,i) << " "
