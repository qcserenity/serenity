...
class AtomGridFactory :
  public RememberingFactory<
    const AtomGrid,
    const Options::RADIAL_GRID_TYPES,
    const Options::SPHERICAL_GRID_TYPES,
    const std::shared_ptr<const AtomType>,
    const unsigned int > {
private:
  /**
...
public:
   ...
   */
  static std::shared_ptr<const AtomGrid> produce(
   ...
   * @returns an atom specific grid
   */
  std::unique_ptr<const AtomGrid> produceNew(
    ...
    const unsigned int accuracy) override final;
 ...
 */
   ...
   */
  static void _radialGrid(
      double alpha,
      unsigned int nRadial,
      std::vector<double>& radPoints,
      std::vector<double>& radWeights,
      const Options::RADIAL_GRID_TYPES& radType);
  /**
   ...
   */
  static void _lebedevSphericalGrid(
      unsigned int i,
      unsigned int& sphN,
