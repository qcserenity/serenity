...
#pragma omp parallel for ordered schedule(static)
    for (unsigned int i=0; i<nAtoms; ++i) {
      ...
      for (unsigned int j=0; j<nAtoms; ++j) {
        ...
        double braggSlaterQuot = sqrt(currentAtom->getAtomType()->getBraggSlaterRadius()
            / otherAtom->getAtomType()->getBraggSlaterRadius());
  ...
  atomGrids[0] = AtomGridFactory::produce(
      _radialType, _sphericalType, currentGridAtm->getAtomType(),_accuracyLevel);
#pragma omp parallel for schedule(dynamic)
  for (unsigned int k = 1; k < nAtoms; ++k) {
    const auto& currentGridAtom = atoms[k];
    /* get the reference grid of the current atom type */
    atomGrids[k] = AtomGridFactory::produce(
        _radialType, _sphericalType, currentGridAtom->getAtomType(), _accuracyLevel);
  }
...
#pragma omp parallel for schedule(dynamic)
  for (unsigned int k = 0; k < nAtoms; ++k) {
    ...
    for (unsigned int l = 0; l < nAtoms; ++l) {
      if(atomDistPtr[l+nAtoms*k]<40.0) significantAtoms.push_back(l);
      if(l!=k) minAtomDist=min(minAtomDist,atomDistPtr[l+nAtoms*k]);
    }
    /* get the reference grid of the current atom type */
          ...
          for (unsigned int i=0;i<nAtoms;i++) {
            ...
            for (unsigned int j=0;j<nAtoms;j++) {
              ...
              if (nuij<=-0.64) {
                ...
                const double poly = ( -5.0*n3*n3*nuij + 21.0*n3*nuij*nuij
                    -35.0*n3 + 35.0*nuij)/16.0 ;
                cellFunction *= 0.5 * (1 - poly);
              }
            }
            if (i == k) weight *= cellFunction;
            cellFunctionSum += cellFunction;
          }
          weight /= cellFunctionSum;
        }
...
#pragma omp parallel for schedule(dynamic)
  ...
  auto grid = std::unique_ptr<AtomCenteredGrid>(
      new AtomCenteredGrid(std::move(gridPoints), std::move(weights), gridIndicesOfAtoms));
...
constexpr double GridFactory::pRecursive(double nu, unsigned int k) {
  ...
  return (k > 1) ? pRecursive((3.0 - nu * nu) * nu / 2.0, k-1) :
      (3.0 - nu * nu) * nu / 2.0;
}
/* Eq. (21) */
