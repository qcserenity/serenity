...
public:
   ...
   */
  CoulombInteractionIntLooper(libint2::Operator op,
    ...
    for (int pIndex=shellPairsB1->size()-1; pIndex >= 0; --pIndex){
      auto& p = (*shellPairsB1)[pIndex];
        ...
        for (auto& q : *shellPairsB2) {
           ...
           */
          if (prescreenFunc(firstI, firstJ, firstA, firstB, nI, nJ, nA, nB, p.factor*q.factor)) continue;
