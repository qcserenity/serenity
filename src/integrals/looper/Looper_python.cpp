...
void loop4c(Looper looper,
            const std::function<void(const unsigned int&, const unsigned int&, const unsigned int&, const unsigned int&, 
...
R"(A looper for all 2e-4c Coulomb-like ordered integrals (i_A j_A|k_B) of two given bases (A,B), 
   ...
   instead it automiatically runs on a single core, no further user input needed.
   In the future this issue should, however, be resolved.)")
  .def(py::init<libint2::Operator,const unsigned int,std::shared_ptr<BasisController>,std::shared_ptr<BasisController>,double,std::pair<unsigned int, unsigned int> >(),
R"(The constructor.
   Args:
     op      (INT_OPERATORS): The operator used in the integrals.
     deriv    (unsigned int): The number of geometrical derivatives to compute.
