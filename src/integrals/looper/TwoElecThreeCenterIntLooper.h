...
public:
   ...
   *        @endcode
   */
  TwoElecThreeCenterIntLooper(libint2::Operator op,
...
#pragma omp parallel for schedule(static, 1)
     for (int kIndex = _auxbasis->reducedIndex(_kRange.second-1); kIndex >=(int)_auxbasis->reducedIndex(_kRange.first); --kIndex){
      ...
      for (auto& p : *shellPairs) {
        ...
        const unsigned int i = (swap)?p.bf2:p.bf1;
        const unsigned int j = (swap)?p.bf1:p.bf2;
        ...
        const unsigned int nJ = basis[j]->getNContracted();
           ...
           */
          if (prescreenFunc(_basis->extendedIndex(i),_basis->extendedIndex(j),
                            nI, nJ, p.factor*k.factor)) {
            continue;
          }
          // calculate integrals
          bool take;
          ...
          if (take){
            ...
            if (swap){
          	  if(integrals[threadId].cols()==9){
          		   const Eigen::VectorXd tmpx (integrals[threadId].col(3));
          		   const Eigen::VectorXd tmpy (integrals[threadId].col(4));
          		   const Eigen::VectorXd tmpz (integrals[threadId].col(5));
          		   integrals[threadId].col(3) = integrals[threadId].col(6);
          		   integrals[threadId].col(4) = integrals[threadId].col(7);
          		   integrals[threadId].col(5) = integrals[threadId].col(8);
          		   integrals[threadId].col(6) = tmpx;
          		   integrals[threadId].col(7) = tmpy;
          		   integrals[threadId].col(8) = tmpz;
            ...
            for (unsigned int K=0;K<nK;++K){
              ...
              for (unsigned int I=0;I<nI;++I){
                const unsigned int ii = _basis->extendedIndex(i)+I;
                for (unsigned int J=0;J<nJ;++J){
                  const unsigned int jj = _basis->extendedIndex(j)+J;
                  if (swap){
                    if (ii>jj) continue;
                    Eigen::VectorXd set(integrals[threadId].row(K*nJ*nI+I*nJ+J));
                    loopEvalFunction(jj,ii,kk,set,threadId);
                  } else {
                    if (jj>ii) continue;
                    Eigen::VectorXd set(integrals[threadId].row(K*nJ*nI+I*nJ+J));
                    loopEvalFunction(ii,jj,kk,set,threadId);
                  }
                } /* primitives of j -> J */
              } /* primitives of i -> I */
            } /* primitives of k -> K */
