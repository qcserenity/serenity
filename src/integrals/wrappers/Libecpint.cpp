...
#include "../ext/libecpint/include/libecpint/ecpint.hpp"
#include "../ext/libecpint/include/libecpint/gshell.hpp"
#include "../ext/libecpint/include/libecpint/multiarr.hpp"
...
MatrixInBasis<SCF_MODES::RESTRICTED> Libecpint::computeECPIntegrals(
    std::shared_ptr<BasisController> basisController,
    std::vector<std::shared_ptr<Atom> > atoms) {
  MatrixInBasis<SCF_MODES::RESTRICTED> result(basisController);
  result.block(0,0,result.rows(),result.cols()) = computeECPIntegrals(basisController,basisController,atoms)
          .block(0,0,result.rows(),result.cols());
  return result;
}

SPMatrix<Options::SCF_MODES::RESTRICTED> Libecpint::computeECPIntegrals(
  ...
  unsigned int maxAngMomBasis = (basisControllerA->getMaxAngularMomentum() < basisControllerB->getMaxAngularMomentum())?
      basisControllerB->getMaxAngularMomentum() : basisControllerA->getMaxAngularMomentum();
  // Parallel region
#pragma omp parallel
  {
        ...
        if(atom->usesECP()) {
          ...
          if(distants < 1e-3 && distants > 1e-10) {
            WarningTracker::printWarning(
                "Warning: A center of an ECP and a basis function shell are close but not identical. I will assume identical centeres.",
          ...
          if (atom->usesECP()) {
            ...
            for (unsigned int ii = 0; ii < nI; ++ii) {
              for (unsigned int jj = 0; jj < nJ; ++jj) {
                ...
                if (shellI->isCartesian()) normFactor *=shellI->getNormFactors()[ii];
                if (shellJ->isCartesian()) normFactor *=shellJ->getNormFactors()[jj];
                eigenShellResult(ii,jj) = normFactor * shellResult(ii,jj);
              }
            }
            // Transform to spherical harmonics if necessary.
            if (shellI->isSpherical()) {
              eigenShellResult = CartesianToSphericalTransformer::getTransformationMatrix(shellI->getAngularMomentum()).transpose() * eigenShellResult;
            }
            if (shellJ->isSpherical()){
              eigenShellResult = eigenShellResult * CartesianToSphericalTransformer::getTransformationMatrix(shellJ->getAngularMomentum());
            }
            // unpack
            result.block(firstI,firstJ,eigenShellResult.rows(),eigenShellResult.cols()) += eigenShellResult;
            if (i != j && singleBasisMode) result.block(firstJ,firstI,eigenShellResult.cols(),eigenShellResult.rows()) += eigenShellResult.transpose();
          }
        } // atoms loop
      } // j loop
    } // i loop
