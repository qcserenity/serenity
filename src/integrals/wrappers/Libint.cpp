...
Libint::Libint(){
  ...
  for (unsigned int k=0; k<3;++k) {
    for (unsigned int j=2; j<5;++j) {
      ...
      _engines[it] = std::vector<std::unique_ptr<libint2::Engine> >(nprocs);
      it = IntegralType(libint2::Operator::emultipole3,k,j);
      _engines[it] = std::vector<std::unique_ptr<libint2::Engine> >(nprocs);
  ...
  for (unsigned int k=0; k<3;++k) {
    for (unsigned int j=2; j<5;++j) {
      ...
      _keep[IntegralType(libint2::Operator::emultipole3,k,j)]    = 0;
...
// TODO it should be possible to generate engines in each thread. 
//#pragma omp parallel for schedule (static,1)
  for (unsigned int i=0; i<(unsigned int)evector.size();++i) {
    ...
    if (op ==  libint2::Operator::coulomb){
      if (nCenter==4){
    	    evector[i].reset(new libint2::Engine(op,N_PRIM_MAX,
    	        AM_MAX,
    	        deriv,
      ...
      if (nCenter==4){
          evector[i].reset(new libint2::Engine(op,N_PRIM_MAX,
              ...
              deriv,
              std::numeric_limits<double>::epsilon(),
        libint2::operator_traits<libint2::Operator::erf_coulomb>::default_params(),
        libint2::BraKet::xx_xx));
      } else if (nCenter==3){
          evector[i].reset(new libint2::Engine(op,N_PRIM_MAX,
              AM_MAX,
...
void Libint::initialize(libint2::Operator op,
  ...
  for (unsigned int j = 0; j< atoms.size(); ++j ){
    q.push_back( {static_cast<double>(atoms[j]->getEffectiveCharge()), {{atoms[j]->getX(), atoms[j]->getY(), atoms[j]->getZ()}}} );
  }
...
void Libint::finalize(libint2::Operator op, unsigned int deriv, unsigned int nCenter){
  ...
  for(auto const &engineMat : _engines) {
    for(unsigned int i=0; i<(unsigned int)omp_get_max_threads();++i){
      if(engineMat.second[i] != nullptr) return;
    }
  }
  if (libint2::initialized())libint2::finalize();
}

void Libint::keepEngines(libint2::Operator op,
    const unsigned int deriv,
    const unsigned int nCenter){
  IntegralType it(op,deriv,nCenter);
  _keep[it] += 1;
...
void Libint::freeEngines(libint2::Operator op,
    const unsigned int deriv,
    const unsigned int nCenter){
  IntegralType it(op,deriv,nCenter);
  if(_keep[it] >0) _keep[it] -= 1;
  if (_keep[it]==0) this->finalize( op, deriv,  nCenter);
}
...
void Libint::clearAllEngines(){
  ...
  bool Libint::compute(libint2::Operator op,
      unsigned int deriv,
  ...
  Eigen::MatrixXd Libint::compute1eInts(libint2::Operator op,
      ...
      for(unsigned int i = 0; i < bas.size(); ++i){
        ...
        for (unsigned int j = 0; j <= i; ++j){
          ...
          if(this->compute(op,0,*bas[i],*bas[j],ints)){
            ...
            if (i!=j) results.block(offI,offJ,nI,nJ)=tmp.transpose();
          }
        }
      }
    } /* END OpenMP parallel */
