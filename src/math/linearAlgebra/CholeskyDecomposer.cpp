...
CholeskyDecomposer::CholeskyDecomposer(
    ...
    const double decompositionThreshold,
    const double screeningDamping,
    const double spanFactor,
    const unsigned int nMaxQual,
    const double negativeThreshold,
    const double negativeFailThreshold,
    const double pruningThreshold):
      _diagonal(diagonal),
...
std::unique_ptr<Eigen::SparseMatrix<double>> CholeskyDecomposer::getCholeskyVectors() {
  _choleskyVectors = std::unique_ptr<Eigen::SparseMatrix<double> >(
      new Eigen::SparseMatrix<double>(_diagonal.rows(),_diagonal.rows()));
  decompose();
...
//  tripletList.reserve(_diagonal.rows() * _diagonal.rows());
  ...
  do {
    ...
    do {
      ...
      for (unsigned int q = 0; q < qualifiedSet.size(); ++q) {
        if (_diagonal(qualifiedSet[q]) == 0.0) continue;
      ...
      for (unsigned int p = 0; p < _diagonal.rows(); ++p) {
        if ((*qualifiedCols)(p,qMaxIndex) == 0.0) continue;
...
std::vector<unsigned int> CholeskyDecomposer::calculateQualifiedSet(
    double dMin) {
...
void CholeskyDecomposer::findQmax(
    double& qMax,
    unsigned int& qMaxIndex,
    std::vector<unsigned int>& qualifiedSet) {
