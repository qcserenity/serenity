...
TEST_F(CholeskyDecomposerTest, CholeskyDecomposition) {
  ...
  auto columnCalculator = [&](std::vector<unsigned int>& qualifiedSet) {
    auto colMat = std::unique_ptr<Eigen::MatrixXd>(new Eigen::MatrixXd(diagonal.rows(), qualifiedSet.size()));
