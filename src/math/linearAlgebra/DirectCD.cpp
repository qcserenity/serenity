...
DirectCD::DirectCD(VectorXd diagonal, std::function<Matrix<double>(VectorXi& qualifiedSet)> matrixColumnCalculator,
                   double decompositionThreshold, double spanFactor)
  : _diagonal(diagonal),
...
void DirectCD::decompose() {
  ...
  do {
      ...
      for (unsigned int p = 0; p < _nDimension; ++p) {
        for (unsigned int q = 0; q < nQualified; ++q) {
          for (unsigned int j = 0; j < _nCholesky; ++j) {
            qualifiedCols(p, q) -= _choleskyVectors.coeffRef(p, j) * _choleskyVectors.coeffRef(qualifiedSet(q), j);
          }
        }
      }
    }
    ...
    do {
      ...
      for (unsigned int p = 0; p < _nDimension; ++p) {
        _diagonal(p) -= _choleskyVectors.coeffRef(p, iCho) * _choleskyVectors.coeffRef(p, iCho);
      }
      // update qualified columns
      for (unsigned int p = 0; p < _nDimension; ++p) {
        for (unsigned int q = 0; q < nQualified; ++q) {
          qualifiedCols(p, q) -= _choleskyVectors.coeffRef(p, iCho) * _choleskyVectors.coeffRef(qualifiedSet(q), iCho);
        }
      }
