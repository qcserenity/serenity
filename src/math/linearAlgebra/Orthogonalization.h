 ...
 /**
  ...
  static void modifiedGramSchmidtLinDep(std::vector<Eigen::MatrixXd>& A, double thresh) {
    ...
    for(unsigned i = 0; i < nCols; ++i){
      ...
      for(unsigned iSet = 0; iSet < nSets; ++iSet){
        ...
        if(A[iSet].col(i).norm() > thresh) ++append;
      }
   ...
   */
   ...
   static void luBiorthogonalization(Eigen::MatrixXd& A, Eigen::MatrixXd& B) {
     ...
     Eigen::MatrixXd Lm1 = lu.matrixLU().triangularView<Eigen::UnitLower>().solve(Eigen::MatrixXd::Identity(A.cols(), A.cols()));
     A = (Lm1 * lu.permutationP().inverse() * A.transpose()).transpose();
     Eigen::MatrixXd Um1 = lu.matrixLU().triangularView<Eigen::Upper>().solve(Eigen::MatrixXd::Identity(B.cols(), B.cols()));
     B = B * Um1;
   }
