...
BFGS::BFGS(Eigen::VectorXd& parameters, double initialStepLength, bool lineSearch, Eigen::MatrixXd initialInverseHess):
  ...
  _initialStepLength(initialStepLength),
  _lineSearch(lineSearch),
  _initialInverseHess(initialInverseHess){
}

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wunused-parameter" // value is ignored -> not needed
void BFGS::optimize(std::function<bool(const Eigen::VectorXd&, double&, Eigen::VectorXd&,std::shared_ptr<Eigen::MatrixXd> hessian, bool print)>updateFunction)  {


  /*
   * Get first set of parameters, gradients
   */
  double value=9999999.9;
  ...
  // I have found multiple accounts claiming c1 should be very small (around 0.0001) while some wrote it should be close to 0.5 (With the original publication putting it at 0.5)
    ...
    while(true){
      ...
      if(value <= (oldValue + c1*stepLength*gradientsOld.dot(stepVector))){
        ...
        if(gradients.dot(stepVector) >= (c2*gradientsOld.dot(stepVector))){
          ...
          //Backtrack if energy increased, otherwise just keep the step to save time despite requiring a change in the steplength
          if(value>oldValue){
            std::cout << "Energy increased... Backtracking..." << std::endl;
            this->_parameters -= stepLength*stepVector;
          ...
          std::cout << "Increasing Steplength" << std::endl;
          stepLength *= 1.5;
        }
      } else {
        //Backtrack if energy increased, otherwise just keep the step to save time despite requiring a change in the steplength
        ...
        std::cout << "Decreasing Steplength" << std::endl;
        stepLength *= 0.5;
      }
    } else {
      /*
