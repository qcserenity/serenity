...
TEST(NewtonRaphsonTest, QuadraticConvergenceWrapper) {
  ...
  auto f = [](const Eigen::VectorXd& vec) {
    return (vec[0]*vec[0]*vec[0]*vec[0]-16.0*vec[0]*vec[0]+5.0*vec[0])/2.0+(vec[1]*vec[1]*vec[1]*vec[1]-16.0*vec[1]*vec[1]+5.0*vec[1])/2.0;
  };
  ...
  auto d2fdx2 = [](const Eigen::VectorXd& vec) {
    return (4.0*3.0*vec[0]*vec[0]-16.0*2.0)/2.0;
  };
  auto d2fdy2 = [](const Eigen::VectorXd& vec) {
    return (4.0*3.0*vec[1]*vec[1]-16.0*2.0)/2.0;
  };
