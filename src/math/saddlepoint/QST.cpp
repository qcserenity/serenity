...
QST::QST(const Eigen::VectorXd& min1,
         const Eigen::VectorXd& min2,
         bool preopt,
...
void QST::optimize(std::function<void(const Eigen::VectorXd&,
                                      double&,
                                      Eigen::VectorXd&,
                                      bool)>updateFunction){
  ...
  if (_guess) assert(_guess->size()==_min1.size());
  _tangent.setZero();
  ...
  for (unsigned int nTotCycles=0; nTotCycles<100;nTotCycles++){
    ...
    if (t>1.0 or t<0.0) {
      ...
      throw SerenityError(
              (std::string)"The TS guess is not on a QST path between the minima (t="+std::to_string(t)+")!");
    }
    ...
    if (deltaE  < 1e-06) convCriteriaMet++;
    if (rmsGrad < 1e-02) convCriteriaMet++;
    if (maxGrad < 2e-02) convCriteriaMet++;
    ...
    auto const updateFunction2 = [&]
      ...
      if(print)std::cout<<"Minimization Cycle "<<nMinCycles+1<<"("<<nTotCycles+1<<")"<<std::endl;
      bool minConverged=false;
      // convergence tests
      if (nMinCycles>0 and  orthGrad.norm() < 1e-05){
        if(print)std::cout<<"minimization stopped bc gradnorm"<<std::endl;
        nTotCycles++;
        minConverged=true;
        _energy=value;
        _gradient=gradients;
