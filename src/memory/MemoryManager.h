 ...
 *
 * Some routines can gain a significant speedup when granted huge chunks of memory. In larger
 * calculations not all of that data can be kept in memory any more and the corresponding objects
 ...
 * but because the manager would have to manage ever object created to have an actual feel for the memory.
 * E.g. 100 density matrices were not be managed but ERIs for one of them were, it is easy to see that the
 * matrices them selves can easily be the bigger chunck of memory.
 *
 * At the moment the idea is to have a class that can tell if big chinks can be added still, and it does
...
public:
  ...
  /**
   *
   * @return The amount of memory used by the current process in total risght now, value in bytes.
   */
  ...
  /**
   * @brief The maxiumum system RAM
   * @returns The maxiumum system memory available.
   */
