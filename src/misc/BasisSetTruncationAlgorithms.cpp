...
BasisSetTruncationAlgorithms::BasisSetTruncationAlgorithms(
    std::shared_ptr<SystemController> activeSystem) :
        _activeSystem(activeSystem){
}

Eigen::VectorXi BasisSetTruncationAlgorithms::netPopulationTruncation(
    const DensityMatrix<Options::SCF_MODES::RESTRICTED> dMatActiveSystem,
  ...
  if (netPopThreshold <= 0.0) {
    WarningTracker::printWarning(
        "WARNING: You selected the 'net population truncation' with a threshold <= 0.",
...
Eigen::VectorXi BasisSetTruncationAlgorithms::primitiveNetPopTruncation(
    const DensityMatrix<Options::SCF_MODES::RESTRICTED> dMatActiveSystem,
    double truncationFactor) {
  assert(truncationFactor >= 0.0
      && "A negative ratio of kept basis functions does not make any sense.");
  unsigned int nBFstot = dMatActiveSystem.cols();
  unsigned int nShellsTot = _activeSystem->getBasisController()->getReducedNBasisFunctions();
  Eigen::VectorXd importance = Eigen::VectorXd::Zero(nBFstot);
  auto sMatSuper(_activeSystem->getOneElectronIntegralController()->getOverlapIntegrals());
...
//  auto idxNotRed = _activeSystem->getAtomCenteredBasisController()->getBasisIndices();
  ...
  for (const auto& atom : actAtoms){
    ...
    if(not atom->isDummy()) {
      ...
      for(const auto& shell : shells) nBasisFunctions += shell->getNContracted();
...
void BasisSetTruncationAlgorithms::truncateBasis(
    Options::BASIS_SET_TRUNCATION_ALGORITHMS truncationAlgorithm,
    double truncationFactor,
 ...
 *                       Helper Functions
 * ============================================================== */
inline void BasisSetTruncationAlgorithms::buildTruncatedBasis(
    Eigen::VectorXi importantShells
    ) {
  ...
  for (const auto& atom : activeAtoms){
    if (atom->isDummy()) {
      Eigen::VectorXi tmp=Eigen::VectorXi::Zero(idx[atomIndex].second-idx[atomIndex].first);
      ...
      Eigen::VectorXi tmp=Eigen::VectorXi::Ones(idx[atomIndex].second-idx[atomIndex].first);
      activeShells.push_back(tmp);
    }
    ++atomIndex;
  }// for i
  ...
  std::cout << "Supersystem basis: " << nBFstot << " basis functions" <<std::endl;
  std::cout << "Active basis: " << nBFsAct << " basis functions" << std::endl;
  std::cout << "Truncated supersystem basis by " << nBFstot - nBFsAct << " basis functions." <<std::endl<<std::endl;
}
