 ...
 *   of the subsystems as the diagonal blocks. The error associated with a not fully relaxed density matrix is given by the
 *   commutator \f$[F,P]\f$ which is a block diagonal matrix as well.\n
 *   Both matrices are mapped to a vector and optimized using a standard DIIS (math/diis/DIIS.h) and then mapped back
 *   to their matrix formulation before the density matrices of the subsystems are updated. For more information about
 *   the DIIS procedure see math/diis/DIIS.h.\n
...
public:
	 ...
	 * @param conditionNumberThreshold The threshold which determines when the the density vector is considered to be ill conditioned.
	 * @param settings The FreezeAndThawTaskSettings, which are used for in the FaT calculations. Contains all settings for the DIIS/Damping.
	 ...
	 * @param diisStartError The density RMSD threshold after which the DIIS starts.
	 */
  FaTConvergenceAccelerator(
	...
	/**
	 * @brief Performs an DIIS step or damps.
	 * @param energy The energy associates with the current density.
	 */
	void accelerateConvergence(double energy);
