...
void takeTime(const string label) {
  if (STARTING_TIMES.find(label) == STARTING_TIMES.end()) {
    ...
    cout << "Small error: You try to add a timer with a label which is already present."
         << endl;
...
void timeTaken(const unsigned int printLevel, string label) {
  if (STARTING_TIMES.find(label) != STARTING_TIMES.end()) {
    ...
    if (printLevel <= iOOptions.timingsPrintLevel) {printTime(
        (endTime.tv_sec - startingTime.tv_sec) * 1000000000 + endTime.tv_nsec - startingTime
            .tv_nsec, label);}
    STARTING_TIMES.erase(label);
  } else {
    cout
        << "Small error: you want to print a timing of something of which there is no starting time stored."
        << endl;
    cout << "Maybe you already printed its timing (which stops the timer)? The label is "
         << label << "." << endl;
  }
}
...
void Timings::printTimes(){
  for (auto it = _timings.begin(); it != _timings.end(); it++ ){
    ...
    if (timeTakenInSeconds > 60.0) {
      int timeTakenInMinutes = (int)(timeTakenInSeconds / 60);
      printf("%4s %40s %6i:%06.3f min\n","",it->first.c_str(),timeTakenInMinutes,(timeTakenInSeconds - (60 * timeTakenInMinutes)));
    } else {
