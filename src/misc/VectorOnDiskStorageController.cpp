...
void VectorOnDiskStorageController::storeVectorSegment(
  ...
  if ((_nDoubles+vectorPart->size())*8 > _cacheLimit) {
    ...
    _vectorPool.insert(std::pair<std::string, std::shared_ptr<Eigen::VectorXd> >
    (label,nullptr));
  } else {
    //Cache limit is not reached yet! Save copy in cache.
    _vectorPool.insert(std::pair<std::string, std::shared_ptr<Eigen::VectorXd> >
    (label,std::make_shared<Eigen::VectorXd>(*vectorPart)));
    _nDoubles += vectorPart->size();
  }
}

std::shared_ptr<Eigen::VectorXd> VectorOnDiskStorageController::getVectorSegment(
    std::string label) {
  /*
   * Search cache.
...
double VectorOnDiskStorageController::operator*(
...
std::shared_ptr<Eigen::VectorXd> VectorOnDiskStorageController::loadVectorSegment(
    std::string label) {
...
void VectorOnDiskStorageController::saveVectorSegment(
...
void VectorOnDiskStorageController::deleteVectorSegmentFromController(std::string label) {
  if(_vectorPool[label])_nDoubles -= _segmentSizes[label];
