...
std::pair<double,double> CCSD::calculateElectronicEnergyCorrections(){
  if (_converged) return std::make_pair(_MP2EnergyCorrection,calculateCCSDEnergyCorrection());
  prepERIS();
...
#pragma omp for schedule(dynamic)
   ...
   * t2 update
   */
  auto newt2_ptr = std::unique_ptr<Matrix<Matrix<double> > > (new Matrix<Matrix<double> >(nocc,nvirt,Eigen::MatrixXd::Zero(nocc,nvirt)));
...
void CCSD::updateF(){
  ...
  if (!_foo) _foo = std::unique_ptr<Matrix<double> > (new Matrix<double>(nocc,nocc));
  if (!_fov) _fov = std::unique_ptr<Matrix<double> > (new Matrix<double>(nocc,nvirt));
  if (!_fvv) _fvv = std::unique_ptr<Matrix<double> > (new Matrix<double>(nvirt,nvirt));
  auto& foo = *_foo;
  auto& fov = *_fov;
  auto& fvv = *_fvv;
  foo.setZero();
...
void CCSD::prepERIS(){
  ...
  if (!_eris){
    _eris = std::unique_ptr<RegularRankFourTensor<double> >(
        new RegularRankFourTensor<double>(nBasisFunc, 0.0));
  }
  auto& eris = *_eris;
