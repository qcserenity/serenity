...
double DeltaSpinSquared<Options::SCF_MODES::UNRESTRICTED>::calculateSpinSquared(unsigned int iState) {
  ...
  for (auto lrscf : _lrscf) {
    ...
    Eigen::MatrixXd x_a = (Eigen::Map<Eigen::MatrixXd>(excVec[0].col(iState).data(), nVirt.alpha, nOcc.alpha)).transpose();
    Eigen::MatrixXd x_b =
    ...
    if (_method == Options::LR_METHOD::TDDFT) {
      Eigen::MatrixXd y_a = (Eigen::Map<Eigen::MatrixXd>(excVec[1].col(iState).data(), nVirt.alpha, nOcc.alpha)).transpose();
      Eigen::MatrixXd y_b =
