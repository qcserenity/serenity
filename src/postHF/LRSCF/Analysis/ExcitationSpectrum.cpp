...
void ExcitationSpectrum<SCFMode>::printSpectrum(const std::shared_ptr<DipoleIntegrals<SCFMode> > dipoles,
  ...
  auto dip_l = dipoles->getLengths(Options::INTEGRAL_TYPE::ANALYTICAL);
  auto dip_v = dipoles->getVelocities(Options::INTEGRAL_TYPE::ANALYTICAL);
  auto dip_m = dipoles->getMagnetics(Options::INTEGRAL_TYPE::ANALYTICAL);
  ...
  Eigen::MatrixXd tdm_m = factor / 2.0 / SPEEDOFLIGHT_AU * xmy.transpose() * (*dip_m);
  for(unsigned int iState = 0; iState < tdm_v.rows(); iState++) tdm_v.row(iState) /= eigenvalues(iState);
  ...
  for (unsigned int iState = 0; iState < eigenvalues.rows(); ++iState) {
    printf(" %3i %10.4f %10.1f %8.1f %12.4f %9.4f %8.4f %8.4f %8.4f\n",
           ...
           eigenvalues(iState) * HARTREE_TO_OOCM,
           HARTREE_TO_NM / eigenvalues(iState),
           AU_TO_CGS * rs_l(iState),
           tdm_m.row(iState).array().square().sum(),
           tdm_m(iState, 0), tdm_m(iState, 1), tdm_m(iState, 2));
  }
  printf("---------------------------------------------------------------------------------------\n");
