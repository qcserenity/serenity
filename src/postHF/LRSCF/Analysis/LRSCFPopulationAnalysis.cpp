...
void LRSCFPopulationAnalysis<SCFMode>::calculateTransitionCharges(
  ...
  for (unsigned I = 0; I < lrscf.size(); ++I) {
    ...
    auto xwf = lrscf[I]->getXWFController();
    ...
    for (unsigned iEigen = 0; iEigen < nEigen; ++iEigen) {
      ...
      for_spin(P, H, nv, no) {
        ...
        // Transition density
        densityAO += P_spin.middleCols(xwf ? 0 : nh, np) * xpy * H_spin.leftCols(nh).transpose();
        // Hole density.
        holeAO -=
            0.5 * H_spin.leftCols(nh) * (xpy.transpose() * xmy + xmy.transpose() * xpy) * H_spin.leftCols(nh).transpose();
        // Particle density.
        particleAO += 0.5 * P_spin.middleCols(xwf ? 0 : nh, np) * (xmy * xpy.transpose() + xpy * xmy.transpose()) *
                      P_spin.middleCols(xwf ? 0 : nh, np).transpose();
        iStartSpin += np * nh;
      };
