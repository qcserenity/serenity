...
template<Options::SCF_MODES T> Kernel<T>::Kernel(
    std::vector<std::shared_ptr<SystemController> > act,
  ...
  if (_superSystemGrid) {
    ...
    _gridController = GridControllerFactory::produce(
        geometry, _systems[0]->getSettings(), Options::GRID_PURPOSES::DEFAULT);
  }
  ...
  if (_systems.size() == 1) {
    ...
    _naddKinFunc = Options::KINFUNCTIONALS::NONE;
    _naddXCFunc = Options::XCFUNCTIONALS::NONE;
  }
  Functional EnaddXC = FunctionalClassResolver::resolveFunctional(naddXCFunc);
  Functional EnaddKIN = FunctionalClassResolver::resolveFunctional(naddKinFunc);
  if (EnaddKIN.isHybrid() || EnaddKIN.isRSHybrid()) throw SerenityError("ERROR: Found hybrid functional for non-additive kinetic kernel.");
  if (EnaddXC.getFunctionalClass() == FUNCTIONAL_CLASSES::GGA ||
      EnaddKIN.getFunctionalClass() == FUNCTIONAL_CLASSES::GGA) _gga = true;
  for (auto sys : _systems) {
    Functional EXC = FunctionalClassResolver::resolveFunctional(sys->getSettings().dft.functional);
    if (EXC.getFunctionalClass() == FUNCTIONAL_CLASSES::GGA) _gga = true;
  }
  if (FunctionalClassResolver::resolveFunctional(_func).getFunctionalClass() == FUNCTIONAL_CLASSES::GGA) _gga = true;
  //Initialize storage objects (this ensures that key is already present when working with it)
  for (auto sys : _systems) {
    _pp.insert(std::make_pair(sys->getSettings().name,DoublySpinPolarizedData<T,GridData<Options::SCF_MODES::RESTRICTED> >(_gridController)));
    if (_gga) _pg.insert(std::make_pair(sys->getSettings().name, makeGradient<DoublySpinPolarizedData<T,GridData<Options::SCF_MODES::RESTRICTED> > >(_gridController)));
    if (_gga) _gg.insert(std::make_pair(sys->getSettings().name, makeHessian<DoublySpinPolarizedData<T,GridData<Options::SCF_MODES::RESTRICTED> > >(_gridController)));
  }
  if (_systems.size() > 1) {
...
template<>
const std::shared_ptr<DoublySpinPolarizedData<Options::SCF_MODES::RESTRICTED,Eigen::VectorXd > > Kernel<Options::SCF_MODES::RESTRICTED>::getPP(
      unsigned int I,
      unsigned int J,
      unsigned int blockSize,
      unsigned int iGridStart) {
      ...
      if(_systems.size() > 1)*pp_Block += (*_pptot).segment(iGridStart,blockSize);
...
template<>
const std::shared_ptr<DoublySpinPolarizedData<Options::SCF_MODES::UNRESTRICTED,Eigen::VectorXd > > Kernel<Options::SCF_MODES::UNRESTRICTED>::getPP(
    unsigned int I,
    unsigned int J,
    unsigned int blockSize,
    unsigned int iGridStart) {
...
template<>
const std::shared_ptr<Gradient<DoublySpinPolarizedData<Options::SCF_MODES::RESTRICTED, Eigen::VectorXd > > > Kernel<Options::SCF_MODES::RESTRICTED>::getPG(
      unsigned int I, unsigned int J, unsigned int blockSize, unsigned int iGridStart) {
...
template<>
const std::shared_ptr<Gradient<DoublySpinPolarizedData<Options::SCF_MODES::UNRESTRICTED, Eigen::VectorXd > > > Kernel<Options::SCF_MODES::UNRESTRICTED>::getPG(
    unsigned int I, unsigned int J, unsigned int blockSize, unsigned int iGridStart) {
...
template<>
const std::shared_ptr<Hessian<DoublySpinPolarizedData<Options::SCF_MODES::RESTRICTED,Eigen::VectorXd > > > Kernel<Options::SCF_MODES::RESTRICTED>::getGG(
    unsigned int I, unsigned int J, unsigned int blockSize, unsigned int iGridStart) {
...
template<>
const std::shared_ptr<Hessian<DoublySpinPolarizedData<Options::SCF_MODES::UNRESTRICTED,Eigen::VectorXd > > > Kernel<Options::SCF_MODES::UNRESTRICTED>::getGG(
    unsigned int I, unsigned int J, unsigned int blockSize, unsigned int iGridStart) {
...
void Kernel<Options::SCF_MODES::RESTRICTED>::storeDerivatives(
    ...
    DensityOnGrid<Options::SCF_MODES::RESTRICTED>& p,
    FUNCTIONAL_CLASSES funcClass,
    ...
    Gradient<DoublySpinPolarizedData<Options::SCF_MODES::RESTRICTED,GridData<Options::SCF_MODES::RESTRICTED> > >& pg,
    int pm) {
  ...
  if (funcClass == FUNCTIONAL_CLASSES::GGA) {
...
void Kernel<Options::SCF_MODES::UNRESTRICTED>::storeDerivatives(
    ...
    DensityOnGrid<Options::SCF_MODES::UNRESTRICTED>& p,
    FUNCTIONAL_CLASSES funcClass,
...
void Kernel<T>::calculateDerivatives() {
  ...
  Functional EnaddXC = FunctionalClassResolver::resolveFunctional(_naddXCFunc);
  Functional EnaddKIN = FunctionalClassResolver::resolveFunctional(_naddKinFunc);
  std::vector<BASIC_FUNCTIONALS> naddBasicFunctionals;
  ...
  if (EnaddXC.getFunctionalClass() == FUNCTIONAL_CLASSES::NONE && EnaddKIN.getFunctionalClass() == FUNCTIONAL_CLASSES::NONE) {
    naddBasicFunctionals.push_back(BASIC_FUNCTIONALS::NONE);
    naddMixingFactors.push_back(0.0);
  } else {
    for (unsigned int i = 0; i < EnaddKIN.getNBasicFunctionals(); ++i) {
      if (FunctionalClassResolver::resolveFunctionalClass(EnaddKIN.getBasicFunctionals()[i]) == FUNCTIONAL_CLASSES::NONE) continue;
    ...
    if (!EnaddXC.isRSHybrid() && !EnaddXC.isHybrid()) {
      ...
      for (unsigned int i = 0; i < EnaddXC.getNBasicFunctionals(); ++i) {
        if (FunctionalClassResolver::resolveFunctionalClass(EnaddXC.getBasicFunctionals()[i]) == FUNCTIONAL_CLASSES::NONE) continue;
        naddBasicFunctionals.push_back(EnaddXC.getBasicFunctionals()[i]);
        naddMixingFactors.push_back(EnaddXC.getMixingFactors()[i]);
      }
    }
    if (naddBasicFunctionals.empty()) naddBasicFunctionals.push_back(BASIC_FUNCTIONALS::NONE);
    if (naddMixingFactors.empty()) naddMixingFactors.push_back(0.0);
  }
  Functional totFunc(naddBasicFunctionals,naddMixingFactors);
  ...
  for (auto sys : _systems) {
    ...
    std::vector<BASIC_FUNCTIONALS> sysBasicFunctionals;
    ...
    Functional EXC = FunctionalClassResolver::resolveFunctional(sys->getSettings().dft.functional);
    if (_func != Options::XCFUNCTIONALS::NONE)   EXC = FunctionalClassResolver::resolveFunctional(_func);
    ...
    Functional sysFunc(sysBasicFunctionals,sysMixingFacotrs);
    ...
    storeDerivatives(funcData, (*density), sysFunc.getFunctionalClass(),_pp.find(name)->second,_gg.find(name)->second,_pg.find(name)->second);
    if (EXC.isRSHybrid() || EXC.isHybrid()) {
      //Separate treatment for hybrids
      auto funcDataHybrid = xcfun.calcData(
          FUNCTIONAL_DATA_TYPE::GRADIENTS,
          EXC,
          densityOnGridController,
          2);
      storeDerivatives(funcDataHybrid, (*density), EXC.getFunctionalClass(),_pp.find(name)->second,_gg.find(name)->second,_pg.find(name)->second);
    }
    ...
    if (EnaddXC.isRSHybrid() || EnaddXC.isHybrid()) {
      ...
      auto funcDataHybrid = xcfun.calcData(
          ...
          densityOnGridController,
          2);
      storeDerivatives(funcDataHybrid, (*density), EnaddXC.getFunctionalClass(),_pp.find(name)->second,_gg.find(name)->second,_pg.find(name)->second,-1);
    }
  }
  //All done for conventional TDDFT
  if (_systems.size() == 1) return;
  //If none functionals are used, all is done
  if (EnaddKIN.getFunctionalClass() == FUNCTIONAL_CLASSES::NONE && EnaddXC.getFunctionalClass() == FUNCTIONAL_CLASSES::NONE) return;
  ...
  std::shared_ptr<ExternalDensityOnGridController<T> > totalDensityOnGridController = std::make_shared<ExternalDensityOnGridController<T> > (totalDensity,totDens);
...
template<>
std::shared_ptr<DensityMatrixDensityOnGridController<Options::SCF_MODES::RESTRICTED> > Kernel<Options::SCF_MODES::RESTRICTED>::getDensityOnGridController(std::shared_ptr<SystemController> sys) {
  auto basisFunctionOnGridController = BasisFunctionOnGridControllerFactory::produce(sys->getSettings(),sys->getBasisController(),_gridController);
  auto densityOnGridCalculator = std::make_shared<DensityOnGridCalculator<Options::SCF_MODES::RESTRICTED> >(basisFunctionOnGridController,sys->getSettings().grid.blockAveThreshold);
  ...
  if (sys->getLastSCFMode() == Options::SCF_MODES::UNRESTRICTED) {
    ...
    densityMatrixController = std::make_shared<DensityMatrixController<Options::SCF_MODES::RESTRICTED> > (sys->getActiveOrbitalController<Options::SCF_MODES::RESTRICTED>(),sys->getNOccupiedOrbitals<Options::SCF_MODES::RESTRICTED>());
  }
  return std::make_shared<DensityMatrixDensityOnGridController<Options::SCF_MODES::RESTRICTED> >(densityOnGridCalculator,densityMatrixController);
}

template<>
std::shared_ptr<DensityMatrixDensityOnGridController<Options::SCF_MODES::UNRESTRICTED> > Kernel<Options::SCF_MODES::UNRESTRICTED>::getDensityOnGridController(std::shared_ptr<SystemController> sys) {
  auto basisFunctionOnGridController = BasisFunctionOnGridControllerFactory::produce(sys->getSettings(),sys->getBasisController(),_gridController);
  auto densityOnGridCalculator = std::make_shared<DensityOnGridCalculator<Options::SCF_MODES::UNRESTRICTED> >(basisFunctionOnGridController,sys->getSettings().grid.blockAveThreshold);
  ...
  if (sys->getLastSCFMode() == Options::SCF_MODES::RESTRICTED) {
    WarningTracker::printWarning((std::string)"WARNING: USE OF RESTRICTED ENVIRONMENT SYSTEM IN UNRESTRICTED EMBEDDING RESPONSE CALCULATION",true);
    //Build unrestricted CoefficientMatrix from restricted one
    std::unique_ptr<CoefficientMatrix<Options::SCF_MODES::UNRESTRICTED> > c = std::unique_ptr<CoefficientMatrix<Options::SCF_MODES::UNRESTRICTED> >(new CoefficientMatrix<Options::SCF_MODES::UNRESTRICTED>(sys->getBasisController()));
    ...
    std::unique_ptr<SpinPolarizedData<Options::SCF_MODES::UNRESTRICTED, Eigen::VectorXd > > e = std::unique_ptr<SpinPolarizedData<Options::SCF_MODES::UNRESTRICTED, Eigen::VectorXd > >(new SpinPolarizedData<Options::SCF_MODES::UNRESTRICTED, Eigen::VectorXd >);
    ...
    std::shared_ptr<OrbitalController<Options::SCF_MODES::UNRESTRICTED> > orbitalController = std::make_shared<OrbitalController<Options::SCF_MODES::UNRESTRICTED>>(std::move(c),sys->getBasisController(),std::move(e));
    //Build DensityMatrixController
    densityMatrixController = std::make_shared<DensityMatrixController<Options::SCF_MODES::UNRESTRICTED> > (orbitalController,sys->getNOccupiedOrbitals<Options::SCF_MODES::UNRESTRICTED>());
  } else {
    densityMatrixController = std::make_shared<DensityMatrixController<Options::SCF_MODES::UNRESTRICTED> > (sys->getActiveOrbitalController<Options::SCF_MODES::UNRESTRICTED>(),sys->getNOccupiedOrbitals<Options::SCF_MODES::UNRESTRICTED>());
  }
  return std::make_shared<DensityMatrixDensityOnGridController<Options::SCF_MODES::UNRESTRICTED> >(densityOnGridCalculator,densityMatrixController);
}
