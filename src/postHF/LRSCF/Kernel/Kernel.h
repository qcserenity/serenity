...
*        \f[
*          f_{xc}^{IJ} = \frac{\delta^2 E_{xc}}{\delta \rho_{tot}^2} + \frac{\delta^2 T}{\delta \rho_{tot}^2} + \delta_{IJ} \frac{\delta^2 E_{xc}}{\delta \rho_{I}^2}
*                        - \delta_{IJ} \frac{\delta^2 E_{xc}}{\delta \rho_{I}^2} - \frac{\delta^2 T}{\delta \rho_{I}^2}
*        \f]
*        for subsystems I and J. Here, the terms \f$ \frac{\delta^2 E_{xc}}{\delta \rho_{tot}^2} \f$, \f$ \frac{\delta^2 T}{\delta \rho_{tot}^2} \f$,
*        \f$ - \delta_{IJ} \frac{\delta^2 E_{xc}}{\delta \rho_{I}^2} \f$ and \f$ \frac{\delta^2 T}{\delta \rho_{I}^2} \f$ are evaluated using the specified non-additive
*        density functionals. The \f$+ \delta_{IJ} \frac{\delta^2 E_{xc}}{\delta \rho_{I}^2} \f$ is evaluated using the exchange-correlation functional specified in the DFT
*        settings of the respective systems.
*        The second functional derivative of a GGA type density functional of the general type \f$ E = \int \epsilon(\rho,\nabla \rho) d\mathbf{r}\f$ evaluates to
*        \f[
*          \frac{\delta^2 E}{\delta \rho^2} = \frac{\partial^2 \epsilon}{\rho^2} - \nabla \frac{\partial^2 \epsilon}{\partial \rho \partial \nabla \rho} +
*                                             \sum_{ij} \nabla_i \nabla_j \frac{\partial^2 \epsilon}{\partial \nabla_i \rho \partial \nabla_j \rho} \; .
*        \f]
*        For the matrix elements of the kernel in the adiabatic approximantion, which are needed for the response matrix, we use integration by parts to move the derivative
*        operators which than solely act on the basis function products. It is thus only necessary to store \f$ \frac{\partial^2 \epsilon}{\rho^2} \f$,
*        \f$ \frac{\partial^2 \epsilon}{\partial \rho \partial \nabla \rho} \f$ and \f$ \frac{\partial^2 \epsilon}{\partial \nabla_i \rho \partial \nabla_j \rho} \f$ for all
*        \f$ i,j\f$ .
*        For more information, including also the unrestricted expressions, see e.g. M. Boeckers, J. Neugebauer, 2018.
*
*/
...
public:
   ...
   * @param superSystemGrid If true, use grid of the supersystem to store the derivatives (Default: use grid of active system)
  ...
  const std::shared_ptr<DoublySpinPolarizedData<T,Eigen::VectorXd > >
   ...
   */
  std::shared_ptr<GridController> getGridController() {return _gridController;};
...
private:
  ...
  Options::KINFUNCTIONALS _naddKinFunc;
  Options::XCFUNCTIONALS _naddXCFunc;
  ...
  void storeDerivatives(
      ...
      Gradient<DoublySpinPolarizedData<T,GridData<Options::SCF_MODES::RESTRICTED> > >& pg,
      int pm = 1);
