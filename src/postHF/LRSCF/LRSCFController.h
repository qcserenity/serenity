...
namespace Serenity {
  ...
  */
  ...
  public:
    ...
    *  
    */
    LRSCFController(std::shared_ptr<SystemController> system,
    ...
    ///@brief Returns the CoefficientMatrix stored in the LRSCFController.
    ...
    ///@brief Returns the corresponding eigenvalues to the stored orbitals can be modified in the LRSCFController.
    ...
    ///@brief Returns the FIJ elements for non-canonical orbitals
    std::shared_ptr<MatrixInBasis<SCFMode>> getFockNonCanon();

    ///@brief Set the FIJ elements for non-canonical orbitals
    ...
    */
    ...
    ///@brief Returns the excitation energies.
    ...
    void editReference(SpinPolarizedData<SCFMode,std::vector<unsigned int> > indexWhiteList, unsigned int system, Options::LRSCF_TYPE type);

    ///@brief Returns the list of indices of orbials included in the calculation.
