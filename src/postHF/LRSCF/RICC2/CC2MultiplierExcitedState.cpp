/**
 * @file CC2MultiplierExcitedState.cpp
 *
 * @date Apr 20, 2020
...
template<>
Eigen::MatrixXd CC2Controller<RESTRICTED>::calculateExcitedStateLagrangeMultiplier(std::vector<Eigen::MatrixXd>& eigenvectors,
                                                                                   Eigen::VectorXd eigenvalues) {
  unsigned nEigen = _settings.ccexdens ? eigenvalues.size() : 0;
  Eigen::MatrixXd rightHandSides = Eigen::MatrixXd::Zero(_no * _nv, 1 + nEigen);
  rightHandSides.col(0) = this->calculateGroundStateLagrangeMultiplier();
  ...
  for (unsigned iEigen = 0; iEigen < nEigen; ++iEigen) {
    ...
    Eigen::Ref<Eigen::VectorXd> rightHandSide = rightHandSides.col(1 + iEigen);
    ...
    for (size_t i = 0; i < _no; ++i) {
      for (size_t j = i; j < _no; ++j) {
        ...
        Tia.segment(i * _nv, _nv).noalias() += rij * leftEigenvector.segment(j * _nv, _nv);
        if (i != j) {
          _Zia.middleRows(j * _nv, _nv).noalias() += rij.transpose() * _Jia->middleRows(i * _nv, _nv);
          Tia.segment(j * _nv, _nv).noalias() += rij.transpose() * leftEigenvector.segment(i * _nv, _nv);
        }
      }
    }
    ...
    // 0 contribution
    rhs += Eab.transpose() * lev - lev * Eij.transpose();
    ...
    for (size_t i = 0; i < _no; ++i) {
      for (size_t j = i; j < _no; ++j) {
        ...
        Eigen::MatrixXd fij = this->getFockAmplitudes(i, j, 0);
        _Yia.middleRows(i * _nv, _nv).noalias() += lij * _Xia.middleRows(j * _nv, _nv);
        _Yia.middleRows(i * _nv, _nv).noalias() += fij * _Bia->middleRows(j * _nv, _nv);
        if (i != j) {
          _Yia.middleRows(j * _nv, _nv).noalias() += lij.transpose() * _Xia.middleRows(i * _nv, _nv);
    ...
    for (size_t i = 0; i < _no; ++i) {
      for (size_t j = i; j < _no; ++j) {
        Eigen::MatrixXd lij = this->getLeftAmplitudes(i, j, eigenvalue);
        _Yia.middleRows(i * _nv, _nv).noalias() += lij * _Bia->middleRows(j * _nv, _nv);
        if (i != j) {
          _Yia.middleRows(j * _nv, _nv).noalias() += lij.transpose() * _Bia->middleRows(i * _nv, _nv);
        }
      }
    }

    rightHandSide += 2 * _Jia->operator*(_Xia.transpose() * leftEigenvector);
    for (size_t Q = 0; Q < _nx; ++Q) {
      Eigen::Map<Eigen::MatrixXd> bij(_Bij->col(Q).data(), _no, _no);
      Eigen::Map<Eigen::MatrixXd> jia(_Jia->col(Q).data(), _nv, _no);
      Eigen::Map<Eigen::MatrixXd> yia(_Yia.col(Q).data(), _nv, _no);
      ...
      rhs.noalias() -= _bab.transpose() * lev * bij.transpose();

      yia.noalias() = lev * _bij.transpose();
    }
    rightHandSide -= this->getJ2GContribution(_Yia.data(), nullptr, nullVector, true);
...
} /* this->calculateExcitedStateLagrangeMultiplier() restricted */

template<>
Eigen::MatrixXd CC2Controller<UNRESTRICTED>::calculateExcitedStateLagrangeMultiplier(std::vector<Eigen::MatrixXd>& eigenvectors,
                                                                                     Eigen::VectorXd eigenvalues) {
  unsigned nEigen = _settings.ccexdens ? eigenvalues.size() : 0;
  Eigen::MatrixXd rightHandSides = Eigen::MatrixXd::Zero(_nDim, 1 + nEigen);
  rightHandSides.col(0) = this->calculateGroundStateLagrangeMultiplier();
  ...
  for (unsigned iEigen = 0; iEigen < nEigen; ++iEigen) {
    ...
    for (size_t i = 0; i < _no_a; ++i) {
      for (size_t j = i; j < _no_a; ++j) {
        ...
        Tia.segment(i * _nv_a, _nv_a).noalias() += rij * leftEigenvector.segment(j * _nv_a, _nv_a);
        if (i != j) {
          _Zia.alpha.middleRows(j * _nv_a, _nv_a).noalias() += rij.transpose() * _Jia->alpha.middleRows(i * _nv_a, _nv_a);
          Tia.segment(j * _nv_a, _nv_a).noalias() += rij.transpose() * leftEigenvector.segment(i * _nv_a, _nv_a);
        }
      }
    }
    for (size_t i = 0; i < _no_b; ++i) {
      for (size_t j = i; j < _no_b; ++j) {
        ...
        Tia.segment(i * _nv_b + _alpha, _nv_b).noalias() += rij * leftEigenvector.segment(j * _nv_b + _alpha, _nv_b);
        if (i != j) {
          _Zia.beta.middleRows(j * _nv_b, _nv_b).noalias() += rij.transpose() * _Jia->beta.middleRows(i * _nv_b, _nv_b);
          Tia.segment(j * _nv_b + _alpha, _nv_b).noalias() +=
              rij.transpose() * leftEigenvector.segment(i * _nv_b + _alpha, _nv_b);
        }
      }
    }
    for (size_t i = 0; i < _no_a; ++i) {
      for (size_t j = 0; j < _no_b; ++j) {
        ...
        Tia.segment(i * _nv_a, _nv_a).noalias() += rij * leftEigenvector.segment(j * _nv_b + _alpha, _nv_b);
        _Zia.beta.middleRows(j * _nv_b, _nv_b).noalias() += rij.transpose() * _Jia->alpha.middleRows(i * _nv_a, _nv_a);
        Tia.segment(j * _nv_b + _alpha, _nv_b).noalias() += rij.transpose() * leftEigenvector.segment(i * _nv_a, _nv_a);
      }
    }
    ...
    // 0 contribution
    rhs_a += Eab_a.transpose() * lev_a - lev_a * Eij_a.transpose();
    rhs_b += Eab_b.transpose() * lev_b - lev_b * Eij_b.transpose();
    ...
    for (size_t i = 0; i < _no_a; ++i) {
      for (size_t j = i; j < _no_a; ++j) {
        ...
        Eigen::MatrixXd fij = this->getFockAmplitudes(i, j, 0, 1);
        _Yia.alpha.middleRows(i * _nv_a, _nv_a).noalias() += lij * _Xia.alpha.middleRows(j * _nv_a, _nv_a);
        _Yia.alpha.middleRows(i * _nv_a, _nv_a).noalias() += fij * _Bia->alpha.middleRows(j * _nv_a, _nv_a);
        if (i != j) {
          _Yia.alpha.middleRows(j * _nv_a, _nv_a).noalias() += lij.transpose() * _Xia.alpha.middleRows(i * _nv_a, _nv_a);
    ...
    for (size_t i = 0; i < _no_b; ++i) {
      for (size_t j = i; j < _no_b; ++j) {
        Eigen::MatrixXd lij = this->getLeftAmplitudes(i, j, eigenvalue, -1);
        Eigen::MatrixXd fij = this->getFockAmplitudes(i, j, 0, -1);
        _Yia.beta.middleRows(i * _nv_b, _nv_b).noalias() += lij * _Xia.beta.middleRows(j * _nv_b, _nv_b);
        _Yia.beta.middleRows(i * _nv_b, _nv_b).noalias() += fij * _Bia->beta.middleRows(j * _nv_b, _nv_b);
        if (i != j) {
          _Yia.beta.middleRows(j * _nv_b, _nv_b).noalias() += lij.transpose() * _Xia.beta.middleRows(i * _nv_b, _nv_b);
    ...
    for (size_t i = 0; i < _no_a; ++i) {
      for (size_t j = 0; j < _no_b; ++j) {
        ...
        Eigen::MatrixXd fij = this->getFockAmplitudes(i, j, 0, 0);
        _Yia.alpha.middleRows(i * _nv_a, _nv_a).noalias() += lij * _Xia.beta.middleRows(j * _nv_b, _nv_b);
        _Yia.alpha.middleRows(i * _nv_a, _nv_a).noalias() += fij * _Bia->beta.middleRows(j * _nv_b, _nv_b);
        _Yia.beta.middleRows(j * _nv_b, _nv_b).noalias() += lij.transpose() * _Xia.alpha.middleRows(i * _nv_a, _nv_a);
    ...
    for (size_t i = 0; i < _no_a; ++i) {
      for (size_t j = i; j < _no_a; ++j) {
        Eigen::MatrixXd lij = this->getLeftAmplitudes(i, j, eigenvalue, 1);
        _Yia.alpha.middleRows(i * _nv_a, _nv_a).noalias() += lij * _Bia->alpha.middleRows(j * _nv_a, _nv_a);
        if (i != j) {
          _Yia.alpha.middleRows(j * _nv_a, _nv_a).noalias() += lij.transpose() * _Bia->alpha.middleRows(i * _nv_a, _nv_a);
        }
      }
    }
    for (size_t i = 0; i < _no_b; ++i) {
      for (size_t j = i; j < _no_b; ++j) {
        Eigen::MatrixXd tij = this->getLeftAmplitudes(i, j, eigenvalue, -1);
    ...
    for (size_t i = 0; i < _no_a; ++i) {
      for (size_t j = 0; j < _no_b; ++j) {
        Eigen::MatrixXd tij = this->getLeftAmplitudes(i, j, eigenvalue, 0);
    ...
    XQ = _Xia.alpha.transpose() * leftEigenvector.head(_alpha) + _Xia.beta.transpose() * leftEigenvector.tail(_beta);
    rightHandSide.head(_alpha) += _Jia->alpha * XQ;
    rightHandSide.tail(_beta) += _Jia->beta * XQ;
    for (size_t Q = 0; Q < _nx; ++Q) {
      Eigen::Map<Eigen::MatrixXd> bij_a(_Bij->alpha.col(Q).data(), _no_a, _no_a);
      ...
      rhs_a.noalias() -= _bab_a.transpose() * lev_a * bij_a.transpose();

      yia_a.noalias() = lev_a * _bij_a.transpose();
      ...
      rhs_b.noalias() -= _bab_b.transpose() * lev_b * bij_b.transpose();

      yia_b.noalias() = lev_b * _bij_b.transpose();
    }
    rightHandSide.head(_alpha) -= this->getJ2GContribution(_Yia.alpha.data(), nullptr, nullVector, true, 1);
    rightHandSide.tail(_beta) -= this->getJ2GContribution(_Yia.beta.data(), nullptr, nullVector, true, -1);
  ...
  return rightHandSides;
} /* this->calculateExcitedStateLagrangeMultiplier() unrestricted */
