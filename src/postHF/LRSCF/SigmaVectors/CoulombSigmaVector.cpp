...
CoulombSigmaVector<SCFMode>::CoulombSigmaVector(
...
template<> std::unique_ptr<std::vector<std::vector<MatrixInBasis<Options::SCF_MODES::RESTRICTED> > > >
CoulombSigmaVector<Options::SCF_MODES::RESTRICTED>::calcF(
  ...
  //Evaluation of the coulomb contribution with the RI approximation (based on CoulombPotential.h and CoulombInteractionPotential.h)
  if (this->_lrscf[I]->getSysSettings().dft.densityFitting == Options::DENS_FITS::RI){
    ...
    if (I == J){
      ...
      auto prescreeningFunc = [&](unsigned int i, 
                                  unsigned int j, 
                                  unsigned int nI, 
                                  unsigned int nJ, 
                                  double schwartz) {
        (void) i;(void) j;(void) nI;(void) nJ;
    ...
    auto const loopEvalFunction = [&]( const unsigned int& i,
    ...
    auto const loopEvalFunction2 = [&](const unsigned int& i,
                                      const unsigned int& j,
                                      const unsigned int& K,
                                      const double& integral,
                                      const unsigned int threadId){
    ...
    auto ri_j_IntController = RI_J_IntegralControllerFactory::getInstance().produce(superSystemBasisController,superSystemAuxBasisController);
    ...
    auto const loopEvalFunction1 = [&]( const unsigned int& i,
                                        const unsigned int& j,
                                        const unsigned int& K,
  ...
  std::vector<std::vector<std::vector<MatrixInBasis<Options::SCF_MODES::RESTRICTED> > > *> f;
    ...
    auto const loopEvalFunction3 = [&](const unsigned int& i,
                                        const unsigned int& j,
                                        const unsigned int& K,
                                        Eigen::VectorXd& integral,
                                        const unsigned int threadID){
    ...
    TwoElecThreeCenterIntLooper looper4(libint2::Operator::coulomb,
        ...
        auxBasisControllerJ,
        1E-10);
    auto const loopEvalFunction4 = [&](const unsigned int& i,
                                        const unsigned int& j,
                                        const unsigned int& K,
                                        Eigen::VectorXd& integral,
                                        const unsigned int threadID){
  ...
  //If no RI is used:
    ...
    if (I==J) {
      ...
      looper.loop(distribute_II,prescreeningFunc);
    } else if (I != J) {
      CoulombInteractionIntLooper looper(
          libint2::Operator::coulomb,
          0,
          this->_lrscf[I]->getBasisController(),
          this->_lrscf[J]->getBasisController(),
          prescreeningThreshold);
      looper.loop(distribute_IJ,prescreeningFunc);
    } else {
      assert(false);
    }
  #ifdef _OPENMP
...
template<>
std::unique_ptr<std::vector<std::vector<MatrixInBasis<Options::SCF_MODES::UNRESTRICTED> > > > CoulombSigmaVector<Options::SCF_MODES::UNRESTRICTED>::calcF(
    unsigned int I,
    unsigned int J,
