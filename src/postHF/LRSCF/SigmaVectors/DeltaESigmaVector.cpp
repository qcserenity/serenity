...
DeltaESigmaVector<SCFMode>::DeltaESigmaVector(
...
template<Options::SCF_MODES SCFMode>
std::unique_ptr<std::vector<std::vector<MatrixInBasis<SCFMode> > > > DeltaESigmaVector<SCFMode>::calcF(
    unsigned int I,
    unsigned int J,
  ...
  if(I == J){
    ...
    return unique_ptr<std::vector<std::vector<MatrixInBasis<SCFMode> > > >(
...
template<Options::SCF_MODES SCFMode>
void DeltaESigmaVector<SCFMode>::addToSigma(
    unsigned int I,
    ...
    //Build sigma vector for canonical orbitals
      ...
      for (unsigned int iSet = 0; iSet < this->_nSet; ++iSet) {
        ...
        this->_sigma[iSet].middleRows(iStartI+iStartSpin,nOccupied_spin*nVirtual_spin) +=
              orbitalEnergyDifferences_spin.asDiagonal() * this->_b[iSet][I].middleRows(iStartSpin,nOccupied_spin*nVirtual_spin);
          iStartSpin += nOccupied_spin*nVirtual_spin;
        };
      }
      F_IJ.reset();
