...
EOSigmaVector<SCFMode>::EOSigmaVector(
...
template<Options::SCF_MODES SCFMode>
std::unique_ptr<std::vector<std::vector<MatrixInBasis<SCFMode> > > > EOSigmaVector<SCFMode>::calcF(
  unsigned int I,
  unsigned int J,
  ...
  if(_eoPot == Options::KIN_EMBEDDING_MODES::NADD_FUNC ||
     _eoPot == Options::KIN_EMBEDDING_MODES::RECONSTRUCTION ||
     _eoPot == Options::KIN_EMBEDDING_MODES::NONE ||
     //For readCoupledExcitation:
     this->_lrscf[J]->getBasisController() == this->_lrscf[I]->getBasisController()) return fock;

  //Calculate overlap Matrix between the different subsystems:
  auto& libint = Libint::getInstance();
  Eigen::MatrixXd SAB = libint.compute1eInts(libint2::Operator::overlap,
  ...
  if(_eoPot == Options::KIN_EMBEDDING_MODES::LEVELSHIFT){
    ...
    auto abPotential = ABEmbeddedBundleFactory<SCFMode>::produce(
          this->_lrscf[I]->getSys(),
          this->_lrscf[J]->getBasisController(),
          this->_lrscf[J]->getSys()->getGeometry(),
          {this->_lrscf[J]->getSys()},
          std::make_shared<EmbeddingSettings>(this->_lrscf[I]->getLRSCFSettings().embedding),
          true);
    SPMatrix<SCFMode> F_AB = abPotential->getABMatrix();
    for_spin(F_AB) {F_AB_spin = F_AB_spin * 0.5;};
    //Calculate sigma vectors for each guess vector with pseudo density matrix
    for(unsigned int iSet = 0; iSet < this->_nSet; ++iSet){
      for(unsigned int iGuess = 0; iGuess < this-> _nGuess; ++iGuess){
        auto& f = (*fock)[iSet][iGuess];
