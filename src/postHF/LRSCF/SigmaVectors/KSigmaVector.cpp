...
KSigmaVector<SCFMode>::KSigmaVector(std::vector<std::shared_ptr<LRSCFController<SCFMode>>> lrscf,
                                    std::vector<Eigen::MatrixXd> b, const double densityScreeningThreshold,
                                    const std::vector<int> pm)
  : SigmaVector<SCFMode>(lrscf, b, densityScreeningThreshold), _pm(pm) {
}

template<Options::SCF_MODES SCFMode>
KSigmaVector<SCFMode>::KSigmaVector(std::vector<std::shared_ptr<LRSCFController<SCFMode>>> lrscf,
                                    const double densityScreeningThreshold)
  : SigmaVector<SCFMode>(lrscf, densityScreeningThreshold), _pm({0}){};

template<>
std::unique_ptr<std::vector<std::vector<MatrixInBasis<Options::SCF_MODES::RESTRICTED>>>>
KSigmaVector<Options::SCF_MODES::RESTRICTED>::calcF(
  ...
  if (I == J && this->_lrscf[I]->getSysSettings().method == Options::ELECTRONIC_STRUCTURE_THEORIES::DFT) {
    auto funcI = resolveFunctional(this->_lrscf[I]->getSysSettings().dft.functional);
    ...
    hfExchangeRatio = funcNadd.getHfExchangeRatio();
    lrExchangeRatio = funcNadd.getLRExchangeRatio();
    mu = funcNadd.getRangeSeparationParameter();
  }
  ...
  auto distributeExchangeII = [&](unsigned int i, unsigned int j, unsigned int k, unsigned int l, const double integral,
    ...
    const double exc = perm * integral * exchangeRatio;
    for (unsigned int iSet = 0; iSet < this->_nSet; ++iSet) {
  ...
  // Function to calculate K pseudo-Fock matrix (for I!=J)
  auto distributeExchangeIJ = [&](unsigned int i, unsigned int j, unsigned int k, unsigned int l, const double integral,
                                  unsigned int threadId) {
  ...
  if (hfExchangeRatio != 0.0) {
    ...
    if (I == J) {
      ...
      looper.loopNoDerivative(distributeExchangeII, prescreeningFunc);
    }
    else if (I != J) {
      ExchangeInteractionIntLooper looper(libint2::Operator::coulomb, 0, this->_lrscf[I]->getBasisController(),
                                          this->_lrscf[J]->getBasisController(), prescreeningThreshold);
      looper.loopNoDerivative(distributeExchangeIJ, prescreeningFunc);
    }
    else {
      assert(false);
    }
  ...
  if (lrExchangeRatio != 0.0) {
    ...
    if (I == J) {
      TwoElecFourCenterIntLooper looper(libint2::Operator::erf_coulomb, 0, this->_lrscf[I]->getBasisController(),
                                        prescreeningThreshold, mu);
      looper.loopNoDerivative(distributeExchangeII, prescreeningFunc);
    }
    else if (I != J) {
      ExchangeInteractionIntLooper looper(libint2::Operator::erf_coulomb, 0, this->_lrscf[I]->getBasisController(),
                                          this->_lrscf[J]->getBasisController(), prescreeningThreshold, mu);
      looper.loopNoDerivative(distributeExchangeIJ, prescreeningFunc);
    }
    else {
      assert(false);
...
KSigmaVector<Options::SCF_MODES::UNRESTRICTED>::calcF(
    unsigned int I, unsigned int J,
  ...
  if (I == J && this->_lrscf[I]->getSysSettings().method == Options::ELECTRONIC_STRUCTURE_THEORIES::DFT) {
    ...
    auto funcI = resolveFunctional(this->_lrscf[I]->getLRSCFSettings().embedding.naddXCFunc);
    hfExchangeRatio = funcI.getHfExchangeRatio();
    lrExchangeRatio = funcI.getLRExchangeRatio();
    mu = funcI.getRangeSeparationParameter();
  }
