...
class KSigmaVector : public SigmaVector<SCFMode> {
 public:
  /**
   * @brief Constructor
   ...
   */
  KSigmaVector(std::vector<std::shared_ptr<LRSCFController<SCFMode>>> lrscf, std::vector<Eigen::MatrixXd> b,
               const double densityScreeningThreshold, const std::vector<int> pm);
   ...
   * @param densityScreeningThreshold The density matrix prescreening threshold.
   */
  KSigmaVector(std::vector<std::shared_ptr<LRSCFController<SCFMode>>> lrscf, const double densityScreeningThreshold);
  /**
