...
SigmaVector<SCFMode>::SigmaVector(
  ...
  for (unsigned int iSet = 0; iSet < _nSet; ++iSet) {
    for (unsigned int iGuess = 0; iGuess < _nGuess; ++iGuess) {
      ...
      for_spin(P,coeff,nOccupied,nVirtual) {
        ...
        P_spin = coeff_spin.leftCols(nOccupied_spin) * guess * coeff_spin.middleCols(nOccupied_spin,nVirtual_spin).transpose();
...
template<Options::SCF_MODES SCFMode>
void SigmaVector<SCFMode>::addToSigma(
    unsigned int I,
    unsigned int iStartI,
  ...
  for (unsigned int iSet = 0; iSet < _nSet; ++iSet) {
    for (unsigned int iGuess = 0; iGuess < _nGuess; ++iGuess ) {
      ...
      for_spin(pF,coeff,nOccupied,nVirtual) {
        pF_spin = coeff_spin.leftCols(nOccupied_spin).transpose() * pF_spin * coeff_spin.middleCols(nOccupied_spin,nVirtual_spin);
...
void SigmaVector<SCFMode>::calcSigma() {
  ...
  for (unsigned int I = 0; I < _nSub; ++ I) {
    ...
    for (unsigned int J = 0; J < _nSub; ++J) {
      ...
      if (maxDens < _densityScreeningThreshold) continue;
