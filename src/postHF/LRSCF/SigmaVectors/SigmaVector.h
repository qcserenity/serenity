 ...
 */
template<Options::SCF_MODES SCFMode> class SigmaVector {
public:
  /**
   * @brief Constructor
  ...
  */
  std::vector<Eigen::MatrixXd>& getSigma() {
    if (!_hasBeenCalculated) this->calcSigma();
    return _sigma;
  }
...
protected:
  ...
  */
  ...
  virtual std::unique_ptr<std::vector<std::vector<MatrixInBasis<SCFMode> > > > calcF(
    unsigned int I,
    unsigned int J,
    std::unique_ptr<std::vector<std::vector<MatrixInBasis<SCFMode> > > > P_J) = 0;

  ///@brief LRSCFController for each reference subsystem.
  std::vector<std::shared_ptr<LRSCFController<SCFMode> > > _lrscf;

  ///@brief A prescreening threshold.
  const double _densityScreeningThreshold;
