...
TEST_F(SigmaVectorTest,DJK_res) {
  ...
  auto systemController =
      SystemController__TEST_SUPPLY::getSystemController(TEST_SYSTEM_CONTROLLERS::H2_DEF2_TZVP_CAMB3LYP);
  ...
  TwoElecFourCenterIntLooper lrlooper(libint2::Operator::erf_coulomb,0,basisController,1E-10,mu);
  auto const storeLRERIS = [&lreris]
                          (const unsigned int&  a,
                              const unsigned int&  b,
                              const unsigned int&  i,
                              const unsigned int&  j,
    ...
    for (unsigned int a = nOcc; a < nOcc + nVirt; ++a, ++ia) {
      for (unsigned int j = 0, jb = 0; j < nOcc; ++j) {
        for (unsigned int b = nOcc; b < nOcc + nVirt; ++b, ++jb) {
          ...
          if (i == j && a == b) D(ia,jb) += orbitalEnergies(a) - orbitalEnergies(i);
  ...
  for (unsigned int iSet = 0; iSet < guess.size(); ++iSet) {
    ...
    J_correct[iSet].block(0,0,nDimension,guess[iSet].cols())+=J*guess[iSet].block(0,0,nDimension,guess[iSet].cols());
    J_correct[iSet].block(0,0,nDimension,guess[iSet].cols())+=J*guess[iSet].block(nDimension,0,nDimension,guess[iSet].cols());
    //Sigma vectors of both subsystems are equal...
    J_correct[iSet].block(nDimension,0,nDimension,guess[iSet].cols())=J_correct[iSet].block(0,0,nDimension,guess[iSet].cols());

    D_correct[iSet].resize(2*nDimension,guess[iSet].cols());
    D_correct[iSet].setZero();
    D_correct[iSet].block(0,0,nDimension,guess[iSet].cols())+=D*guess[iSet].block(0,0,nDimension,guess[iSet].cols());
    D_correct[iSet].block(nDimension,0,nDimension,guess[iSet].cols())=D*guess[iSet].block(nDimension,0,nDimension,guess[iSet].cols());

    KP_correct[iSet].resize(2*nDimension,guess[iSet].cols());
    KP_correct[iSet].setZero();
    KP_correct[iSet].block(0,0,nDimension,guess[iSet].cols())+=KP*guess[iSet].block(0,0,nDimension,guess[iSet].cols());
    KP_correct[iSet].block(0,0,nDimension,guess[iSet].cols())+=KP*guess[iSet].block(nDimension,0,nDimension,guess[iSet].cols());
    KP_correct[iSet].block(nDimension,0,nDimension,guess[iSet].cols())=KP_correct[iSet].block(0,0,nDimension,guess[iSet].cols());
  }

  //Test Coulomb contribution
  CoulombSigmaVector<Options::SCF_MODES::RESTRICTED> JSigmaVector({lrscfController,lrscfController},guess,0.0);
  std::vector<Eigen::MatrixXd> J_test = JSigmaVector.getSigma();
  //Test orbital energy difference contribution
  DeltaESigmaVector<Options::SCF_MODES::RESTRICTED> DSigmaVector({lrscfController,lrscfController},guess,0.0);
  std::vector<Eigen::MatrixXd> D_test = DSigmaVector.getSigma();
  //Test exchange contributions
  KSigmaVector<Options::SCF_MODES::RESTRICTED> KPSigmaVector({lrscfController,lrscfController},guess,0.0,{1,1});
