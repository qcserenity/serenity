...
namespace Serenity {
  ...
  Besley<SCFMode>::Besley(std::shared_ptr<SystemController> system,
                          unsigned int nAtoms,
                          std::vector<double> besleyCutoff) :
      _system(system),
      _nAtoms(nAtoms),
      _besleyCutoff(besleyCutoff){
      
      _nAtoms = _system->getNAtoms() < _nAtoms ? _system->getNAtoms() : _nAtoms;
      if(_nAtoms == _system->getNAtoms()) printf("ATTENTION: Using all atoms for Besley restriction!");
  }
  ...
  SpinPolarizedData<SCFMode,std::vector<unsigned int> > Besley<SCFMode>::getWhiteList(){
    ...
    for_spin(nOccupied,mulliken,whiteList){
      for (unsigned int iMo = 0; iMo < nOccupied_spin; ++iMo){
        ...
        if (value > _besleyCutoff[0]) whiteList_spin.push_back(iMo);
      }
    };/* Occupied */
    ...
    for_spin(nOccupied,coefficients,whiteList){
      for(unsigned int iMo = nOccupied_spin; iMo < coefficients_spin.cols(); ++iMo){
        ...
        if (value/norm > _besleyCutoff[1]) whiteList_spin.push_back(iMo);
      }
    };/* Virtual */
