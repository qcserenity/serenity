...
void CouplingConstruction<SCFMode>::solve(std::vector<std::shared_ptr<LRSCFController<SCFMode>>> lrscf,
  ...
  if (settings.method == Options::LR_METHOD::TDA) {
    ...
    if (settings.saveResponseMatrix) {
      std::ofstream file("ResponseMatrix_1.txt");
    ...
    if (settings.transitionCharges) {
      printf("  I've found the keyword transisitonCharges to be true. Please use the TDA!\n\n");
    }
    ...
    // Normalize and obtain X and Y from (X+Y) and (X-Y).
    for (unsigned iEigen = 0; iEigen < nEigen; ++iEigen) {
      double clr = (*eigenvectors)[1].col(iEigen).dot((*eigenvectors)[0].col(iEigen));
      (*eigenvectors)[0].col(iEigen) *= 0.5 / std::sqrt(clr);
      (*eigenvectors)[1].col(iEigen) *= 0.5 / std::sqrt(clr);
    }

    Eigen::MatrixXd xpy = (*eigenvectors)[0];
    Eigen::MatrixXd xmy = (*eigenvectors)[1];
    (*eigenvectors)[0] = xpy + xmy;
    (*eigenvectors)[1] = xpy - xmy;
  }
}
