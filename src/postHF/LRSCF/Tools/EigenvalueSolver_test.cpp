...
TEST_F(EigenvalueSolverTest,TDDFT) {
  //Construct test matrices A+B and A-B
  ...
  auto sigmaVectorCalculator = [&] (std::vector<Eigen::MatrixXd>&  guessVectors) {
    auto sigma = std::unique_ptr<std::vector<Eigen::MatrixXd> >(new std::vector<Eigen::MatrixXd>(1));
    (*sigma)[0] = diagonalElements.cwiseSqrt().asDiagonal() * APB * diagonalElements.cwiseSqrt().asDiagonal() * guessVectors[0];
    return sigma;
  };
  ...
  EigenvalueSolver solver(
      ...
      1000,
      nEigen,
      Options::RESPONSE_PROBLEM::TDDFT,
      sigmaVectorCalculator,
      nullptr);
  Eigen::VectorXd itEigenvalues = solver.getEigenvalues();

  //Solve eigenvalue problem using Eigen
  Eigen::MatrixXd matrix = diagonalElements.cwiseSqrt().asDiagonal() * APB * diagonalElements.cwiseSqrt().asDiagonal();
  Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> eigenvalueSolver(matrix);
  Eigen::VectorXd exEigenvalues = eigenvalueSolver.eigenvalues().cwiseSqrt();
...
TEST_F(EigenvalueSolverTest,RPA) {
  ...
  for(unsigned int i = 0; i < 2 * nDimension; ++i){
    if (exEigenvalues(i) > 0) evPair.push_back(std::make_pair(exEigenvalues(i),i));
  }
...
TEST_F(EigenvalueSolverTest,RPAInitialGuess) {
  ...
  //Transform X and Y to (X+Y) and (X-Y)
  Eigen::MatrixXd X = (*guessPtr)[0];
  Eigen::MatrixXd Y = (*guessPtr)[1];
  (*guessPtr)[0] = (X + Y) / std::sqrt(2);
