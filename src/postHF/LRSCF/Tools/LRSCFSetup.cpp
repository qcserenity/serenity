...
void LRSCFSetup<SCFMode>::printInfo(
  ...
  for(unsigned int I = 0; I < lrscf.size(); ++I){
    printf("%5i ",I+1);
    auto nOccupied = lrscf[I]->getNOccupied();
    auto nVirtual = lrscf[I]->getNVirtual();
    printf("%20s ",lrscf[I]->getSysSettings().name.c_str());
    printf("%5s ","act");
    for_spin(nOccupied) {printf("%8i ", nOccupied_spin);};
    for_spin(nVirtual) {printf("%8i ", nVirtual_spin);};
  ...
  for(unsigned int I = 0; I < envSys.size(); ++I){
    ...
    printf("%20s ",envSys[I]->getSettings().name.c_str());
    printf("%5s ","env");
    for_spin(nOccupied) {printf("%8i ", nOccupied_spin);};
    for_spin(nVirtual) {printf("%8i ", nVirtual_spin);};
    unsigned int nDimI = 0;
  ...
  printf("\n Type        : %22s \n",(type == Options::LRSCF_TYPE::ISOLATED) 
    ? "Isolated" : (type ==Options::LRSCF_TYPE::UNCOUPLED) ? "FDEu" : "FDEc");
  ...
  Options::resolve<Options::XCFUNCTIONALS>(naddXCFunc_string,naddxc);
  Options::resolve<Options::KINFUNCTIONALS>(naddKinFunc_string,naddkin);
  Options::resolve<Options::XCFUNCTIONALS>(func,xc);
  printf(" NaddKinFunc : %22s \n", naddKinFunc_string.c_str());
  printf(" NaddXCFunc  : %22s \n\n", naddXCFunc_string.c_str());
  if (settings.func != Options::XCFUNCTIONALS::NONE) printf(" Func        : %22s \n\n", func.c_str()); 
}

template<Options::SCF_MODES SCFMode>
Point LRSCFSetup<SCFMode>::getGaugeOrigin(
      const LRSCFTaskSettings& settings,
  ...
  Point gaugeOrigin(
    settings.gaugeOrigin[0] / BOHR_TO_ANGSTROM,
    settings.gaugeOrigin[1] / BOHR_TO_ANGSTROM,
  ...
  if(std::find(settings.gaugeOrigin.begin(),settings.gaugeOrigin.end(),std::numeric_limits<double>::infinity()) != settings.gaugeOrigin.end()){
    printf("  Set gauge-origin to center of mass.\n");
    Geometry supergeo;
  ...
  printf("  Gauge-origin (Angstrom): %8.4f %8.4f %8.4f\n\n",
    ...
    gaugeOrigin.getY() * BOHR_TO_ANGSTROM,
    gaugeOrigin.getZ() * BOHR_TO_ANGSTROM);
...
LRSCFSetup<SCFMode>::setupFDEcTransformation(
      const LRSCFTaskSettings& settings,
    ...
    for (unsigned int I = 0; I < referenceLoadingType.size(); ++I) {
      ...
      if (settings.uncoupledSubspace.empty()) {
        ...
        (*eigenvectors)[0].block(iCountRows - (*vecI)[0].rows(),iCountCols - (*vecI)[0].cols(),
            (*vecI)[0].rows(),(*vecI)[0].cols()) = (*vecI)[0] + (*vecI)[1];
        if (!(settings.tda)) {
          (*eigenvectors)[1].block(
              iCountRows - (*vecI)[1].rows(),iCountCols - (*vecI)[1].cols(),
              (*vecI)[1].rows(),(*vecI)[1].cols()) =(*vecI)[0] - (*vecI)[1];;
        }
        ...
        for (unsigned int iState = 0; iState < nStates; ++iState) {
          (*eigenvectors)[0].block(
              iCountRows - (*vecI)[0].rows(),
              iCountCols,
              (*vecI)[0].rows(),
              1)  = (*vecI)[0].col(settings.uncoupledSubspace[iStart+iState]-1) 
                    + (*vecI)[1].col(settings.uncoupledSubspace[iStart+iState]-1);
          if (!(settings.tda)) {
            (*eigenvectors)[1].block(
                iCountRows - (*vecI)[1].rows(),
                iCountCols,
                (*vecI)[1].rows(),
                1)  = (*vecI)[0].col(settings.uncoupledSubspace[iStart+iState]-1) 
                      - (*vecI)[1].col(settings.uncoupledSubspace[iStart+iState]-1);
...
template<Options::SCF_MODES SCFMode>
void LRSCFSetup<SCFMode>::setupLRSCFController(
      const LRSCFTaskSettings& settings,
      const Eigen::MatrixXi& couplingPatternMatrix,
      const std::vector<std::shared_ptr<SystemController> >& act,
      const std::vector<std::shared_ptr<SystemController> >& env,
  ...
  for (auto lrscf : lrscfAll){
    ...
    if (type == Options::LRSCF_TYPE::COUPLED) {
      ...
      try {
        ...
        //If the lrscfController is part of an coupled uncoupled calculation the coupled vectors and uncoupled vectors are read in
        if((unsigned int) couplingPatternMatrix.rows() > act.size() && iSystem < act.size()) {
          type_str = ".fdec";
        }else{
          type_str = "";
        }
        std::string fName = lrscf->getSys()->getSettings().path + lrscf->getSys()->getSettings().name + type_str + ".lrscfSpace." + mode + ".h5";
    ...
    if(settings.excludeProjection && type != Options::LRSCF_TYPE::ISOLATED){
      ...
      for(unsigned int iSysAct = 0; iSysAct < lrscfAll.size() + env.size(); iSysAct++){
        ...
        if(iSysAct != iSystem && iSysAct < lrscfAll.size()) envSys = lrscfAll[iSysAct]->getSys();
        if(iSysAct >= lrscfAll.size()) envSys = env[iSysAct - lrscfAll.size()];
        if(envSys == nullptr){
          //DO nothing
        }else{
          ...
          auto overlapAB = libint.compute1eInts(libint2::Operator::overlap,
                                                lrscf->getBasisController(),
                                                envSys->getBasisController());
          ...
          for_spin(nOccEnv, nVirtual, nOccupied, coefEnv, coefAct){
            ...
            overlapMO.push_back(coefEnv_spin.block(0,0,nBasisEnv,nOccEnv_spin).transpose()*
                                 overlapAB*coefAct_spin.block(0,nOccupied_spin,nBasisAct,nVirtual_spin));
          };
          //Find orbitals of env system through overlap criteria
          unsigned int spinCounter = 0;
          for_spin(indexWhiteList,nOccupied){
            auto oldIndexWhiteList = indexWhiteList_spin;
            for (unsigned int col = 0; col < overlapMO[spinCounter].cols(); col++){
              ...
              if(sum > 0.99){
                indexWhiteList_spin.erase(
                  std::remove(indexWhiteList_spin.begin(), indexWhiteList_spin.end(),
                  oldIndexWhiteList[col + nOccupied_spin]), indexWhiteList_spin.end());
              }
            }
            spinCounter += 1;
          };
