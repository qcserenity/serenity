...
class ResponseLambda {
 public:
  ...
  /**
   * @brief Returns the symmetrized TDDFT sigma vector calculator.
   * @return The symmetrized TDDFT sigma vector calculator.
   */
  SigmaCalculator getTDDFTSigma();
   ...
   */
  XWFSigmaCalculator getRightXWFSigma();
 ...
 private:
  ...
  ///@brief SCF factor (2 for restricted, 1 for unrestricted).
  ...
  std::unique_ptr<Sigmavector<SCFMode>> D, J, K, DFK, F, EO;

  ///@brief Simplified TDDFT sigma vector.
  std::unique_ptr<GrimmeSigmavector<SCFMode>> _grimme;
