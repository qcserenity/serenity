...
SPMatrix<SCFMode>& ABCoulombInteractionPotential<SCFMode>::getMatrix(){
  if(!_abPotential) {
    _abPotential.reset(
        new SPMatrix<SCFMode>(this->_basisA->getNBasisFunctions(),this->_basisB->getNBasisFunctions()));
...
void ABCoulombInteractionPotential<SCFMode>::calculateFockMatrixNoFitting() {
  ...
  for (const auto& densityC : _envDMatController) {
    ...
    if(!_shellPairsAB) _shellPairsAB = ABShellPairCalculator::calculateShellPairData_AB(this->_basisA,this->_basisB);
    ...
    std::vector<SPMatrix<Options::SCF_MODES::RESTRICTED> > eriContr(omp_get_max_threads(),SPMatrix<Options::SCF_MODES::RESTRICTED> (
        this->_basisA->getNBasisFunctions(),this->_basisB->getNBasisFunctions()));
#else
    std::vector<Eigen::MatrixXd> ints(1);
    std::vector<SPMatrix<Options::SCF_MODES::RESTRICTED> > eriContr(1,SPMatrix<Options::SCF_MODES::RESTRICTED> (
        this->_basisA->getNBasisFunctions(),this->_basisB->getNBasisFunctions()));
    eriContr[0].setZero();
#endif
    // loops over shells
#pragma omp parallel for schedule(static, 1)
    for (int pIndex=_shellPairsAB->size()-1; pIndex >= 0; --pIndex){
      ...
      for (const auto& q : *shellPairsC) {
        ...
        if (firstC2>firstC1) continue;

        // calculate integrals
        if (_libint->compute(op, 0, basA, basB, basC1, basC2, ints[threadId])){
              ...
              for (unsigned int C1=0;C1 <basisC[c1]->getNContracted();++C1){
                ...
                for (unsigned int C2=0;C2 <basisC[c2]->getNContracted();++C2,++counter){
                  ...
                  Eigen::VectorXd set(ints[threadId].row(counter));
                    eriContr[threadId](aa,bb) += pre * totDensC(cc1,cc2) * set(0);
...
void ABCoulombInteractionPotential<SCFMode>::calculateFockMatrixRI() {
  ...
  for (unsigned int iC = 0; iC <_envDMatController.size(); ++iC) {
    ...
    auto nCAuxBasisFunctions = auxBasisControllerC->getNBasisFunctions();
    ...
    auto prescreeningFunc = [&](
      ...
      (void)nI;
      (void)nJ;
      if (fabs(maxDens*schwartz) < _actSystem->getSettings().basis.integralThreshold) return true;
      return false;
    };
        ...
        std::vector<SPMatrix<RESTRICTED> > eriContr(omp_get_max_threads(),SPMatrix<RESTRICTED> (
            this->_basisA->getNBasisFunctions(),this->_basisB->getNBasisFunctions()));
#else
        // or just one
        Eigen::MatrixXd sumMat(nABAuxBasFunc,1);
        std::vector<SPMatrix<RESTRICTED> > eriContr(1,SPMatrix<RESTRICTED> (
            this->_basisA->getNBasisFunctions(),this->_basisB->getNBasisFunctions()));
        eriContr[0].setZero();
#endif
        sumMat.setZero();
...
#ifdef _OPENMP
        Eigen::MatrixXd sumMatC(nCAuxBasisFunctions,omp_get_max_threads());
#else
        Eigen::MatrixXd sumMatC(nCAuxBasisFunctions,1);
#endif
        sumMatC.setZero();
        if(this->_basisA != basisControllerC) {
          // Build A+B+C aux-basis in order to calculate the inverse two center matrix.
          Basis superABCBasis;
          Basis superABCAuxBasis;
          superABCBasis.reserve(_superABBasis->getBasis().size()+ basisControllerC->getBasis().size());
          superABCAuxBasis.reserve(_auxBasisControllerAB->getBasis().size()+ auxBasisControllerC->getBasis().size());
          superABCBasis.insert(superABCBasis.end(), _superABBasis->getBasis().begin(),_superABBasis->getBasis().end());
          superABCBasis.insert(superABCBasis.end(), basisControllerC->getBasis().begin(),basisControllerC->getBasis().end());
          superABCAuxBasis.insert(superABCAuxBasis.end(),_auxBasisControllerAB->getBasis().begin(), _auxBasisControllerAB->getBasis().end());
          superABCAuxBasis.insert(superABCAuxBasis.end(),auxBasisControllerC->getBasis().begin(), auxBasisControllerC->getBasis().end());
          auto superABCBasisController = std::shared_ptr<BasisController>(new CustomBasisController(superABCBasis,"tmpSuperABCBasis"));
          auto superABCAuxBasisController = std::shared_ptr<BasisController>(new CustomBasisController(superABCAuxBasis,"tmpSuperABCAuxBasis"));
          // Calculate inverse two center matrix.
          auto ri_j_IntController = RI_J_IntegralControllerFactory::getInstance()
          .produce(superABCBasisController,superABCAuxBasisController);
          const auto inverseM = ri_j_IntController->getInverseM();
          /*
           * TODO: MB: I tried to use the .data() function provided by Eigen. But it appears that I did something wrong
           *           and the results were not as good as I liked. Therefore, a lot of lines are commented out in the
           *           following part of the code.
           */
//          auto sumptr = sumMat.data();
//          auto sumptrC=sumMatC.data();
//          auto densptr = densityC.total().data();
//          auto nBFs_C = densityC.cols();
          TwoElecThreeCenterIntLooper looper1(libint2::Operator::coulomb,
              0,
              basisControllerC,
              _auxBasisControllerAB,
              1E-10);
...
//            sumptr[K+nABAuxBasFunc*threadId] +=  (i == j ? 1.0 : 2.0) * integral * densptr[i*nBFs_C+j];
            sumMat(K,threadId) +=  (i == j ? 1.0 : 2.0) * integral[0] * densityC(i,j);
          };
          looper1.loop(loopEvalFunction1,prescreeningFunc);
//          _ri_j_IntegralController_C_AB[iC]->loopOver3CInts(loopEvalFunction1,prescreeningFunc);
          // Most likely already build.
          auto ri_j_IntegralControllerC =  RI_J_IntegralControllerFactory::getInstance()
          .produce(basisControllerC,auxBasisControllerC);
          auto const loopEvalFunction2 = [&]
                                          (const unsigned int& i,
                                              ...
                                              const unsigned int threadId){
//            sumptrC[K+nAuxBasisC*threadId] +=  (i == j ? 1.0 : 2.0) * integral * densptr[i*nBFs_C+j];
            sumMatC(K,threadId) +=  (i == j ? 1.0 : 2.0) * integral * densityC(i,j);
          };
          ri_j_IntegralControllerC->loopOver3CInts(loopEvalFunction2,prescreeningFunc);
          Eigen::VectorXd sumPMuNu_DMuNuAB = sumMat.rowwise().sum();
          sumMat.resize(1,1);
          Eigen::VectorXd sumPMuNu_DMuNuC = sumMatC.rowwise().sum();
          sumMatC.resize(1,1);
          Eigen::VectorXd sumPMuNu_DMuNu(sumPMuNu_DMuNuAB.rows()+sumPMuNu_DMuNuC.rows());
...
//          unsigned int nBFs_A = this->_basisA->getNBasisFunctions();
          ABTwoElecThreeCenterIntLooper looper3(libint2::Operator::coulomb,
              0,
              this->_basisA,
              this->_basisB,
              _auxBasisControllerAB,
          ...
          auto const loopEvalFunction3 = [&]
                                          (const unsigned int& i,
                                              ...
                                              Eigen::VectorXd& integral,
                                              const unsigned int threadID){
//            eriContr[threadID].data()[i*nBFs_A+j] += integral[0] * coeffptr[K];
            eriContr[threadID](i,j) += integral[0] * coeffptr[K];
          };
          looper3.loop(loopEvalFunction3);
//          unsigned int nBFs_B = this->_basisB->getNBasisFunctions();
          ABTwoElecThreeCenterIntLooper looper4(libint2::Operator::coulomb,
              0,
              this->_basisA,
              this->_basisB,
              auxBasisControllerC,
          ...
          auto const loopEvalFunction4 = [&]
                                          (const unsigned int& i,
                                              ...
                                              Eigen::VectorXd& integral,
                                              const unsigned int threadID){
//            eriContr[threadID].data()[i*nBFs_A+j] += integral[0] * coeffptr[K+nABAuxBasFunc];
          ...
          if(!_ri_j_IntegralController_A_AB) {
            _ri_j_IntegralController_A_AB = RI_J_IntegralControllerFactory::getInstance().produce(
                this->_basisA,
                _auxBasisControllerAB);
          }
          const auto inverseM = _ri_j_IntegralController_A_AB->getInverseM();
//          auto sumptr = sumMat.data();
//          auto densptr = densityC.data();
//          auto nBFs_C = densityC.cols();
          auto const loopEvalFunction1 = [&]
                                          (const unsigned int& i,
                                              const unsigned int& j,
...
//            sumptr[K+nABAuxBasFunc*threadId] +=  (i == j ? 1.0 : 2.0) * integral * densptr[i*nBFs_C+j];
          ...
          if (!_ri_j_IntegralController_A_B_AB) {
            _ri_j_IntegralController_A_B_AB = RI_J_IntegralControllerFactory::getInstance().produce(
                this->_basisA,
                _auxBasisControllerAB,
                this->_basisB);
          }
