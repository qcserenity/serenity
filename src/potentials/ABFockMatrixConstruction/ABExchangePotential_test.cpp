...
protected:
  ABExchangePotentialTest(): systemControllerA(
...
//  auto basisB = systemControllerA->getBasisController();
  std::vector<std::shared_ptr<DensityMatrixController<SPIN> > > envDmat(1,
      systemControllerA->getElectronicStructure<SPIN>()
      ->getDensityMatrixController());

  ABExchangePotential<SPIN> abExchangePot(
      systemControllerA,
      basisA,
      basisA,
      envDmat,
  ...
  ABCoulombInteractionPotential<SPIN> abCoulombIntPot(
      systemControllerA,
      basisA,
      basisA,
      envDmat);

  HFPotential<SPIN> hfPotential(systemControllerA,envDmat[0],1.0,1E-10);
...
//  auto basisB = systemControllerA->getBasisController();
  ...
  ExchangeInteractionPotential<SPIN> exchangeInteractionPotential(
      basisA,envDmat,1.0,1E-10);
