...
ABNAddFuncPotential<SCFMode>::ABNAddFuncPotential(
  ...
  //density matrices
  //active density
  _actSystem->getElectronicStructure<SCFMode>()->getDensityMatrixController()
  ...
  auto basisFunctionsOnGridA = BasisFunctionOnGridControllerFactory::produce(
      _actSystem->getSettings(), this->_basisA, grid);
  auto basisFunctionsOnGridB = BasisFunctionOnGridControllerFactory::produce(
      _actSystem->getSettings(), this->_basisB, grid);

  _gridToMatrix_AB = std::make_shared<ScalarOperatorToMatrixAdder<SCFMode> >(
      basisFunctionsOnGridA,
      basisFunctionsOnGridB,
      _actSystem->getSettings().grid.blockAveThreshold);
}
...
SPMatrix<SCFMode>& ABNAddFuncPotential<SCFMode>::getMatrix() {
  ...
  if(!_abPotential) {
    ...
    if(!_envDensityOnGrid) {
      ...
      bool isGGA = _functional.getFunctionalClass() == FUNCTIONAL_CLASSES::GGA;
      const auto& actSettings = _actSystem->getSettings();
      if (!isGGA) {
        ...
        for (const auto& dMat : _densityMatrices) {
          auto basisFunctionsOnGridC = BasisFunctionOnGridControllerFactory::produce(
              actSettings, dMat->getDensityMatrix().getBasisController(), _grid);
          DensityOnGridCalculator<SCFMode> densOnGridCalc(
              basisFunctionsOnGridC,actSettings.grid.blockAveThreshold);
    ...
    if (!isGGA) {
      ...
      auto basisFunctionsOnGridAct = BasisFunctionOnGridControllerFactory::produce(
          actSettings,
          _actSystem->getElectronicStructure<SCFMode>()->getDensityMatrix().getBasisController(),
          _grid);
      DensityOnGridCalculator<SCFMode> densOnGridCalc(
          basisFunctionsOnGridAct,actSettings.grid.blockAveThreshold);
      // Calculating and adding density.
      *actDensityOnGrid = densOnGridCalc.calcDensityOnGrid(_actSystem->getElectronicStructure<SCFMode>()->getDensityMatrix());
      ...
      *actDensityOnGrid = densOnGridCalc.calcDensityAndGradientOnGrid(
          _actSystem->getElectronicStructure<SCFMode>()->getDensityMatrix(),
          *actDensityGradOnGrid);
      *superDensityOnGrid = _envDensityOnGrid->getDensityOnGrid();
      *superDensityOnGrid += *actDensityOnGrid;
      *superDensityGradOnGrid = _envDensityOnGrid->getDensityGradientOnGrid();
      *superDensityGradOnGrid+= *actDensityGradOnGrid;
      // Building controllers.
      actDensOnGridController = std::make_shared<ExternalDensityOnGridController<SCFMode> >(actDensityOnGrid, actDensityGradOnGrid);
      superDensOnGridController = std::make_shared<ExternalDensityOnGridController<SCFMode> >(superDensityOnGrid, superDensityGradOnGrid);
    } /* else !isGGA */
