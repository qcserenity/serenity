...
protected:
  CoulombInteractionPotentialTest(): systemControllerAct(
...
TEST_F(CoulombInteractionPotentialTest, H2_dimer_RI_rFockMatrix) {
  ...
  std::vector<std::shared_ptr<DensityMatrixController<Options::SCF_MODES::RESTRICTED> > > envDmat(1,
      systemControllerEnv->getElectronicStructure<Options::SCF_MODES::RESTRICTED>()
      ->getDensityMatrixController());
  std::vector<std::shared_ptr<BasisController> > envAuxBas(1,
      systemControllerEnv->getBasisController(Options::BASIS_PURPOSES::AUX_COULOMB));

	CoulombInteractionPotential<Options::SCF_MODES::RESTRICTED> coulIntPot(systemControllerAct,
	    {systemControllerEnv},
	    actBasis,
	    envDmat,
	    systemControllerAct->getBasisController(Options::BASIS_PURPOSES::AUX_COULOMB),
	    envAuxBas);
...
TEST_F(CoulombInteractionPotentialTest, H2_dimer_RI_uFockMatrix) {
  ...
  auto actBasis = systemControllerAct->getBasisController();
  std::vector<std::shared_ptr<DensityMatrixController<Options::SCF_MODES::UNRESTRICTED> > > envDmat(1,
      systemControllerEnv->getElectronicStructure<Options::SCF_MODES::UNRESTRICTED>()
      ->getDensityMatrixController());
  std::vector<std::shared_ptr<BasisController> > envAuxBas(1,
      systemControllerEnv->getBasisController(Options::BASIS_PURPOSES::AUX_COULOMB));

  CoulombInteractionPotential<Options::SCF_MODES::UNRESTRICTED> coulIntPot(systemControllerAct,
      {systemControllerEnv},
      actBasis,
      envDmat,
      systemControllerAct->getBasisController(Options::BASIS_PURPOSES::AUX_COULOMB),
...
TEST_F(CoulombInteractionPotentialTest, H2_dimer_NORI_uFockMatrix) {
  ...
  std::vector<std::shared_ptr<DensityMatrixController<Options::SCF_MODES::UNRESTRICTED> > > envDmat(1,
      ...
      ->getDensityMatrixController());
  std::vector<std::shared_ptr<BasisController> > envAuxBas(1,
      systemControllerEnv->getBasisController());

  CoulombInteractionPotential<Options::SCF_MODES::UNRESTRICTED> coulIntPot(systemControllerAct,
      {systemControllerEnv},
      actBasis,
      envDmat,
      systemControllerAct->getBasisController(),
      envAuxBas);
...
TEST_F(CoulombInteractionPotentialTest, H2_dimer_RI_rGradient) {
  ...
  auto basisController = systemControllerAct->getAtomCenteredBasisController();
  auto auxBasisController = systemControllerAct->getAtomCenteredBasisController(Options::BASIS_PURPOSES::AUX_COULOMB);
