...
ECPInteractionPotential<SCFMode>::ECPInteractionPotential(
    ...
    _actAtoms(actAtoms),
    _envAtoms(envAtoms),
    _envDensities(envDensities) {
  // Build potential for interaction between active denstiy and env ECPs.
  _envActDensECP = std::make_shared<EffectiveCorePotential<SCFMode> > (_actSystem,_envAtoms, this->_basis);
  // Build potential for interaction between env denstiy and act ECPs.
  for (const auto& envDensity : _envDensities) {
    auto newECP = std::make_shared<EffectiveCorePotential<SCFMode> > (
        _actSystem,_actAtoms,
...
Eigen::MatrixXd ECPInteractionPotential<SCFMode>::getGeomGradients() {
  Eigen::MatrixXd gradient(_actSystem->getGeometry()->getAtoms().size(),3);
  gradient.setZero();
  gradient += _envActDensECP->getGeomGradients();
  for (const auto& ecp : _actEnvDensECPs) gradient += ecp->getGeomGradients();
  return gradient;
}
