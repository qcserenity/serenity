...
TEST_F(ECPInteractionPotentialTest, HI_I2_rECPMatrix) {
  ...
  ECPInteractionPotential<scfmode> ecpInt(
      systemController_HI,
      systemController_HI->getGeometry()->getAtoms(),
      systemController_I2->getGeometry()->getAtoms(),
  ...
  // zero due to symmetry.
    ...
    EXPECT_NEAR(ecpInt.getEnergy(
        systemController_HI->getElectronicStructure<scfmode>()->getDensityMatrix()), 7.4618060206549205e-06, 1e-8);
