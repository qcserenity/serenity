...
FuncPotential<SCFMode>::FuncPotential(
...
_densOnGridController = std::make_shared<DensityMatrixDensityOnGridController<SCFMode> >(
    densOnGridCalculator, _dMatController);
...
FuncPotential<SCFMode>::getEnergy(const DensityMatrix<SCFMode>& /*P*/){
  if (!_potential) getMatrix();
  return _energy;
};

template <Options::SCF_MODES SCFMode> FockMatrix<SCFMode>&
FuncPotential<SCFMode>::getMatrix(){
  Timings::takeTime("Active System - Functional Pot.");
  if (!_potential){
    if (_functional.getFunctionalClass() == FUNCTIONAL_CLASSES::MODELL) {
      SAOPPotential<SCFMode> saopCalc (
          this->_basis,
          _system,
...
FuncPotential<SCFMode>::getGeomGradients(){
