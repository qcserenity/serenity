...
FockMatrix<SCFMode>& HCorePotential<SCFMode>::getMatrix() {
  ...
  if (!_potential){
    auto intC = _system->getOneElectronIntegralController();
...
Eigen::MatrixXd HCorePotential<SCFMode>::getGeomGradients() {
  ...
  const auto& orbitalSet = _system->getActiveOrbitalController<SCFMode>();
  ...
  DensityMatrix<SCFMode> matrix(_system->getElectronicStructure<SCFMode>()->getDensityMatrix());
  matrix = calcEnergyWeightedDensityMatrix(_system,orbitalSet);
  for_spin(matrix){
    matrix_spin *= -1.0;
  };

  unsigned int nBasisFunctionsRed = orbitalSet->getBasisController()->getReducedNBasisFunctions();
  auto basisIndicesRed = _system->getAtomCenteredBasisController()->getBasisIndicesRed();
  auto mapping = createBasisToAtomIndexMapping(basisIndicesRed,nBasisFunctionsRed);

  auto& basis = _system->getAtomCenteredBasisController()->getBasis();
...
#pragma omp for schedule(static,1)
    for (unsigned int i = 0; i < basis.size(); ++i) {
      unsigned int offI = _system->getBasisController()->extendedIndex(i);
      const unsigned int nI = basis[i]->getNContracted();
      for (unsigned int j = 0; j <= i; ++j) {
        unsigned int offJ = _system->getBasisController()->extendedIndex(j);
...
DensityMatrix<RESTRICTED> densMatrix(_system->getElectronicStructure<SCFMode>()->getDensityMatrix().total());
...
template <Options::SCF_MODES SCFMode>
std::vector<unsigned int> HCorePotential<SCFMode>::createBasisToAtomIndexMapping(
