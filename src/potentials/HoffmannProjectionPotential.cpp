...
HoffmannProjectionPotential<SCFMode>::HoffmannProjectionPotential(
  ...
  if(checkFunctionalForLRXC(settings.naddXCFunc)||checkFunctionalForLRXC(activeSystem->getSettings().dft.functional)) {
    throw SerenityError("The Hoffmann projection operator does currently not support range separated hybrids.");
  }
  ...
  auto activeDensityMatrixController = _activeSystem->getElectronicStructure<SCFMode>()->getDensityMatrixController();
  for(unsigned int iEnv = 0; iEnv < _environmentSystems.size(); ++iEnv) {
    auto pickedSystem = _environmentSystems[iEnv];
    if(settings.longRangeNaddKinFunc != Options::KINFUNCTIONALS::NONE) {
      ...
      auto orbitalPopulations = MullikenPopulationCalculator<SCFMode>::calculateAtomwiseOrbitalPopulations(
          envCoeff,
          pickedSystem->getOneElectronIntegralController()->getOverlapIntegrals(),
    ...
    if(_s_ABs[iEnv]) {
      ...
      auto otherDensityMatrixController = getOtherDensityMatrixController(
          pickedDensityMatrixController,activeDensityMatrixController,_envDensityCont);
      ...
      _embeddingBundle.push_back(FDEPotentialBundleFactory<SCFMode>::produce(
          ...
          otherDensityMatrixController,
          adjustedSettings_ptr,
          (supersystemgrid)? supersystemgrid : pickedSystem->getGridController(),
          nullptr,
          topDown));
    }// if _s_ABs[iEnv]
    else {
...
HoffmannProjectionPotential<SCFMode>::adjustSettings(EmbeddingSettings& settings) {
...
template <Options::SCF_MODES SCFMode>std::vector<std::shared_ptr<SystemController> >
HoffmannProjectionPotential<SCFMode>::getOtherSystemController(
    std::shared_ptr<SystemController> pickedSystem) {
  std::vector<std::shared_ptr<SystemController> > otherSystemController = {_activeSystem};
  for(auto env : _environmentSystems) {
    if(env != pickedSystem) otherSystemController.push_back(env);
  }
  return otherSystemController;
}

template <Options::SCF_MODES SCFMode> std::vector<std::shared_ptr<DensityMatrixController<SCFMode> > >
HoffmannProjectionPotential<SCFMode>::getOtherDensityMatrixController(
  ...
  std::vector<std::shared_ptr<DensityMatrixController<SCFMode> > > otherDensityMatrixController
  = {activeDensityMatrixController};
  for(auto env : environmentDensityMatrixController) {
    if(env != pickedDensityMatrixController) otherDensityMatrixController.push_back(env);
  }
  return otherDensityMatrixController;
}

template <Options::SCF_MODES SCFMode> FockMatrix<SCFMode>&
HoffmannProjectionPotential<SCFMode>::getMatrix() {
  if (!_potential) {
    ...
    for(unsigned int iEnv = 0; iEnv < _environmentSystems.size(); ++iEnv) {
      if(_s_ABs[iEnv]) {
        ...
        if(_notProjectedEnvDensities.size() > 0) projectedMatrix -= _notProjectedEnvDensities[iEnv]->getDensityMatrix();
...
HoffmannProjectionPotential<SCFMode>::getGeomGradients() {
...
HoffmannProjectionPotential<SCFMode>::getEnergy(const DensityMatrix<SCFMode>& P) {
  double energy = 0.0;
...
template <Options::SCF_MODES SCFMode> bool
HoffmannProjectionPotential<SCFMode>::checkFunctionalForLRXC(Options::XCFUNCTIONALS functional) {
  auto func = FunctionalClassResolver::resolveFunctional(functional);
