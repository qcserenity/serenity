...
TEST_F(HoffmannProjectionPotentialTest, restricted_unrestricted) {
  ...
  HoffmannProjectionPotential<Options::SCF_MODES::RESTRICTED> hoffPotential(
      act,{env},settings);
...
TEST_F(HoffmannProjectionPotentialTest, unrestricted) {
  ...
  EmbeddingSettings settings;
  settings.naddXCFunc = act->getSettings().dft.functional;
  HoffmannProjectionPotential<Options::SCF_MODES::UNRESTRICTED> hoffPotential(
      act,{env},settings);
  auto f = hoffPotential.getMatrix();
...
/**
 * @test
 * @brief Tests HoffmannProjectionPotential.h/.cpp: Tests restricted fock matrix (Hoffmann) with unrestricted environment.
 */
TEST_F(HoffmannProjectionPotentialTest, restricted_longRangeKinetic) {
  ...
  EmbeddingSettings settings;
  settings.naddXCFunc = act->getSettings().dft.functional;
  settings.longRangeNaddKinFunc = Options::KINFUNCTIONALS::LLP91K;
  ...
  HoffmannProjectionPotential<Options::SCF_MODES::RESTRICTED> hoffPotential(
      act,{env},settings,nullptr,false,supersystemGrid);
  auto f = hoffPotential.getMatrix();
...
/**
 * @test
 * @brief Tests HoffmannProjectionPotentialTest.h/.cpp: Tests restricted fock matrix; with completely truncated projector.
 */
TEST_F(HoffmannProjectionPotentialTest, restricted_truncProjection) {
  ...
  HoffmannProjectionPotential<SPIN> hoffPotential(
      act,{env},settings);
