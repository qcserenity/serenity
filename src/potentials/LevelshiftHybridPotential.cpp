...
LevelshiftHybridPotential<SCFMode>::LevelshiftHybridPotential(
    ...
    Potential<SCFMode>(activeSystemController->getBasisController()),
    _activeSystem(activeSystemController),
    _environmentSystems(envSystems),
    _isHybrid(isHybrid){
  /*
  ...
  for(const auto& envSys : _environmentSystems) {
    ...
    if(_isHybrid) {
      ...
      auto orbitalPopulations = MullikenPopulationCalculator<SCFMode>::calculateAtomwiseOrbitalPopulations(
          envCoeff,
          envSys->getOneElectronIntegralController()->getOverlapIntegrals(),
    ...
    auto projectedDensityMatrixController = std::make_shared<DensityMatrixController<SCFMode> > (
        projectedDensity);
    ...
    _levelshiftPotentials.push_back(std::make_shared<LevelshiftPotential<SCFMode> > (
        ...
        projectedDensityMatrixController,
        levelShiftParameter));
  }//for envSys
  ...
  if(_isHybrid) {
    ...
    _activeSystem->getElectronicStructure<SCFMode>()->getDensityMatrixController()->addSensitiveObject(
        ObjectSensitiveClass<DensityMatrix<SCFMode> >::_self);
...
FockMatrix<SCFMode>& LevelshiftHybridPotential<SCFMode>::getMatrix(){
  if(!_potential) {
    ...
    for(const auto& levelShift : _levelshiftPotentials) *_potential += levelShift->getMatrix();
    if(_naddFuncPotential) *_potential +=_naddFuncPotential->getMatrix();
  }
  return *_potential;
}
...
double LevelshiftHybridPotential<SCFMode>::getEnergy(const DensityMatrix<SCFMode>& P){
  ...
  double energy = 0.0;
  for(const auto& levelShift : _levelshiftPotentials) energy += levelShift->getEnergy(P);
  if(_naddFuncPotential) energy += _naddFuncPotential->getEnergy(P);
  return energy;
}
