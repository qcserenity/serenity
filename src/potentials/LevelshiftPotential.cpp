...
LevelshiftPotential<SCFMode>::LevelshiftPotential(
...
LevelshiftPotential<SCFMode>::getEnergy(const DensityMatrix<SCFMode>& P){
...
LevelshiftPotential<SCFMode>::getMatrix(){
  if (!_potential){

    auto& libint = Libint::getInstance();
    auto overlap = libint.compute1eInts(libint2::Operator::overlap,
         this->_basis,
...
LevelshiftPotential<SCFMode>::getGeomGradients(){
