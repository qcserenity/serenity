...
protected:
        NEInteractionPotentialTest(): systemController(
...
TEST_F(NEInteractionPotentialTest, H2_dimer_rFockMatrix) {
  ...
  std::shared_ptr<BasisController> basis = systemController
      ->getAtomCenteredBasisController();
...
TEST_F(NEInteractionPotentialTest, H2_dimer_rgrad) {
  ...
  std::shared_ptr<BasisController> basisAct = systemControllerAct
      ->getAtomCenteredBasisController();
  std::vector<std::shared_ptr<const Geometry> > geometries;
...
TEST_F(NEInteractionPotentialTest, H2_dimer_ugrad) {
  ...
  NEInteractionPotential<Options::SCF_MODES::UNRESTRICTED> NEPot(systemControllerAct,{systemControllerEnv},basisAct,geometries);
