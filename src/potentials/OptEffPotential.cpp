...
OptEffPotential<SCFMode>::OptEffPotential(
    std::shared_ptr<BasisFunctionOnGridController> basFuncOnGridController,
...
void OptEffPotential<Options::SCF_MODES::RESTRICTED>::calculateOEP(
  ...
  double densDiff=99999.9;
  double oldDensDiff=99999.9;

  SpinPolarizedData<Options::SCF_MODES::RESTRICTED, Eigen::VectorXd> OEPCoeffs(_potBasisFuncOnGridController->getBasisController()->getNBasisFunctions());
  OEPCoeffs.setZero();
...
void OptEffPotential<Options::SCF_MODES::UNRESTRICTED>::calculateOEP(
  ...
  unsigned int cycle=0;
  SpinPolarizedData<Options::SCF_MODES::UNRESTRICTED, double> densDiff(99999.9);
  SpinPolarizedData<Options::SCF_MODES::UNRESTRICTED, double> oldDensDiff(99999.9);
  ...
  auto const updateFunction = [&]
    ...
    SpinPolarizedData<Options::SCF_MODES::UNRESTRICTED, Eigen::VectorXd> tmpGrads=getGradient(targetDens,densMatOnGrid, OEPCoeffs);
...
template<Options::SCF_MODES SCFMode>
void OptEffPotential<SCFMode>::calculateOEP(
    const DensityOnGrid<SCFMode>& targetDens,
...
OptEffPotential<SCFMode>::getGeomGradients(){
...
template<Options::SCF_MODES SCFMode>
SpinPolarizedData<SCFMode, double> OptEffPotential<SCFMode>::calculateOEPLB(
    const DensityOnGrid<SCFMode>& targetDens,
    std::shared_ptr<OrbitalController<SCFMode>> resultOrbitals,
  ...
  SpinPolarizedData<SCFMode, double> oldDensDiff(99999999.9);
      ...
      for (unsigned int gridPt=blockStart;gridPt<blockEnd;gridPt++){
        for_spin(densMatOnGrid, targetDens, oepGrid, densDiff){
          if(targetDens_spin[gridPt]<1e-9){
            ...
            densDiff_spin[omp_get_thread_num()]+=weights[gridPt]*fabs(densMatOnGrid_spin[gridPt]-targetDens_spin[gridPt]);
          }else{
            double tmp=oepGrid_spin[gridPt];
...
template<Options::SCF_MODES SCFMode>
SpinPolarizedData<SCFMode, Eigen::VectorXd> OptEffPotential<SCFMode>::getGradient(
    const DensityOnGrid<SCFMode>& targetDens,
  ...
  for (unsigned int block=0; block<nBlocks; block++){
    ...
    for (unsigned int k=0; k<nBasisFunc; k++){
      for_spin(densMatOnGrid, targetDens, grad){
        grad_spin[k]+=(weights.segment(blockStart,blockEnd-blockStart).array()
            *basisFuncOnGrid->functionValues.col(k).array()
            *(densMatOnGrid_spin.segment(blockStart,blockEnd-blockStart).array()
                -targetDens_spin.segment(blockStart,blockEnd-blockStart).array())).sum();
      };
    }
  }
  /*
...
template<Options::SCF_MODES SCFMode>
std::shared_ptr<SpinPolarizedData<SCFMode, Eigen::MatrixXd>> OptEffPotential<SCFMode>::getHessian(
    std::shared_ptr<OrbitalController<SCFMode>> resultOrbitals){
...
#pragma omp for schedule(dynamic)
  for(unsigned int i=0; i<nBasisFunc; i++){
    for_spin(threeCenterInts,_nOccOrbs){
      Eigen::MatrixXd tmp(_nOccOrbs_spin,_basisFuncOnGridController->getBasisController()->getNBasisFunctions()-_nOccOrbs_spin);
      tmp.setZero();
  ...
  for (unsigned int i=0; i<nThreads; ++i) {
    ...
    for(unsigned int j=0; j<nBasisFunc; j++){
      for_spin(tmpVec,_nOccOrbs){
        Eigen::MatrixXd tmpMat(_nOccOrbs_spin,_basisFuncOnGridController->getBasisController()->getNBasisFunctions()-_nOccOrbs_spin);
        tmpMat.setZero();
    ...
    for (unsigned int basFuncT=0; basFuncT<nBasisFunc; basFuncT++){
      for_spin(_nOccOrbs,MOsOnGrid,ints){
        for (unsigned int occMo=0; occMo<_nOccOrbs_spin; occMo++){
          ...
          intermediateProd.array()=MOsOnGrid_spin.col(occMo).segment(blockStart,nBlock).array()*basisFuncOnGrid->functionValues.col(basFuncT).array();
          for (unsigned int virtMO=0; virtMO<_basisFuncOnGridController->getBasisController()->getNBasisFunctions()-_nOccOrbs_spin; virtMO++){
            ints_spin[basFuncT](occMo, virtMO)+=(weights.segment(blockStart,nBlock).array()*intermediateProd.array()*MOsOnGrid_spin.col(virtMO+_nOccOrbs_spin).segment(blockStart,nBlock).array()).sum();
          }
        }
      };
    }
...
#pragma omp for schedule(dynamic)
    for (unsigned int basFuncU=0; basFuncU<nBasisFunc; basFuncU++){
      for (unsigned int occMo=0; occMo<_nOccOrbs_spin; occMo++){
        for (unsigned int virtMo=0; virtMo<_basisFuncOnGridController->getBasisController()->getNBasisFunctions()-_nOccOrbs_spin; virtMo++){
          double intermediateProd=4*threeCenterInts_spin[basFuncU](occMo,virtMo)/(orbEnergies_spin[occMo]-orbEnergies_spin[virtMo+_nOccOrbs_spin]);
          for (unsigned int basFuncT=0; basFuncT<nBasisFunc; basFuncT++){
...
template<Options::SCF_MODES SCFMode>
void OptEffPotential<SCFMode>::updateDensity(
    DensityOnGrid<SCFMode>& densMatOnGrid,
    std::shared_ptr<OrbitalController<SCFMode>> resultOrbitals,
    const FockMatrix<SCFMode>& initialGuess,
...
#pragma omp for schedule(dynamic)
  ...
  scalarOpToMat.addScalarOperatorToMatrix(fockMat, OEPGrid);
  fockMat+=initialGuess;
  if(_exc>0.0) fockMat+=getX(densMat);
  resultOrbitals->updateOrbitals(fockMat, _oneEIntController);
  densMat = densMatUpdater.getDensityMatrix();
  densMatOnGrid=_densOnGridCalculator->calcDensityOnGrid(densMat);
}
...
FockMatrix<RESTRICTED> OptEffPotential<RESTRICTED>::getX(DensityMatrix<RESTRICTED>& densityMatrix){
  ...
  auto prescreeningFunc = [&](
     ...
     */
    if (maxDens*schwartz < 1e-12) return true;
    ...
    if (maxDBlock*schwartz < 1e-12) return true;
    /*
...
void OptEffPotential<Options::SCF_MODES::UNRESTRICTED>::calculateOEPCarter(
  ...
  coeffs.segment(nBasisFunc*nBasisFunc,nBasisFunc*nBasisFunc)=Eigen::Map<Eigen::VectorXd>(pot.beta.data(), nBasisFunc*nBasisFunc);
  ...
  auto const updateFunction = [&]
    ...
    pot.beta=Eigen::Map<Eigen::MatrixXd>(coeffs.segment(nBasisFunc*nBasisFunc,nBasisFunc*nBasisFunc).data(), nBasisFunc,nBasisFunc);
    ...
    gradients.segment(0,nBasisFunc*nBasisFunc)=Eigen::Map<Eigen::VectorXd>(targetCopy.alpha.data(), nBasisFunc*nBasisFunc);
    gradients.segment(nBasisFunc*nBasisFunc,nBasisFunc*nBasisFunc)=Eigen::Map<Eigen::VectorXd>(targetCopy.beta.data(), nBasisFunc*nBasisFunc);
    gradients.segment(0,nBasisFunc*nBasisFunc)-=Eigen::Map<Eigen::VectorXd>(densMat.alpha.data(), nBasisFunc*nBasisFunc);
    gradients.segment(nBasisFunc*nBasisFunc,nBasisFunc*nBasisFunc)-=Eigen::Map<Eigen::VectorXd>(densMat.beta.data(), nBasisFunc*nBasisFunc);
