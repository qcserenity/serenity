...
template<Options::SCF_MODES SCFMode>class DensityOnGridCalculator;
...
public:
  ...
  OptEffPotential(
      ...
      const double smoothFactor=0.0,
      const double singValThreshold=0.0,
      const double exc=0.0);
   ...
   */
  SpinPolarizedData<SCFMode, double> calculateOEPLB(
      const DensityOnGrid<SCFMode>& targetDens,
      std::shared_ptr<OrbitalController<SCFMode>> resultOrbitals,
      const FockMatrix<SCFMode>& initialGuess,
      double damping = 0.95,
   ...
   *        the Fermi-Amaldi potential is a popular choice.
   */
  void calculateOEPCarter(
      const DensityMatrix<SCFMode>& targetDens,
      std::shared_ptr<OrbitalController<SCFMode>> resultOrbitals,
      const FockMatrix<SCFMode>& initialGuess,
      const unsigned int maxCycles=5000);
...
private:
  ...
  std::shared_ptr<SpinPolarizedData<SCFMode, Eigen::MatrixXd>> getHessian(
      std::shared_ptr<OrbitalController<SCFMode>> resultOrbitals);
  void updateDensity(
