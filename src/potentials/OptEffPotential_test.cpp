...
TEST_F(OptEffPotentialTest, oep) {
  ...
  auto basisFuncOnGridController=BasisFunctionOnGridControllerFactory::produce(
      system->getSettings(),basisController,gridController);
  ...
  CoulombPotentialOnGridCalculator coulOnGridCalc(basisFuncOnGridController);

  GridPotential<Options::SCF_MODES::RESTRICTED> initialGuess(gridController);
  coulOnGridCalc.calculateElectronElectron<Options::SCF_MODES::RESTRICTED>(initialGuess,densMat);
  coulOnGridCalc.calculateElectronNuclei(initialGuess,system->getGeometry()->getAtoms());
  ...
  OptEffPotential<Options::SCF_MODES::RESTRICTED> oepCalc(
      basisFuncOnGridController,
      basisFuncOnGridController,
      oneEIntController,
  ...
  DensityMatrixController<Options::SCF_MODES::RESTRICTED> recDMatController(resultOrbitals,system->getElectronicStructure<Options::SCF_MODES::RESTRICTED>()->getDensityMatrixController()->getOccupations());

  const auto& recDens=densOnGridCalculator->calcDensityOnGrid(recDMatController.getDensityMatrix());

  SpinPolarizedData<Options::SCF_MODES::RESTRICTED, Eigen::VectorXd> dummy(Eigen::VectorXd::Zero(basisController->getNBasisFunctions()));

  auto grad=oepCalc.getGradient(targetDens,
      recDens,
      dummy);
...
TEST_F(OptEffPotentialTest, oepUnres) {
  ...
  coulOnGridCalc.calculateElectronElectron<Options::SCF_MODES::UNRESTRICTED>(tmp,densMat);
  coulOnGridCalc.calculateElectronNuclei(tmp,system->getGeometry()->getAtoms());

  for_spin(initialGuess){
    initialGuess_spin+=tmp;
  };

  OptEffPotential<Options::SCF_MODES::UNRESTRICTED> oepCalc(
      basisFuncOnGridController,
      basisFuncOnGridController,
      oneEIntController,
      densOnGridCalculator,
  ...
  DensityMatrixController<Options::SCF_MODES::UNRESTRICTED> recDMatController(resultOrbitals,system->getElectronicStructure<Options::SCF_MODES::UNRESTRICTED>()->getDensityMatrixController()->getOccupations());

  SpinPolarizedData<Options::SCF_MODES::UNRESTRICTED, Eigen::VectorXd> dummy(Eigen::VectorXd::Zero(basisController->getNBasisFunctions()));
