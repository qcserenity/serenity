...
SAOPPotential<SCFMode>::SAOPPotential(
  ...
  _basisFunctionOnGridController =
      BasisFunctionOnGridControllerFactory::produce(_systemController->getSettings(), this->_basis, this->_grid);
  _gridToMatrix = std::make_shared<ScalarOperatorToMatrixAdder<SCFMode> >(
      _basisFunctionOnGridController, _systemController->getSettings().grid.blockAveThreshold);
}

template <Options::SCF_MODES SCFMode> FockMatrix<SCFMode>&
SAOPPotential<SCFMode>::getMatrix() {
    if (!_potential){
      ...
      if (_orbitalController->orbitalsAvailable()) {
        ...
        for_spin(_nOCC, coefficients, densityMatrices) {
          MatrixInBasis<Options::SCF_MODES::RESTRICTED> newDensityMatrix(_systemController->getBasisController());
         ...
         */
        Functional functional({BASIC_FUNCTIONALS::VWN5C});
        auto funcDataVWN = xcFun.calcData(
            FUNCTIONAL_DATA_TYPE::GRADIENTS,
            functional,
            _densityOnGridController);
        auto& nu_LDA_c = *funcDataVWN.dFdRho;
        functional = Functional({BASIC_FUNCTIONALS::SLATERX});
        auto funcDataSLATER = xcFun.calcData(
            FUNCTIONAL_DATA_TYPE::GRADIENTS,
            functional,
            _densityOnGridController);
         ...
         */
        functional = Functional({BASIC_FUNCTIONALS::PW91C});
        auto funcDataPerdew = xcFun.calcData(
            FUNCTIONAL_DATA_TYPE::GRADIENTS,
            functional,
            _densityOnGridController);
        double energyPerdew = funcDataPerdew.energy;
        auto epuvPerdew = funcDataPerdew.epuv;
        functional = Functional({BASIC_FUNCTIONALS::BECKEX});
        auto funcDataBECKE = xcFun.calcData(
            FUNCTIONAL_DATA_TYPE::GRADIENTS,
            functional,
            _densityOnGridController);
...
#pragma omp parallel for schedule(static,1)
        for (unsigned int iBlock = 0; iBlock < nBlocks; ++iBlock) {
            ...
            for_spin(dRho_dr, density, recRho, nu_lb, nu_LDA_x) {
              if (density_spin(r) > 1E-9) {
                ...
                if(SCFMode == Options::SCF_MODES::UNRESTRICTED) {
                  ...
                  nu_lb_spin(reducedIndex) = nu_LDA_x_spin[r]
                                           -(_beta * x_r * x_r * pow(density_spin(r), 1.0/3.0))
                                           / (1+3*_beta*x_r*log(x_r+sqrt(x_r*x_r+1)));
                } else {
                  auto x_r = dRho_dr_spin.norm()/(2*pow(density_spin(r)/2, 4.0/3.0));
                  nu_lb_spin(reducedIndex) = nu_LDA_x_spin[r]
                                           -(_beta * x_r * x_r * pow(density_spin(r)/2, 1.0/3.0))
                                           / (1+3*_beta*x_r*log(x_r+sqrt(x_r*x_r+1)));
                }
              }
            };
          ...
          for_spin(nu_lb, exp_eps, sqrt_eps, qout, saopGridPotential) {
            ...
            blockPot = ((nu_lb_spin * exp_eps_spin.transpose())
                + ((qout_spin*sqrt_eps_spin) + nu_hole)
                * (E-exp_eps_spin).transpose()).cwiseProduct(qout_spin).rowwise().sum();
            for (unsigned int r = firstGridIndex; r < lastGridIndex; ++r) {
              unsigned int reducedIndex = r - firstGridIndex;
        ...
        assert(evaluatedGridPoints.sum() == static_cast<int>(_grid->getNGridPoints()) && "not all grid points were evaluated! Something is wrong here!");
        /*
        ...
        auto funcData = xcFun.calcData(
            FUNCTIONAL_DATA_TYPE::GRADIENTS,
            FunctionalClassResolver::resolveFunctional(Options::FUNCTIONALS::LDA),
            _densityOnGridController);
...
SAOPPotential<SCFMode>::getEnergy(const DensityMatrix<SCFMode>& /*P*/){
  if (!_potential) getMatrix();
  return _energy;
};

template <Options::SCF_MODES SCFMode>Eigen::MatrixXd
SAOPPotential<SCFMode>::getGeomGradients() {
