...
protected:
  SAOPPotentialTest(): systemController(
      SystemController__TEST_SUPPLY::getSystemController(TEST_SYSTEM_CONTROLLERS::Ne2_6_31Gs)){
  }
...
TEST_F(SAOPPotentialTest, Ne_dimer_rFockMatrix) {
  ...
  auto basisFunctionOnGridController = BasisFunctionOnGridControllerFactory::produce(
      systemController->getSettings(), basis, grid);
  ...
  auto densOnGridCon = std::make_shared<DensityMatrixDensityOnGridController<SCFMode> >(
          densOnGridCalculator, densMatrixCon);
