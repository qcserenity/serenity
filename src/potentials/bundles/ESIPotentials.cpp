...
ESIPotentials<SCFMode>::ESIPotentials(
  ...
  _nePot.reset(new NEInteractionPotential<SCFMode>(_actSystem,_envSystems,_activeDMat->getDensityMatrix().getBasisController(),_envGeoms));
  _coulPot.reset(new CoulombInteractionPotential<SCFMode>(_actSystem,_envSystems,_activeDMat->getDensityMatrix().getBasisController(),_envDMats));
...
ESIPotentials<SCFMode>::getFockMatrix(const DensityMatrix<SCFMode>& P,
  ...
  const auto& Vee = _coulPot->getMatrix();
  ...
  if (!_enAttr||!_nnRep){
    ...
    for (const auto& envGeo : _envGeoms) {
      for (const auto& envAtom : envGeo->getAtoms()) {
        if (envAtom->isDummy()) continue;
        for (const auto& actAtom : _activeGeom->getAtoms()) {
          if (actAtom->isDummy()) continue;
          *_nnRep += (envAtom->getEffectiveCharge() * actAtom->getEffectiveCharge()) /
                      distance(*envAtom, *actAtom);
        }
      }
    }
  }
