...
std::shared_ptr<PotentialBundle<SCFMode> > FDEPotentialBundleFactory<SCFMode>::produce(
  ...
  return produceNew(
      activeSystem,
      activeDensMatController,
      environmentSystems,
      envDensMatController,
      settings,
      grid,
      supersystem,
      topDown,
...
std::shared_ptr<PotentialBundle<SCFMode> > FDEPotentialBundleFactory<SCFMode>::produceNew(
  ...
  if(naddXCfunc.isHybrid()
    ...
    WarningTracker::printWarning(
        "Warning: Exact exchange is calculated with orbitals that may not be orthogonal.",iOOptions.printSCFCycleInfo);
  }
  auto nonAddExchange = std::make_shared<NAddFuncPotential<SCFMode> >(
      ...
      grid,
      naddXCfunc,
      std::make_pair(true,(gridCutOff<0)?
          eConts:std::vector<std::shared_ptr<EnergyComponentController> >(0)),
  ...
  auto ecpInt = std::make_shared<ECPInteractionPotential<SCFMode> >(
      activeSystem,
  ...
  if(settings->embeddingMode==Options::KIN_EMBEDDING_MODES::NADD_FUNC){
    kin = std::make_shared<NAddFuncPotential<SCFMode> >(
        ...
        std::make_pair(false,(gridCutOff<0)?
            eConts:std::vector<std::shared_ptr<EnergyComponentController> >(0)),
    ...
    kin = std::make_shared<LevelshiftHybridPotential<SCFMode> >(
        ...
        topDown,
        std::make_pair(false,(gridCutOff<0 && !topDown)?eConts:std::vector<std::shared_ptr<EnergyComponentController> >(0)));
    ...
    auto superPot_AA = std::shared_ptr<PotentialBundle<SCFMode> >(
        new FDEPotentials<SCFMode>(
            ...
            esiPot,
            nonAddExchange,
            std::shared_ptr<Potential<SCFMode> >(
    ...
    auto superPot_AA = std::shared_ptr<PotentialBundle<SCFMode> >(
        new FDEPotentials<SCFMode>(
            ...
            std::shared_ptr<Potential<SCFMode> >(
                    new ZeroPotential<SCFMode>(activeSystem->getBasisController())),
            ecpInt));
    kin = std::make_shared<HoffmannProjectionPotential<SCFMode> >(
        activeSystem,
        environmentSystems,
        *settings,
        superPot_AA,
    ...
    if(topDown) {
      kin = std::make_shared<TDReconstructionPotential<SCFMode> >(
              ...
              settings->potentialBasis.empty()?
                  environmentSystems[0]->getBasisController()->getBasisString() : settings->potentialBasis,
      ...
      kin=std::make_shared<BUReconstructionPotential<SCFMode>>(
          ...
          settings->smoothFactor,
          settings->potentialBasis.empty()?
              activeSystem->getBasisController()->getBasisString() : settings->potentialBasis,
          settings->singValThreshold,
          settings->lbDamping,
          settings->lbCycles,
          settings->carterCycles);
    ...
    auto superPot_AA = std::shared_ptr<PotentialBundle<SCFMode> >(
        new FDEPotentials<SCFMode>(
            ...
            std::shared_ptr<Potential<SCFMode> >(
                    new ZeroPotential<SCFMode>(activeSystem->getBasisController())),
            ecpInt));
    std::cout << "  Applying a fermi-shifted Huzinaga operator with an fermi level of "
