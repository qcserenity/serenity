...
double SCFAnalysis<Options::SCF_MODES::UNRESTRICTED>::S2(){
  ...
  if (_systemController->getSettings().method == Options::ELECTRONIC_STRUCTURE_THEORIES::DFT) {
    ...
    auto densityOnGridCalculator =
        std::make_shared<DensityOnGridCalculator<Options::SCF_MODES::UNRESTRICTED> >(
    ...
    auto densityOnGridController =
        std::make_shared<DensityMatrixDensityOnGridController<Options::SCF_MODES::UNRESTRICTED> >(
    ...
    for (unsigned int i = 0; i < spinDensity.rows(); ++i) {
      if (spinDensity(i) > 0) spinDensity(i) = 0.0;
    }
    ...
    Eigen::MatrixXd Sab = coeff.alpha.block(0,0,overlap.rows(),nEl.alpha).transpose() * overlap * coeff.beta.block(0,0,overlap.rows(),nEl.beta);
