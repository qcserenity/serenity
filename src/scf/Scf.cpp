...
template <Options::SCF_MODES SCFMode>void Scf<SCFMode>::perform(
  ...
  ConvergenceController<SCFMode> convergenceController(
      settings,
      es->getDensityMatrixController(),
      orbitalController,
      es->getOneElectronIntegralController(),
      energyComponentController);
//  convergenceController.setOrthoS(*orbitalController->getTransformMatrix());
  convergenceController.accelerateConvergence(F);
  ...
  while (true) {
     ...
     */
    convergenceController.accelerateConvergence(F);
    orbitalController->updateOrbitals(convergenceController.getLevelshift(),
                                     F,
                                     es->getOneElectronIntegralController());
    ...
    if (counter>=settings.scf.maxCycles) {
      ...
      if(!allowNotConverged) throw SerenityError(
              (string)"Cancelling SCF after "+settings.scf.maxCycles+" cycles. NOT CONVERGED!!!");
      break;
    }
  }
