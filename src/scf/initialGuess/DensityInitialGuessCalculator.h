...
public:
   ...
   */
  virtual std::unique_ptr<DensityMatrix<Options::SCF_MODES::RESTRICTED> > calculateInitialDensity(
      std::shared_ptr<SystemController> systemController) = 0;
  
  std::unique_ptr<ElectronicStructure<Options::SCF_MODES::RESTRICTED> > calculateInitialGuess(
      const std::shared_ptr<SystemController> systemController) override final;
