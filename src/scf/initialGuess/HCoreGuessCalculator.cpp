...
unique_ptr<ElectronicStructure<SCFMode> > HCoreGuessCalculator<SCFMode>::calculateInitialGuess(
...
//  auto F = oneIntController->getOneElectronIntegrals();
  ...
  auto orbs = shared_ptr<OrbitalController<SCFMode> >(
      new OrbitalController<SCFMode>(
        move(coefficientMatrix),
        systemController->getBasisController(),
        move(eigenvalues)));
  orbs->setCanOrthTh(systemController->getSettings().scf.canOrthThreshold);
  std::unique_ptr<ElectronicStructure<SCFMode> > elecStruct (
      new ElectronicStructure<SCFMode>(
          orbs,
      systemController->getOneElectronIntegralController(),
      systemController->getNOccupiedOrbitals<SCFMode>()));
  return elecStruct;
}
