...
void InitialGuessCalculator<Options::SCF_MODES::UNRESTRICTED>::scrambleOrbitals(
  ...
  if (nElectrons.alpha + nElectrons.beta <= 1 || nElectrons.alpha == 0 || nElectrons.beta == 0 || nElectrons.alpha != nElectrons.beta) return;
  ...
  if (nAlphaElec == nRow) return;
  const unsigned int scramble = (nRow<4 || nAlphaElec <= 2 || nElectrons.beta <= 2 || nRow-nAlphaElec<2 || nRow-nElectrons.beta<2)?1:2;
...
UnrestrictedFromRestrictedGuess::calculateInitialGuess(
  ...
  auto unrestrictedOrbs = std::shared_ptr<OrbitalController<Options::SCF_MODES::UNRESTRICTED> >(new
    OrbitalController<Options::SCF_MODES::UNRESTRICTED>(restrictedOrbs->getBasisController()));
  ...
  SpinPolarizedData<Options::SCF_MODES::UNRESTRICTED, Eigen::VectorXd > unrestrictedEigenvalues = unrestrictedOrbs->getEigenvalues();
  ...
  // Scramble orbitals
  this->scrambleOrbitals(
          *unrestrictedOrbs, systemController->getNElectrons<Options::SCF_MODES::UNRESTRICTED>());
  std::unique_ptr<ElectronicStructure<Options::SCF_MODES::UNRESTRICTED> > urES (
      ...
      unrestrictedOrbs,
      restrictedESguess->getOneElectronIntegralController(),
