...
TEST(SettingsTests, ReadFromFile) {
  ...
  newSettingsFile.open (pathToTestsResources+"TestSystem_H2_6_31Gs_BP86/TestSystem_H2_6_31Gs_BP86.settings", std::ifstream::in);
  Settings settings(newSettingsFile);
  EXPECT_EQ("TestSystem_H2_6_31Gs_BP86",settings.name);
  EXPECT_TRUE(settings.dft.functional==Options::XCFUNCTIONALS::BP86);
}
...
TEST(SettingsTests, Set) {
  ...
  EXPECT_TRUE(settings.dft.functional==Options::XCFUNCTIONALS::PBE0);
}
