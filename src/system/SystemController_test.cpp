...
TEST_F(SystemControllerTest, addition_res_res) {
  ...
  auto cR(result->template getElectronicStructure<RESTRICTED>()->getMolecularOrbitals()->getCoefficients());
  for_spin(cA,cE,cR){
    cR_spin.block(0,0,4,4) -= cA_spin;
    cR_spin.block(4,4,4,4) -= cE_spin;
    EXPECT_NEAR(0.0,cR_spin.sum(),1e-12);
  };
  std::remove((act->getSettings().path+"TestSystem_H2_6_31Gs_ACTIVE_FDE+TestSystem_H2_6_31Gs_ENVIRONMENT_FDE/TestSystem_H2_6_31Gs_ACTIVE_FDE+TestSystem_H2_6_31Gs_ENVIRONMENT_FDE.xyz").c_str());
  std::remove((act->getSettings().path+"TestSystem_H2_6_31Gs_ACTIVE_FDE+TestSystem_H2_6_31Gs_ENVIRONMENT_FDE/TestSystem_H2_6_31Gs_ACTIVE_FDE+TestSystem_H2_6_31Gs_ENVIRONMENT_FDE.settings").c_str());
  std::remove((act->getSettings().path+"TestSystem_H2_6_31Gs_ACTIVE_FDE+TestSystem_H2_6_31Gs_ENVIRONMENT_FDE").c_str());
}
/**
 * @test
 * @brief Tests SystemController.h/.cpp: Tests addition of two systems.
...
EXPECT_NE(nullptr,SystemController__TEST_SUPPLY::getSystemController(TEST_SYSTEM_CONTROLLERS::H2_MINBAS_LARGE_DISTANCE,true));
...
EXPECT_NE(nullptr,SystemController__TEST_SUPPLY::getSystemController(TEST_SYSTEM_CONTROLLERS::H2_6_31Gs_ENVIRONMENT_FDE,true));
...
EXPECT_NE(nullptr,SystemController__TEST_SUPPLY::getSystemController(TEST_SYSTEM_CONTROLLERS::WATER_DEF2_SVP_CAMB3LYP,true));
}
