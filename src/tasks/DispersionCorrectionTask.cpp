...
DispersionCorrectionTask::DispersionCorrectionTask(
...
void DispersionCorrectionTask::run(){
  ...
  printSubSectionTitle("DFT Dispersion Correction Calculation");
   ...
   */
  if (settings.functional == Options::XCFUNCTIONALS::NONE){
    ...
    for (unsigned int i=0;i<grad.rows();++i){
      printf("%4s  %5d %2s %+15.10f  %+15.10f  %+15.10f\n","",(i+1),
                                                              _systemController->getGeometry()
                                                                                ->getAtoms()[i]->getAtomType()
                                                                                ->getElementSymbol().c_str(),
  ...
  if (settings.hessian){
    ...
    for (auto dir : hess){
      for (unsigned int i=0;i<dir.rows();++i){
        printf("%4s  %5d %2s %+15.10f","",(i+1),_systemController->getGeometry()
                                                                 ->getAtoms()[i]->getAtomType()
                                                                 ->getElementSymbol().c_str(),
                                               dir(i,0));
