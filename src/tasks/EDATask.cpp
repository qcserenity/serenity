...
void EDATask<SCFMode>::run(){
  ...
  double eES,eESX,eESXCT,eESXPLX,eESPL,eESXEX,eInt,e0;
  {
    ...
    scf.run();
    e0 =   _systemA->getElectronicStructure<SCFMode>()->getEnergy()
          +_systemB->getElectronicStructure<SCFMode>()->getEnergy();
  ...
  printf("%12s %9f %8s %14f %8s %14f %6s\n",
      "E_{ES}= ",(eES - e0), "Hartree", (eES - e0) * HARTREE_TO_KCAL_PER_MOL, " kcal/mol",
      (eES - e0) * HARTREE_TO_KJ_PER_MOL, "kJ/mol");
  printf("%12s %9f %8s %14f %8s %14f %6s\n",
      "E_{EX}= ",(eESXEX - eES), "Hartree", (eESXEX - eES) * HARTREE_TO_KCAL_PER_MOL, " kcal/mol",
      (eESXEX - eES) * HARTREE_TO_KJ_PER_MOL, "kJ/mol");
  printf("%12s %9f %8s %14f %8s %14f %6s\n",
      "E_{PL}= ",(eESPL - eES), "Hartree", (eESPL - eES) * HARTREE_TO_KCAL_PER_MOL, " kcal/mol",
      (eESPL - eES) * HARTREE_TO_KJ_PER_MOL, "kJ/mol");
  printf("%12s %9f %8s %14f %8s %14f %6s\n",
      "E_{CT}= ",(eESXCT - eESX), "Hartree", (eESXCT - eESX) * HARTREE_TO_KCAL_PER_MOL, " kcal/mol",
      (eESXCT - eESX) * HARTREE_TO_KJ_PER_MOL, "kJ/mol");
  printf("%12s %9f %8s %14f %8s %14f %6s\n",
      "E_{EXPL}= ",(eES+eESXPLX-eESX-eESPL), "Hartree", (eES+eESXPLX-eESX-eESPL) * HARTREE_TO_KCAL_PER_MOL, " kcal/mol",
  ...
  printf("%11s\n","-------------");
  printf("%12s %9f %8s %14f %8s %14f %6s\n",
      "E_{inter}= ",eInt, "Hartree", eInt * HARTREE_TO_KCAL_PER_MOL, " kcal/mol",
      eInt * HARTREE_TO_KJ_PER_MOL, "kJ/mol");
  printf("%11s\n","-------------");
}
