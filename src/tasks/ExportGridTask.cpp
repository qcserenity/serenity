...
ExportGridTask::ExportGridTask( const std::shared_ptr<SystemController> system) :
    ...
    for (unsigned int iAtom = 0;iAtom!=nAtoms;++iAtom){
      for (unsigned int iPoint = indices[iAtom].first ; iPoint != indices[iAtom].second; ++iPoint){
        fprintf (outptFile, "%4d  %+14.10E  %+14.10E  %+14.10E  %+14.10E\n"
                ,iAtom,points(0,iPoint),points(1,iPoint),points(2,iPoint),weights[iPoint]);
      }
    }
  }else{
    ...
    for (unsigned int iAtom = 0;iAtom!=nAtoms;++iAtom){
      for (unsigned int iPoint = indices[iAtom].first ; iPoint != indices[iAtom].second; ++iPoint){
        fprintf (outptFile, " %+14.10E  %+14.10E  %+14.10E  %+14.10E\n"
                ,points(0,iPoint),points(1,iPoint),points(2,iPoint),weights[iPoint]);
      }
    }
  }
  fclose (outptFile);
