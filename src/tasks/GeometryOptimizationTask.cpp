...
GeometryOptimizationTask<SCFMode>::GeometryOptimizationTask(
...
void GeometryOptimizationTask<SCFMode>::run() {
  ...
  if (_passiveSystems.size()==0 and _activeSystems.size()==1){
    ...
    auto const updateFunction = [&]
      ...
      if(printInfo)printSubSectionTitle("Cycle: "+std::to_string(cycle));
       ...
       */
      if(printInfo)printSmallCaption("Geometry Relaxation");
      ...
      _activeSystems[0]->getGeometry()->updateTrajFile(_activeSystems[0]->getHDF5BaseName(),
          value,gradNorm);
      ...
      if (abs(RMSgrad) < settings.rmsgradThresh) convCriteriaMet++;
      if (abs(value-oldEnergy) < settings.energyChangeThresh) convCriteriaMet++;
      if (gradients.array().abs().maxCoeff() < settings.maxGradThresh) convCriteriaMet++;
      if (std::sqrt(step.squaredNorm()/step.size()) < settings.stepThresh) convCriteriaMet++;
      if (step.array().abs().maxCoeff() < settings.maxStepThresh) convCriteriaMet++;
      if (convCriteriaMet>=3 and cycle>1){
        print("Convergence reached. Exiting...");
        converged=true;
      }
      if (cycle==settings.maxCycles) {
        if(printInfo)WarningTracker::printWarning("WARNING: Geometry Optimization not yet converged!",true);
        converged=true;
      }
    ...
    assert(_activeSystems[0]->getSettings().method == Options::ELECTRONIC_STRUCTURE_THEORIES::DFT && "Check logic in this task for its compatible with HF.");
    double oldEnergy=_activeSystems[0]->getElectronicStructure<SCFMode>()->getEnergy(ENERGY_CONTRIBUTIONS::FDE_SUPERSYSTEM_ENERGY_DFT_DFT);
    ...
    auto const updateFunction = [&]
      ...
      (void)hessian;
      if(printInfo) printSubSectionTitle("Cycle: "+std::to_string(optCycle));
      ...
      assert(_activeSystems[0]->getSettings().method == Options::ELECTRONIC_STRUCTURE_THEORIES::DFT && "Check logic in this task for its compatibility with HF.");
      value =_activeSystems[0]->getElectronicStructure<SCFMode>()->getEnergy(ENERGY_CONTRIBUTIONS::FDE_SUPERSYSTEM_ENERGY_DFT_DFT);

      gradients = Eigen::Map<const Eigen::VectorXd>( SupersystemGeometry.getGradients().data(), SupersystemGeometry.getNAtoms()*3);
