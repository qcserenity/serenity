...
TEST_F(GeometryOptimizationTest, h2Hf) {
  ...
  auto geomTask=GeometryOptimizationTask<Options::SCF_MODES::RESTRICTED>(
		  activesystems);
...
TEST_F(GeometryOptimizationTest, H2DimerDftNori) {
  ...
  settings.dft.densityFitting = Options::DENS_FITS::NONE;
  settings.dft.functional = Options::XCFUNCTIONALS::PW91;
  ...
  auto envSystem = SystemController__TEST_SUPPLY::getSystemController(
  ...
  geomTask.settings.naddKinFunc = Options::KINFUNCTIONALS::PW91K;
  geomTask.settings.naddXCFunc = Options::XCFUNCTIONALS::PW91;
  geomTask.run();
  ...
  EXPECT_EQ(0,std::remove((envSystem->getSettings().path+"GeometryOptimizationTest_H2DimerDftNori_env.energies.res.h5").c_str()));
...
TEST_F(GeometryOptimizationTest, CO_DFT_no_RI) {
  ...
  settings.basis.label = "STO-6G";
  auto systemController = SystemController__TEST_SUPPLY::getSystemController(
      TEST_SYSTEM_CONTROLLERS::CO_MINBAS, settings);
  std::vector<std::shared_ptr<SystemController> > activesystems;
  activesystems.push_back(systemController);
  auto geomTask=GeometryOptimizationTask<Options::SCF_MODES::RESTRICTED>(
      activesystems);
...
TEST_F(GeometryOptimizationTest, h2DftRi) {
  ...
  settings.basis.label = "STO-6G";
  auto systemController = SystemController__TEST_SUPPLY::getSystemController(
      TEST_SYSTEM_CONTROLLERS::H2_DEF2_TZVP, settings);
  std::vector<std::shared_ptr<SystemController> > activesystems;
  activesystems.push_back(systemController);
  auto geomTask=GeometryOptimizationTask<Options::SCF_MODES::RESTRICTED>(
		  activesystems);
