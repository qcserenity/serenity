...
void LRSCFTask<SCFMode>::run() {
   ...
   */
  if (settings.nEigen > nDimension)
    settings.nEigen = nDimension;
  ...
  // Specific rotation factor for ORD
  double rotFactor = 1.;
  ...
  if (type == Options::LRSCF_TYPE::COUPLED && settings.nEigen > 0) {
    eigenvectors = LRSCFSetup<SCFMode>::setupFDEcTransformation(settings, _couplingPatternMatrix, _referenceLoadingType,
  ...
  if (fxcSigma) {
    printf("\n Prepare kernel ..\n\n");
    kernel = std::make_shared<Kernel<SCFMode>>(_act, _env, settings.superSystemGrid, settings.embedding.naddKinFunc,
                                               settings.embedding.naddXCFunc, settings.func);
    printf(" .. done.\n\n");
  } /* Calculate kernel */
  ...
  // Exchange sigma to be used and if erf engines need to be kept
  for (const auto& sys : _act) {
    // HF in any system
    if (sys->getSettings().method == Options::ELECTRONIC_STRUCTURE_THEORIES::HF) {
      kSigma = true;
    }
    // Hybrid DFT in any system
    else {
      auto func = resolveFunctional(sys->getSettings().dft.functional);
      if (func.isHybrid() || func.getLRExchangeRatio() > 0.0)
        kSigma = true;
      if (func.isRSHybrid())
        rsHybrid = true;
    }
  }
  // naddXC might also require kSigma
  auto naddXCfunc = resolveFunctional(settings.embedding.naddXCFunc);
  if (naddXCfunc.isHybrid() || naddXCfunc.getLRExchangeRatio())
    kSigma = true;
  if (naddXCfunc.isRSHybrid())
    rsHybrid = true;
  ...
  std::shared_ptr<SigmaVector<SCFMode>> D = nullptr, J = nullptr, F = nullptr, K = nullptr, EO = nullptr;
  // Convenience typedef
  typedef std::function<std::unique_ptr<std::vector<Eigen::MatrixXd>>(std::vector<Eigen::MatrixXd> & guessVectors)> ResponseLambda;
  ...
  ResponseLambda MKSigma = [&](std::vector<Eigen::MatrixXd>& guessVectors) {
    ...
    D = std::make_shared<DeltaESigmaVector<SCFMode>>(_lrscf, guessVectors, settings.pseudoDensityThreshold);
    // Coulomb (only needed for first set of guess vectors (A+B))
    J = std::make_shared<CoulombSigmaVector<SCFMode>>(_lrscf, guessAPB, settings.pseudoDensityThreshold);
    // Exchange
    if (kSigma)
      K = std::make_shared<KSigmaVector<SCFMode>>(_lrscf, guessVectors, settings.pseudoDensityThreshold, xc);
    // Kernel (only needed for first set of guess vectors (A+B))
    if (fxcSigma)
      F = std::make_shared<KernelSigmaVector<SCFMode>>(_lrscf, guessAPB, settings.pseudoDensityThreshold, kernel);
    ...
    if (eoSigma)
      EO = std::make_shared<EOSigmaVector<SCFMode>>(_lrscf, guessVectors, settings.pseudoDensityThreshold,
                                                    settings.embedding.levelShiftParameter, settings.embedding.embeddingMode);

    // Add everything to sigma vectors
    auto sigma = std::unique_ptr<std::vector<Eigen::MatrixXd>>(new std::vector<Eigen::MatrixXd>(guessVectors.size()));
    ...
    for (unsigned iSet = 0; iSet < guessVectors.size(); ++iSet) {
      ...
      if (eoSigma)
        (*sigma)[iSet] += factorEO * EO->getSigma()[iSet];
      //(A+B)
      if (iSet % 2 == 0) {
        (*sigma)[iSet] += factor * J->getSigma()[iSet / 2];
        if (fxcSigma)
          (*sigma)[iSet] += factor * F->getSigma()[iSet / 2];
      }
    }
    return sigma;
  }; /* (A+B)*b and (A-B)*b */
  ...
  ResponseLambda HSigma = [&](std::vector<Eigen::MatrixXd>& guessVectors) {
    ...
    // Orbital-energy differences
    D = std::make_shared<DeltaESigmaVector<SCFMode>>(_lrscf, (*sigma), settings.pseudoDensityThreshold);
    // Coulomb
    J = std::make_shared<CoulombSigmaVector<SCFMode>>(_lrscf, (*sigma), settings.pseudoDensityThreshold);
    // Kernel
    if (fxcSigma)
      F = std::make_shared<KernelSigmaVector<SCFMode>>(_lrscf, (*sigma), settings.pseudoDensityThreshold, kernel);

    for (unsigned iSet = 0; iSet < guessVectors.size(); ++iSet) {
      (*sigma)[iSet] = D->getSigma()[iSet];
      (*sigma)[iSet] += factor * J->getSigma()[iSet];
      if (fxcSigma)
        (*sigma)[iSet] += factor * F->getSigma()[iSet];
      // Multiply again
  ...
  ResponseLambda ASigma = [&](std::vector<Eigen::MatrixXd>& guessVectors) {
    ...
    D = std::make_shared<DeltaESigmaVector<SCFMode>>(_lrscf, guessVectors, settings.pseudoDensityThreshold);
    // Coulomb
    J = std::make_shared<CoulombSigmaVector<SCFMode>>(_lrscf, guessVectors, settings.pseudoDensityThreshold);
    // Exchange
    if (kSigma)
      K = std::make_shared<KSigmaVector<SCFMode>>(_lrscf, guessVectors, settings.pseudoDensityThreshold,
                                                  std::vector<int>(guessVectors.size(), 0));
    // Kernel
    if (fxcSigma)
      F = std::make_shared<KernelSigmaVector<SCFMode>>(_lrscf, guessVectors, settings.pseudoDensityThreshold, kernel);
    // External orthogonality
    if (eoSigma)
      EO = std::make_shared<EOSigmaVector<SCFMode>>(_lrscf, guessVectors, settings.pseudoDensityThreshold,
                                                    settings.embedding.levelShiftParameter, settings.embedding.embeddingMode);
    ...
    for (unsigned iSet = 0; iSet < guessVectors.size(); ++iSet) {
      ...
      if (kSigma)
        (*sigma)[iSet] -= K->getSigma()[iSet];
      if (fxcSigma)
        (*sigma)[iSet] += factor / 2.0 * F->getSigma()[iSet];
      if (eoSigma)
        (*sigma)[iSet] += factorEO * EO->getSigma()[iSet];
    }
  ...
  // Turn off grid output in Eigenvalue- and ResponseSolver
  ...
  // Eigenvalue Solver
  if (settings.nEigen > 0 || eigenvectors) {
    ...
    if (initialSubspace > nDimension)
      initialSubspace = nDimension;

    // Note: If this is a coupled calculation and no full FDEc was requested, only perform a single iteration
    auto eigenSolver = std::make_shared<EigenvalueSolver>(
        settings.saveResponseMatrix, nDimension, settings.nEigen, diagonal, settings.convThresh,
    ...
    for (auto lrscf : _lrscf) {
      if (type != Options::LRSCF_TYPE::COUPLED) {
        ...
        if (!settings.tda)
          (*vec)[1] = (*eigenvectors)[1].block(iStart, 0, nDimI, (*eigenvectors)[1].cols());
        iStart += nDimI;
  ...
  if (!settings.frequencies.empty() || !settings.frequencyRange.empty()) {
    ...
    if (!settings.frequencyRange.empty()) {
      // Sanity check
      assert(settings.frequencyRange.size() == 3 && settings.frequencyRange[0] < settings.frequencyRange[1] &&
             settings.frequencyRange[0] + settings.frequencyRange[1] > settings.frequencyRange[2] &&
             "LRSCFTask :: frequencyRange input error!");
    ...
    // Larger subspace needed as subspace collapse does not seem to work properly for response solver
    settings.maxSubspaceDimension = settings.frequencies.size() * 100;

    auto responseSolver = std::make_shared<ResponseSolver>(
        nDimension, 0, diagonal, settings.convThresh, settings.maxCycles, settings.maxSubspaceDimension,
        settings.frequencies, settings.damping, settings.gauge, (*dipoles->getLengths()), (*dipoles->getVelocities()),
        (*dipoles->getMagnetics()), MKSigma, nullptr);
