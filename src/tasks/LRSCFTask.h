...
template<Options::SCF_MODES SCFMode> class LRSCFController;
...
struct LRSCFTaskSettings {
  LRSCFTaskSettings():
    ...
    func(Options::XCFUNCTIONALS::NONE), 
    analysis(true), 
    setAlpha({}), 
    excludeAlpha({}),
    setBeta({}), 
    ...
    localMO(false), 
    gaugeOrigin({std::numeric_limits<double>::infinity(),
    ...
    {
      embedding.naddXCFunc = Options::XCFUNCTIONALS::NONE;
      embedding.naddKinFunc = Options::KINFUNCTIONALS::NONE;
      embedding.embeddingMode = Options::KIN_EMBEDDING_MODES::NONE;
    };
  REFLECTABLE(
    (unsigned int) nEigen,
    (double) pseudoDensityThreshold,
    (double) convThresh,
    (unsigned int) maxCycles,
    (unsigned int) maxSubspaceDimension,
    (Options::RESPONSE_PROBLEM) responseType,
    (bool) tda,
    (double) dominantThresh,
    (bool) superSystemGrid,
    (Options::XCFUNCTIONALS) func,
    (bool) analysis,
    (std::vector<unsigned int>) setAlpha,
    (std::vector<unsigned int>) excludeAlpha,
    (std::vector<unsigned int>) setBeta,
    (std::vector<unsigned int>) excludeBeta,
    (unsigned int) besleyAtoms,
    (std::vector<double>) besleyCutoff,
    (bool) excludeProjection,
    (std::vector<double>) energyInclusion,
    (std::vector<double>) energyExclusion,
    (std::vector<unsigned int>) uncoupledSubspace,
    (bool) fullFDEc,
    (Options::LRSCF_TYPE) loadType,
    (bool) localMO,
    (std::vector<double>) gaugeOrigin,
    (std::vector<unsigned int>) couplingPattern,
    (double) localVirtualOrbitals,
    (double) envVirtualOrbitals,
    (bool) saveResponseMatrix
  )
  public:
...
public:
  ...
  *                 (FDEc) is determined by the number and choice of uncoupled transitions which were determined before.
  ...
  *        -superSystemGrid: Uses supersystem Grid for the evaluation of the Kernel Contribution in the subsystem TDDFT case
  *        -func: IF another function should be used for the Kernel than for the system during SCF
  *        -analysis: If false, LRSCF analysis and excitation / CD spectra will be suppressed
  *        -setAlpha: Set alpha reference orbitals given a set of reference orbitals (e.g. from a SCF calculation)
  *        -excludeAlpha: Exclude alpha orbitals from provided reference
  ...
  *        -uncoupledSubspace: Uncoupled subspace for the FDEc-LRSCF problem. Given a set of active subsystems, a subspace
  *                            of excitations vectors can be defined by
  *                            uncoupledSubspace { 2 1 2
  *                                            3 4 8 10}
  *                            where the first number n gives the number of states of that subsystem and the following n
  *                            numbers define the respective transitions. For this example, vectors 1 and 2 are taken from
  *                            subsystem one,  and vectors 4, 8 and 10 are chosen from active subsystem 2. If uncoupledSubspace
  *                            is not set, all uncoupled vectors will be used to span the subspace.
  *        -fullFDEc: Solve full FDEc problem using approximate solutions as initial guess
  *        -loadType: Reference states used to build unitary transformation matrix for FDEc calculations
  *        -localMO: Perform supersystem TDDFT with local orbitals
  *        -gaugeOrigin: The gauge origin for dipole integrals (important for properties)
  *        -couplingPattern: Sets a certain coupling pattern:
  *                          For example {1 1 0
  *                                       2 2 0
  *                                       0 0 1} 
  *                          means: Read in coupled solution of 1 and 2 and uncoupled solution from 1 and perform FDEc step
  *                          IMPORTANT: This needs to be done in order coupled -> uncoupled
  *                                     With ordered numbers 1 ... 2 ... 3 etc and the numbers refer to the ordering of the systems in the input (1: first system etc ...)
  *        -localVirtualOrbitals: Select canonical virtual orbitals located on the active subsystem based on a modified overlap criterion
  *        -envVirtualOrbitals: Select canonical virtual orbitals located on the environment subsystems based on a modified overlap criterion
  *        -saveResponseMatrix: Saves TDA response matrix if requested
  *       
  */
