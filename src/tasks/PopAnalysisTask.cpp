...
void PopAnalysisTask<SCFMode>::run(){
  if (settings.mulliken) {
    ...
    if (bfCont){
      ...
      auto bfpop = calculator.calculateBasisFunctionPopulations(
          dMat,
          es->getOneElectronIntegralController()->getOverlapIntegrals());
  ...
  if (settings.hirshfeld) {
    ...
    auto basFuncOnGridController=BasisFunctionOnGridControllerFactory::produce(
      ...
      auto densOnGridCalc=std::make_shared<DensityOnGridCalculator<SCFMode> >(
          basFuncOnGridController, 0.0);
      auto densMatController = _systemController->getElectronicStructure<SCFMode>()->getDensityMatrixController();
      auto densOnGridController = std::make_shared<DensityMatrixDensityOnGridController<SCFMode>>(densOnGridCalc,densMatController);
...
template <>
void PopAnalysisTask<RESTRICTED>::print(std::string type,
    const SpinPolarizedData<RESTRICTED, Eigen::VectorXd >& populations){
...
template <>
void PopAnalysisTask<UNRESTRICTED>::print(std::string type,
    const SpinPolarizedData<UNRESTRICTED, Eigen::VectorXd >& populations){
  // getting the atoms of the system and the number of atoms.
  const auto& atoms = _systemController->getAtoms();
  unsigned int nAtoms = atoms.size();
  printSubSectionTitle(type + " Population Analysis" );
  ...
  for (unsigned int i=0; i<nAtoms; i++) {
    ...
    if (atoms[i]->usesECP()) {
      printf( "%4s %5d %9s %11f %11f %11f %2s %3i %24s\n",
            ...
            populations.alpha[i]+populations.beta[i],
            populations.alpha[i]-populations.beta[i],charge,
            " (", atoms[i]->getNCoreElectrons(), " core electrons present)");
    } else {
