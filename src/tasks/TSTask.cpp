...
TSTask::TSTask(std::shared_ptr<SystemController> ts,
...
void TSTask::run() {
  ...
  assert(_ts);
  auto system=_ts;
  auto getData=[&system](const Eigen::VectorXd& parameters,
      double& value,
    ...
    if (print){
      system->getGeometry()->printToFile(system->getHDF5BaseName(), system->getSettings().identifier);
  ...
  if (_env.size()==0){
    ...
    std::string hessianpath(_ts->getSettings().path+_ts->getSettings().name+".hess.h5");
    struct stat buffer;
