...
std::map<TEST_GRID_CONTROLLERS, std::shared_ptr<GridController> >
    GridController__TEST_SUPPLY::_testGridControllers;
...
void GridController__TEST_SUPPLY::prepare(TEST_GRID_CONTROLLERS kind) {
  switch (kind) {
    case TEST_GRID_CONTROLLERS::TINY:
    {
      ...
      _testGridControllers[kind] = std::make_shared<GridController>(
              std::unique_ptr<Grid>(new Grid(std::move(points), std::move(weights))));
    }
      break;
    case TEST_GRID_CONTROLLERS::VERY_SMALL:
    {
      std::unique_ptr<Eigen::Matrix3Xd> points(new Eigen::Matrix3Xd(3,5));
      (*points) << -1.0,0.0,0.0,0.8,0.1,
                    0.0,0.5,-0.4,0.1,-0.1,
                    0.0,0.4,-0.5,-0.1,0.1;
