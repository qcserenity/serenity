...
if (len(sys.argv) > 2 + nSubsystems):
  ...
  for iCoupling in range(nFDECouplings):
    coupling = sys.argv[3 + nSubsystems + iCoupling].split("#")
...
if (len(sys.argv) > 2 + nSubsystems + 1 + nFDECouplings):
  ...
  for iCoupling in range(nTCCouplings):
    coupling = sys.argv[3 + nSubsystems + 1 + nFDECouplings + iCoupling].split("#")
    I = name_to_index[coupling[0]]
    J = name_to_index[coupling[1]]
...
# Determine composite index of coupling to be calculated.
DD_coupling_incides = []
if (len(sys.argv) > 2 + nSubsystems + 1 + nFDECouplings + 1 + nTCCouplings):
  nDDCouplings = int(sys.argv[2 + nSubsystems + 1 + nFDECouplings + 1 + nTCCouplings])
  print("\n Including %5i dipole-dipole couplings :"%(nDDCouplings))
  print(" ------------------------------------------------")
  for iCoupling in range(nDDCouplings):
    coupling = sys.argv[3 + nSubsystems + 1 + nFDECouplings + 1 + nTCCouplings + iCoupling].split("#")
    I = name_to_index[coupling[0]]
    J = name_to_index[coupling[1]]
