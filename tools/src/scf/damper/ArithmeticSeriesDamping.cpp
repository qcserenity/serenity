...
ArithmeticSeriesDamping<T>::ArithmeticSeriesDamping(const double dStart, const double dStep, const double dEnd, int iStartUp)
  : Damper<T>(), _dStart(dStart), _dStep(dStep), _dEnd(dEnd), _iStartUp(iStartUp + 1), _dampingFactor(dStart), _initialized(false) {
  assert(_iStartUp >= 0 and _dStart > 0.0 and _dEnd > 0.0 and _dStep > 0.0);
  assert(_dStart >= _dEnd - _dStep);
}

template<Options::SCF_MODES T>
void ArithmeticSeriesDamping<T>::damp(FockMatrix<T>& newMatrix) {
  if (_initialized) {
    // Update iStartUp.
    _iStartUp -= 1;
...
template<Options::SCF_MODES T>
void ArithmeticSeriesDamping<T>::damp(SpinPolarizedData<T, Eigen::MatrixXd>& newMatrix) {
  if (_initialized) {
    // Update iStartUp.
    _iStartUp -= 1;
